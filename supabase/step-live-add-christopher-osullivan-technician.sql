-- Live access add: Christopher O'Sullivan technician access for Taylor Metal Products / Salem, OR.
--
-- This is a live data operation, not a schema migration. Christopher already
-- has an auth.users account, so this grants company membership directly and
-- sets the default location to Salem, OR.
...
with constants as (
  select
    ...
    'christopher.osullivan@taylormetal.com'::text as email,
    'Christopher O''Sullivan'::text as full_name
),
target_user as (
...
where lower(au.email) = lower('christopher.osullivan@taylormetal.com')
