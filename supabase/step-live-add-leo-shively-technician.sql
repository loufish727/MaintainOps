-- Live access add: Leo Shively technician access for Taylor Metal Products / Salem, OR.
--
-- This is a live data operation, not a schema migration. Leo already has an
...
with constants as (
  select
    ...
    'leo.shively@taylormetal.com'::text as email,
    'Leo Shively'::text as full_name
),
target_user as (
...
where lower(au.email) = lower('leo.shively@taylormetal.com')
