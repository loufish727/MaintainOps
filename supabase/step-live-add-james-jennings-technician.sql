-- Live access add: James Jennings technician access for Taylor Metal Products / Salem, OR.
--
-- This is a live data operation, not a schema migration. James already has an
...
with constants as (
  select
    ...
    'james.jennings@taylormetal.com'::text as email,
    'James Jennings'::text as full_name
),
target_user as (
...
where lower(au.email) = lower('james.jennings@taylormetal.com')
