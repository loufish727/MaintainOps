-- Live access add: Stephen DeYoung accounting access for Taylor Metal Products / Salem, OR.
--
-- This is a live data operation, not a schema migration. Stephen already has an
...
with constants as (
  select
    ...
    'stephen.deyoung@taylormetal.com'::text as email,
    'Stephen DeYoung'::text as full_name
),
target_user as (
...
membership as (
  ...
  select company_id, user_id, 'accounting', salem_location_id
  from target_user
...
where lower(au.email) = lower('stephen.deyoung@taylormetal.com')
