(() => {
  // src/appShell/companyLocationState.js
  function createCompanyLocationStateHelpers({
    ...
    function activeLocationStorageKey2(companyId = getActiveCompanyId(), userId = getSessionUserId()) {
      return companyId && userId ? `${activeLocationStorageKeyBase}:${userId}:${companyId}` : activeLocationStorageKeyBase;
    }
    function readStoredActiveLocationId2(companyId = getActiveCompanyId(), userId = getSessionUserId()) {
      const scopedKey = activeLocationStorageKey2(companyId, userId);
    ...
    function persistActiveLocationId2(locationId, companyId = getActiveCompanyId(), userId = getSessionUserId()) {
    ...
    function activeCompanyMembership2() {
      return getCompanies().find((company) => company.id === getActiveCompanyId()) || null;
    }
    function companyOptionLabel2(company) {
      const companies2 = getCompanies();
      const name = company?.name || "Company";
      const duplicateCount = companies2.filter((item) => String(item.name || "").trim().toLowerCase() === String(name).trim().toLowerCase()).length;
      return duplicateCount > 1 ? `${name} (${String(company.id || "").slice(0, 8)})` : name;
    }
    function storedLocationForLoadedCompany2() {
      const locations2 = getLocations();
      const scopedKey = activeLocationStorageKey2();
      const scopedLocationId = scopedKey !== activeLocationStorageKeyBase ? storage.getItem(scopedKey) : "";
      if (scopedLocationId && locations2.some((location) => location.id === scopedLocationId)) {
        return scopedLocationId;
      }
      const storedLocationId = readStoredActiveLocationId2();
      if (storedLocationId && locations2.some((location) => location.id === storedLocationId)) {
        return storedLocationId;
      }
      const activeLocationId2 = getActiveLocationId();
      if (activeLocationId2 && locations2.some((location) => location.id === activeLocationId2)) {
        return activeLocationId2;
      }
      const defaultLocationId = activeCompanyMembership2()?.default_location_id || "";
      if (defaultLocationId && locations2.some((location) => location.id === defaultLocationId)) {
        return defaultLocationId;
      }
      return locations2[0]?.id || "";
    }
    return {
      activeLocationStorageKey: activeLocationStorageKey2,
      readStoredActiveLocationId: readStoredActiveLocationId2,
      persistActiveLocationId: persistActiveLocationId2,
      activeCompanyMembership: activeCompanyMembership2,
      companyOptionLabel: companyOptionLabel2,
      storedLocationForLoadedCompany: storedLocationForLoadedCompany2
    };
  }
  ...
  function createLazyResourceHelpers({
    ...
    function hasPlatformPerformanceDisplayHelpers2() {
      return Boolean(platformPerformanceDisplayHelpers);
    }
    function clearPlatformPerformanceResourcesError2() {
      platformPerformanceResourcesError = "";
    }
    function getPlatformPerformanceDisplayHelpers2() {
    ...
    async function ensurePlatformPerformanceResourcesLoaded2() {
  ...
  `;
    ...
    function renderPlatformPerformanceLazyPanel2(state = {}) {
    ...
    function schedulePlatformPerformanceResourceLoad2() {
      if (getActiveSection() !== "performance" || platformPerformanceDisplayHelpers || platformPerformanceResourcesPromise) return;
      ensurePlatformPerformanceResourcesLoaded2().then(() => {
    ...
    return {
      ...
      clearPlatformPerformanceResourcesError: clearPlatformPerformanceResourcesError2,
      ensureConversionResourcesLoaded: ensureConversionResourcesLoaded2,
      ensurePlatformPerformanceResourcesLoaded: ensurePlatformPerformanceResourcesLoaded2,
      ensureQrLibraryLoaded: ensureQrLibraryLoaded2,
      getConversionDisplayHelpers: getConversionDisplayHelpers2,
      getPlatformPerformanceDisplayHelpers: getPlatformPerformanceDisplayHelpers2,
      hasConversionDisplayHelpers: hasConversionDisplayHelpers2,
      hasPlatformPerformanceDisplayHelpers: hasPlatformPerformanceDisplayHelpers2,
      renderConversionsLazyPanel: renderConversionsLazyPanel2,
      renderPlatformPerformanceLazyPanel: renderPlatformPerformanceLazyPanel2,
      scheduleConversionResourceLoad: scheduleConversionResourceLoad2,
      schedulePlatformPerformanceResourceLoad: schedulePlatformPerformanceResourceLoad2,
  ...
  var PENDING_JOIN_TOKEN_STORAGE_KEY = "maintainops.pendingJoinToken";
  var { escapeHtml } = window.MaintainOpsDom;
  var {
    postgrestSearchTerm,
    isoDate,
    isoDateTime,
  ...
  var { withSetupError } = window.MaintainOpsOperationResults;
  var { withOperationTimeout } = window.MaintainOpsOperationTimeout;
  var { createAuthSessionFlow } = window.MaintainOpsAuthSessionFlow;
  var { shouldRenderForAuthEvent } = window.MaintainOpsAuthRenderPolicy;
  var { createMessageWorkflow } = window.MaintainOpsMessageWorkflow;
  var { createPreventiveMaintenanceWorkflow } = window.MaintainOpsPreventiveMaintenanceWorkflow;
  var { createProcedureWorkflow } = window.MaintainOpsProcedureWorkflow;
  var { createTeamWorkflow } = window.MaintainOpsTeamWorkflow;
  var { createCompanySettingsWorkflow } = window.MaintainOpsCompanySettingsWorkflow;
  var { createAppIssueWorkflow } = window.MaintainOpsAppIssueWorkflow;
  var { createPublicRequestLinkWorkflow } = window.MaintainOpsPublicRequestLinkWorkflow;
  var { createPartInventoryWorkflow } = window.MaintainOpsPartInventoryWorkflow;
  var { createWorkOrderQuickUpdateWorkflow } = window.MaintainOpsWorkOrderQuickUpdateWorkflow;
  var { createAssetWorkflow } = window.MaintainOpsAssetWorkflow;
  var { createRequestLifecycleWorkflow } = window.MaintainOpsRequestLifecycleWorkflow;
  var { createWorkOrderCreationWorkflow } = window.MaintainOpsWorkOrderCreationWorkflow;
  var { createWorkOrderDetailEditWorkflow } = window.MaintainOpsWorkOrderDetailEditWorkflow;
  var { createPartUsageWorkflow } = window.MaintainOpsPartUsageWorkflow;
  var { createMediaStorageWorkflow } = window.MaintainOpsMediaStorageWorkflow;
  var { createCompanyLogoWorkflow } = window.MaintainOpsCompanyLogoWorkflow;
  var { createAssetFinancialWorkflow } = window.MaintainOpsAssetFinancialWorkflow;
  var { createPartDeleteWorkflow } = window.MaintainOpsPartDeleteWorkflow;
  var { createProcedureChecklistWorkflow } = window.MaintainOpsProcedureChecklistWorkflow;
  ...
  var { bindWorkSectionJumpEvents } = window.MaintainOpsWorkSectionJumpEvents;
  var { bindGlobalSearchNavigationEvents } = window.MaintainOpsGlobalSearchNavigationEvents;
  var { bindWorkspaceSearchEvents } = window.MaintainOpsWorkspaceSearchEvents;
  var { bindWorkspaceFilterPaginationEvents } = window.MaintainOpsWorkspaceFilterPaginationEvents;
  var { bindWorkspaceFinancialNavigationEvents } = window.MaintainOpsWorkspaceFinancialNavigationEvents;
  var { bindWorkspaceDetailNavigationEvents } = window.MaintainOpsWorkspaceDetailNavigationEvents;
  var { bindWorkspaceInventoryFilterEvents } = window.MaintainOpsWorkspaceInventoryFilterEvents;
  var { bindWorkspaceWorkOrderStatusEvents } = window.MaintainOpsWorkspaceWorkOrderStatusEvents;
  var { bindWorkspaceWorkOrderAssignmentEvents } = window.MaintainOpsWorkspaceWorkOrderAssignmentEvents;
  var { bindWorkspaceWorkOrderDowntimeEvents } = window.MaintainOpsWorkspaceWorkOrderDowntimeEvents;
  var { bindWorkspaceWorkOrderDetailStatusEvents } = window.MaintainOpsWorkspaceWorkOrderDetailStatusEvents;
  var { createWorkspaceWorkOrderCompletionEvents } = window.MaintainOpsWorkspaceWorkOrderCompletionEvents;
  var { createWorkspaceWorkOrderDeleteEvents } = window.MaintainOpsWorkspaceWorkOrderDeleteEvents;
  var { bindWorkspaceTeamWorkViewEvents } = window.MaintainOpsWorkspaceTeamWorkViewEvents;
  var { bindWorkspacePartDetailEvents } = window.MaintainOpsWorkspacePartDetailEvents;
  var { bindWorkspaceMessageUiEvents } = window.MaintainOpsWorkspaceMessageUiEvents;
  var { bindWorkspacePartSearchEvents } = window.MaintainOpsWorkspacePartSearchEvents;
  var { bindWorkspaceManagerDashboardEvents } = window.MaintainOpsWorkspaceManagerDashboardEvents;
  var { bindWorkspaceSectionNavigationEvents } = window.MaintainOpsWorkspaceSectionNavigationEvents;
  var { bindWorkspaceMessageThreadEvents } = window.MaintainOpsWorkspaceMessageThreadEvents;
  var { bindWorkspaceIssueAdminUiEvents } = window.MaintainOpsWorkspaceIssueAdminUiEvents;
  var { bindWorkspacePartDeleteCancelEvents } = window.MaintainOpsWorkspacePartDeleteCancelEvents;
  var { bindWorkspaceWorkMessageStartEvents } = window.MaintainOpsWorkspaceWorkMessageStartEvents;
  var { bindWorkspaceReportIssueCommandEvents } = window.MaintainOpsWorkspaceReportIssueCommandEvents;
  var { bindWorkspaceSubmitRequestCommandEvents } = window.MaintainOpsWorkspaceSubmitRequestCommandEvents;
  var { bindWorkspaceNewWorkOrderCommandEvents } = window.MaintainOpsWorkspaceNewWorkOrderCommandEvents;
  var { bindWorkspaceExportCsvCommandEvents } = window.MaintainOpsWorkspaceExportCsvCommandEvents;
  var { bindWorkspaceAssetDeleteCancelEvents } = window.MaintainOpsWorkspaceAssetDeleteCancelEvents;
  var { bindWorkspaceRequestDeleteCancelEvents } = window.MaintainOpsWorkspaceRequestDeleteCancelEvents;
  var { bindWorkspaceScheduleDeleteCancelEvents } = window.MaintainOpsWorkspaceScheduleDeleteCancelEvents;
  var { bindWorkspaceProcedureDeleteCancelEvents } = window.MaintainOpsWorkspaceProcedureDeleteCancelEvents;
  var { autoGrowTextarea, bindWorkspaceTextareaAutoGrow } = window.MaintainOpsWorkspaceTextareaAutoGrow;
  var { bindWorkspaceDatePickerControls } = window.MaintainOpsWorkspaceDatePickerControls;
  var { bindWorkspaceTeamInviteCancelEvents } = window.MaintainOpsWorkspaceTeamInviteCancelEvents;
  var { bindWorkspaceTeamInviteCopyEvents } = window.MaintainOpsWorkspaceTeamInviteCopyEvents;
  var { bindWorkspaceQuickFixCommandEvents } = window.MaintainOpsWorkspaceQuickFixCommandEvents;
  var { bindWorkspaceAssetQuickFixEvents } = window.MaintainOpsWorkspaceAssetQuickFixEvents;
  var { bindWorkspacePublicRequestLinkCopyEvents } = window.MaintainOpsWorkspacePublicRequestLinkCopyEvents;
  var { bindWorkspacePublicRequestLinkAdminEvents } = window.MaintainOpsWorkspacePublicRequestLinkAdminEvents;
  ...
  var { createRequestQueryFilterHelpers } = window.MaintainOpsRequestQueryFilters;
  var { createWorkOrderSearchHelpers } = window.MaintainOpsWorkOrderSearch;
  var { createWorkspaceListBuilders } = window.MaintainOpsWorkspaceListBuilders;
  var { listLocations, createLocation: createLocationRecord } = window.MaintainOpsLocationsService;
  var {
    listProfiles,
    listCompanyMembers,
    listTeamInvites,
  ...
  var { listParts } = window.MaintainOpsPartsService;
  var { listAssets, listAssetFinancials } = window.MaintainOpsAssetsService;
  var {
    selectWorkOrders,
  ...
  var { notifyRequestEmailer } = window.MaintainOpsRequestEmailNotificationService;
  var { addSignedUrlsToRows, createDeferredSignedUrlLoader } = window.MaintainOpsSignedUrlService;
  var { createWorkspaceQueueLoaders } = window.MaintainOpsWorkspaceQueueLoadersService;
  var {
    listAppIssueReports,
  ...
  var { createRelationshipDisplayHelpers } = window.MaintainOpsRelationshipDisplay;
  var { createDashboardDisplayHelpers } = window.MaintainOpsDashboardDisplay;
  var { segmentIcon, navIcon } = window.MaintainOpsIconDisplay;
  var { assetTypeLabel, assetStatusLabel } = window.MaintainOpsEquipmentLabels;
  var { createEmptyStateTextHelpers } = window.MaintainOpsEmptyStateText;
  var { createRequestDisplayHelpers } = window.MaintainOpsRequestDisplay;
  var { createGlobalSearchDisplayHelpers } = window.MaintainOpsGlobalSearchDisplay;
  var { createWorkQueueDisplayHelpers } = window.MaintainOpsWorkQueueDisplay;
  var { createPlanningDisplayHelpers } = window.MaintainOpsPlanningDisplay;
  var { createMiniWorkOrderDisplayHelpers } = window.MaintainOpsMiniWorkOrderDisplay;
  var { createPaginationDisplayHelpers } = window.MaintainOpsPaginationDisplay;
  var { createPartsDisplayHelpers } = window.MaintainOpsPartsDisplay;
  var { createOptionDisplayHelpers } = window.MaintainOpsOptionDisplay;
  var { createSetupDisplayHelpers } = window.MaintainOpsSetupDisplay;
  var { createStorageDashboardDisplayHelpers } = window.MaintainOpsStorageDashboardDisplay;
  var { createRequestPhotoDisplayHelpers } = window.MaintainOpsRequestPhotoDisplay;
  var { createMessageBadgeDisplayHelpers } = window.MaintainOpsMessageBadgeDisplay;
  var { createNavBadgeDisplayHelpers } = window.MaintainOpsNavBadgeDisplay;
  var { createAppIssueDisplayHelpers } = window.MaintainOpsAppIssueDisplay;
  var { createWorkMessageDisplayHelpers } = window.MaintainOpsWorkMessageDisplay;
  var { createWorkRecommendationDisplayHelpers } = window.MaintainOpsWorkRecommendationDisplay;
  var { createCommandCardDisplayHelpers } = window.MaintainOpsCommandCardDisplay;
  var { createWorkCommandDisplayHelpers } = window.MaintainOpsWorkCommandDisplay;
  var { createMissingWorkDetailDisplayHelpers } = window.MaintainOpsMissingWorkDetailDisplay;
  var { createPartSourceDisplayHelpers } = window.MaintainOpsPartSourceDisplay;
  var { createAssetCardDisplayHelpers } = window.MaintainOpsAssetCardDisplay;
  var { createFinancialDisplayHelpers } = window.MaintainOpsFinancialDisplay;
  var { createProcedureOptionsDisplayHelpers } = window.MaintainOpsProcedureOptionsDisplay;
  var { createMessageThreadLabelDisplayHelpers } = window.MaintainOpsMessageThreadLabelDisplay;
  var { createMessageThreadButtonDisplayHelpers } = window.MaintainOpsMessageThreadButtonDisplay;
  var { createMessageComposerDisplayHelpers } = window.MaintainOpsMessageComposerDisplay;
  var { createAppIssuePanelDisplayHelpers } = window.MaintainOpsAppIssuePanelDisplay;
  var { createInviteLocationDisplayHelpers } = window.MaintainOpsInviteLocationDisplay;
  var { createPartSetupDisplayHelpers } = window.MaintainOpsPartSetupDisplay;
  var { createTeamMemberDisplayHelpers } = window.MaintainOpsTeamMemberDisplay;
  var { createTeamWorkloadDisplayHelpers } = window.MaintainOpsTeamWorkloadDisplay;
  var { createManagerDashboardDisplayHelpers } = window.MaintainOpsManagerDashboardDisplay;
  var { createLocationDisplayHelpers } = window.MaintainOpsLocationDisplay;
  var { createDowntimeEmailDisplayHelpers } = window.MaintainOpsDowntimeEmailDisplay;
  var { createSetupErrorDisplayHelpers } = window.MaintainOpsSetupErrorDisplay;
  var { createWorkOrderErrorDisplayHelpers } = window.MaintainOpsWorkOrderErrorDisplay;
  var { createAssignmentDisplayHelpers } = window.MaintainOpsAssignmentDisplay;
  var { createWorkOrderDescriptionDisplayHelpers } = window.MaintainOpsWorkOrderDescriptionDisplay;
  var { createWorkOrderChangeDisplayHelpers } = window.MaintainOpsWorkOrderChangeDisplay;
  var { createActivityFeedDisplayHelpers } = window.MaintainOpsActivityFeedDisplay;
  var { createPartInventoryDisplayHelpers } = window.MaintainOpsPartInventoryDisplay;
  var { createPartUsageDisplayHelpers } = window.MaintainOpsPartUsageDisplay;
  var { createRequestQueueDisplayHelpers } = window.MaintainOpsRequestQueueDisplay;
  var { createDeleteBlockerDisplayHelpers } = window.MaintainOpsDeleteBlockerDisplay;
  var { createAssetHierarchyDisplayHelpers } = window.MaintainOpsAssetHierarchyDisplay;
  var { createMaintenanceListDisplayHelpers } = window.MaintainOpsMaintenanceListDisplay;
  var { createSearchFilterDisplayHelpers } = window.MaintainOpsSearchFilterDisplay;
  var { createWorkOrderSortDisplayHelpers } = window.MaintainOpsWorkOrderSortDisplay;
  var { createLocationFilterDisplayHelpers } = window.MaintainOpsLocationFilterDisplay;
  var { createMessageThreadFilterDisplayHelpers } = window.MaintainOpsMessageThreadFilterDisplay;
  var { createSetupStatusDisplayHelpers } = window.MaintainOpsSetupStatusDisplay;
  var { createWorkOrderStatusFilterDisplayHelpers } = window.MaintainOpsWorkOrderStatusFilterDisplay;
  var { createWorkOrderSearchDisplayHelpers } = window.MaintainOpsWorkOrderSearchDisplay;
  var { createMyWorkQueueDisplayHelpers } = window.MaintainOpsMyWorkQueueDisplay;
  var { createMessageCenterErrorDisplayHelpers } = window.MaintainOpsMessageCenterErrorDisplay;
  var { createAppIssueErrorDisplayHelpers } = window.MaintainOpsAppIssueErrorDisplay;
  var { createWorkOrderDetailDisplayHelpers } = window.MaintainOpsWorkOrderDetailDisplay;
  var { createAssetDetailDisplayHelpers } = window.MaintainOpsAssetDetailDisplay;
  var { createEquipmentStructureGuideDisplayHelpers } = window.MaintainOpsEquipmentStructureGuideDisplay;
  var { createMessageCenterDisplayHelpers } = window.MaintainOpsMessageCenterDisplay;
  var { createCreateWorkOrderDisplayHelpers } = window.MaintainOpsCreateWorkOrderDisplay;
  var { createQuickFixDisplayHelpers } = window.MaintainOpsQuickFixDisplay;
  var {
    workspaceLoading,
    workspaceLoadError,
    authForm,
  ...
  var { createQuickFixWorkflow } = window.MaintainOpsQuickFixWorkflow;
  var workspaceUiState = createWorkspaceUiState({ storage: localStorage });
  var {
  ...
  var { createMessageDisplayHelpers } = window.MaintainOpsMessageDisplay;
  var { renderEquipmentStructureGuide } = createEquipmentStructureGuideDisplayHelpers();
  var supabaseClient;
  var session;
  var QR_CODE_RESOURCE = {
  ...
  var managerCompletedWorkReady = true;
  var assetRelationshipOpenKeys = /* @__PURE__ */ new Set();
  var assetRelationshipPages = {};
  var maintenanceRequests = [];
  var requestServerTotal = 0;
  var requestDashboardCounts = { active: 0, converted: 0, all: 0 };
  var requestsReady = false;
  var publicRequestLinks = [];
  var publicRequestLinksReady = true;
  var preventiveSchedules = [];
  var companyMembers = [];
  var teamInvites = [];
  var teamInvitesReady = true;
  var teamInviteCancelError = "";
  var teamInviteLinks = [];
  var teamInviteLinksReady = true;
  var teamInviteLinkError = "";
  var requestNotificationRecipients = [];
  var requestNotificationRecipientsReady = true;
  var requestNotificationRecipientError = "";
  var workspaceLoadWarnings = [];
  var messageThreads = [];
  var messageThreadMembers = [];
  var messagesByThreadId = {};
  var messageReadsByThreadId = {};
  var messagesReady = true;
  var messageWorkOrderLinksReady = true;
  var appIssueReports = [];
  var appIssueReportsReady = true;
  var storageDashboard = null;
  var storageDashboardReady = true;
  var storageDashboardError = "";
  var platformPerformance = null;
  var platformPerformanceReady = true;
  var platformPerformanceError = "";
  var platformPerformanceCompanyId = "";
  var platformPerformanceLoadedAt = 0;
  var platformPerformanceLoadPromise = null;
  ...
  var messageThreadFilter = workspaceUiState.getMessageThreadFilter();
  function setMessageThreadFilterState(value) {
    messageThreadFilter = value;
    workspaceUiState.setMessageThreadFilter(value);
  }
  var messageSearchQuery = workspaceUiState.getMessageSearchQuery();
  function setMessageSearchQueryState(value) {
    messageSearchQuery = value;
    workspaceUiState.setMessageSearchQuery(value);
  }
  var messageComposerWorkOrderId = workspaceUiState.getMessageComposerWorkOrderId();
  ...
  var messageComposerOpen = workspaceUiState.getMessageComposerOpen();
  ...
  var parts = [];
  var partCostsReady = true;
  var partSuppliersReady = true;
  var partMachineNotesReady = true;
  var partDocumentsReady = true;
  var partDocumentsByPartId = {};
  var assetParts = [];
  var assetPartsReady = true;
  var assetDocumentsReady = true;
  var assetDocumentsByAssetId = {};
  var assetFinancialsReady = true;
  var assetFinancials = [];
  var assetFinancialsByAssetId = {};
  var assetDocumentSigningByAssetId = {};
  var procedureTemplates = [];
  var proceduresReady = false;
  var schedulesReady = false;
  var outcomesReady = true;
  var safetyChecksReady = true;
  var photosReady = true;
  var adminDeleteSqlConfirmed = localStorage.getItem("maintainops.adminDeleteSqlConfirmed") === "true";
  var partsUsedByWorkOrder = {};
  var eventsByWorkOrder = {};
  var assetEventsByAssetId = {};
  var assetEventsReady = true;
  var commentsByWorkOrder = {};
  var photosByWorkOrder = {};
  var stepResultsByWorkOrder = {};
  var profilesByUserId = {};
  var commentsError = "";
  var requestPhotosReady = true;
  var activeWorkOrderId = workspaceUiState.getActiveWorkOrderId();
  function setActiveWorkOrderIdState(value) {
    activeWorkOrderId = value;
    workspaceUiState.setActiveWorkOrderId(value);
  }
  var activeAssetId = workspaceUiState.getActiveAssetId();
  function setActiveAssetIdState(value) {
    activeAssetId = value;
    workspaceUiState.setActiveAssetId(value);
  }
  var activeAssetHistoryId = null;
  var activeFinancialAssetId = null;
  var activePartId = workspaceUiState.getActivePartId();
  function setActivePartIdState(value) {
    activePartId = value;
    workspaceUiState.setActivePartId(value);
  }
  var pendingDeleteWorkOrderId = null;
  var pendingDeletePartId = null;
  var pendingDeleteAssetId = null;
  var pendingDeleteRequestId = null;
  var pendingDeleteScheduleId = null;
  var pendingDeleteProcedureId = null;
  var pendingCancelInviteId = null;
  var pendingRevokeInviteLinkId = null;
  var showPartSourceManager = false;
  var createWorkOrderMode = false;
  var quickFixMode = false;
  var quickFixAssetId = null;
  var quickFixRequestId = null;
  var publicAppUrlOverride = localStorage.getItem("maintainops.publicAppUrl") || "";
  var {
    publicRequestUrl,
    publicRequestQrUrl,
  ...
  var activeSection = workspaceUiState.getActiveSection();
  ...
  var appError = "";
  var appNotice = "";
  var appNoticeTone = "success";
  var noticeTimer;
  var workOrderActionWarningId = "";
  var workOrderActionWarning = "";
  var { renderCompanyCreate } = createCompanySetupWorkflow({
  ...
  var { applyRequestQueryFilters } = createRequestQueryFilterHelpers({
    activeCompanyId: () => activeCompanyId,
    activeLocationId: () => activeLocationId,
    assets: () => assets,
    locationsReady: () => locationsReady,
  ...
  var {
    ...
    searchQuery: () => workspaceUiState.getSearchQuery(),
    SEARCH_ID_CHUNK_SIZE,
    setExactWorkOrderSearchCache: (value) => {
      exactWorkOrderSearchCache = value;
    },
    setWorkOrderPage: (value) => {
      workspaceUiState.setWorkOrderPage(value);
    },
    setWorkOrderRelatedSearch: (value) => {
      workOrderRelatedSearch = value;
    },
  ...
  var {
    ...
    assetPartRows,
    canDeleteParts,
    deletePartRecord: (id) => supabaseClient.from("parts").delete().eq("id", id).eq("company_id", activeCompanyId).select("id"),
    documentRef: document,
    ...
    showNotice,
    verifyPartDeleted: (id) => supabaseClient.from("parts").select("id").eq("id", id).eq("company_id", activeCompanyId).maybeSingle(),
    withOperationTimeout
  });
  var {
  ...
  var relationshipDisplayHelpers = createRelationshipDisplayHelpers({
    escapeHtml,
    photoMetaText,
    partUsageUnitCost,
    money,
  ...
  var dashboardDisplayHelpers = createDashboardDisplayHelpers({
    escapeHtml,
    getActiveStatusFilter: () => workspaceUiState.getActiveStatusFilter(),
  ...
  function partMachineOptions() {
    return [...new Set(assets.filter(matchesActiveLocation).map((asset) => String(asset.name || "").trim()).filter(Boolean))].sort((a, b) => a.localeCompare(b));
  }
  ...
  var messageDisplayHelpers = createMessageDisplayHelpers({
    escapeHtml,
  ...
  function renderAuth(mode, initialError = "") {
    ...
    document.querySelector("#auth-form").addEventListener("submit", async (event) => {
      ...
      try {
        const response = isSignup ? await withOperationTimeout(
  ...
  async function resetLoginState() {
    ...
    Object.keys(localStorage).filter((key) => key.startsWith("maintainops.") || key.startsWith("sb-")).forEach((key) => localStorage.removeItem(key));
    session = null;
    activeCompanyId = "";
    activeLocationId = "";
    appError = "";
  ...
  async function loadCompanies() {
    ...
    if (!companyRpc.error) {
      companies = (companyRpc.data || []).map((company) => ({
        id: company.id,
        name: company.name,
  ...
  var REQUEST_RELATION_SELECT = "*, assets(name, location_id), locations(name)";
  var REQUEST_ASSET_FALLBACK_SELECT = "*, assets(name)";
  var REQUEST_FALLBACK_SELECT = "*";
  var {
    fetchRequestPage,
    countRequests,
  ...
  async function loadPlatformPerformance(options = {}) {
    ...
    const isFresh = isCurrentCompany && Date.now() - platformPerformanceLoadedAt < 6e4;
  ...
  async function loadMessageCenter() {
    ...
    const { data: threads, error: threadError } = await supabaseClient.from("message_threads").select("*").eq("company_id", activeCompanyId).order("updated_at", { ascending: false });
    if (threadError) {
      messagesReady = false;
      return;
    }
    ...
    const [memberResponse, messageMetaResponse, readResponse] = await Promise.all([
      supabaseClient.from("message_thread_members").select("*").eq("company_id", activeCompanyId).in("thread_id", threadIds),
      supabaseClient.from("messages").select("id, thread_id, sender_id, created_at, deleted_at").eq("company_id", activeCompanyId).in("thread_id", threadIds).order("created_at", { ascending: true }),
      supabaseClient.from("message_reads").select("*").eq("company_id", activeCompanyId).eq("user_id", session.user.id).in("thread_id", threadIds)
    ]);
    ...
    const visibleThreadIds = new Set(messageThreadMembers.filter((member) => member.user_id === session.user.id && !member.deleted_at).map((member) => member.thread_id));
  ...
  async function loadActiveMessageThreadMessages(threadId) {
    if (!threadId) return;
    const activeMessageResponse = await supabaseClient.from("messages").select("*").eq("company_id", activeCompanyId).eq("thread_id", threadId).order("created_at", { ascending: true });
  ...
  async function loadPublicRequestLinks() {
    ...
    const { data, error } = await supabaseClient.from("public_request_links").select("*").eq("company_id", activeCompanyId).order("created_at", { ascending: true });
    if (error) {
  ...
  async function loadComments() {
    ...
    const { data, error } = await supabaseClient.from("work_order_comments").select("*").eq("company_id", activeCompanyId).in("work_order_id", ids).order("created_at", { ascending: true });
    if (error) {
      commentsByWorkOrder = {};
  ...
  async function loadPhotos() {
    ...
    const ids = workOrders.map((workOrder) => workOrder.id);
    const { data, error } = await supabaseClient.from("work_order_photos").select("*").eq("company_id", activeCompanyId).in("work_order_id", ids).order("created_at", { ascending: false });
    if (error) {
      photosReady = false;
      photosByWorkOrder = {};
      return;
  ...
  async function loadPartsUsed() {
    ...
    const ids = workOrders.map((workOrder) => workOrder.id);
    const { data } = await supabaseClient.from("work_order_parts").select("*, parts(*)").eq("company_id", activeCompanyId).in("work_order_id", ids).order("created_at", { ascending: true });
  ...
  async function loadAssetParts() {
    ...
    const { data, error } = await supabaseClient.from("asset_parts").select("*, parts(*)").eq("company_id", activeCompanyId).in("asset_id", ids).order("created_at", { ascending: true });
    if (error) {
      assetParts = [];
  ...
  async function loadAssetDocuments() {
    ...
    const ids = assets.map((asset) => asset.id);
    const { data, error } = await supabaseClient.from("asset_documents").select("*").eq("company_id", activeCompanyId).in("asset_id", ids).order("created_at", { ascending: false });
    if (error) {
    ...
    assetDocumentsReady = true;
    assetDocumentsByAssetId = (data || []).reduce((groups, document2) => {
      groups[document2.asset_id] ||= [];
      groups[document2.asset_id].push(document2);
      return groups;
    }, {});
  }
  async function loadPartDocuments() {
    ...
    const { data, error } = await supabaseClient.from("part_documents").select("*").eq("company_id", activeCompanyId).in("part_id", ids).order("created_at", { ascending: false });
    if (error) {
    ...
    partDocumentsReady = true;
    partDocumentsByPartId = (data || []).reduce((groups, document2) => {
      groups[document2.part_id] ||= [];
      groups[document2.part_id].push(document2);
      return groups;
    }, {});
  ...
  async function loadWorkOrderEvents() {
    ...
    const ids = workOrders.map((workOrder) => workOrder.id);
    const { data } = await supabaseClient.from("work_order_events").select("*").eq("company_id", activeCompanyId).in("work_order_id", ids).order("created_at", { ascending: false });
    if (!data) {
      eventsByWorkOrder = {};
      return;
    }
  ...
  async function loadAssetEvents() {
    ...
    const ids = assets.map((asset) => asset.id);
    const { data, error } = await supabaseClient.from("asset_events").select("*").eq("company_id", activeCompanyId).in("asset_id", ids).order("created_at", { ascending: false });
    if (error) {
      assetEventsByAssetId = {};
  ...
  async function loadStepResults() {
    ...
    const ids = workOrders.map((workOrder) => workOrder.id);
    const { data } = await supabaseClient.from("work_order_step_results").select("*").eq("company_id", activeCompanyId).in("work_order_id", ids);
  ...
  var { ensureGroupSignedUrls: ensureAssetDocumentSignedUrls } = createDeferredSignedUrlLoader({
  ...
  function renderWorkspace() {
    ...
    const existingPerformanceFrame = activeSection === "performance" ? document.querySelector("[data-platform-spatial-frame]") : null;
    ...
    const locationSwitchNote = locationsReady ? canSwitchLocation ? "Changing this moves your active workspace." : "Enable Mobile tech in Team to switch locations." : "Run location setup to enable locations.";
    ...
    const assetAreaOptions = [...new Set(locationAssets.map((asset) => String(asset.location || "").trim()).filter(Boolean))].sort((a, b) => a.localeCompare(b));
  ...
  `;
    ...
    <section class="work-command-summary asset-command-summary asset-master-summary" aria-label="Equipment master summary">
      ...
      const filterAttribute = card.statusFilter ? `data-asset-status-filter="${escapeHtml(card.statusFilter)}"` : `data-asset-type-filter="${escapeHtml(card.typeFilter)}"`;
      return `
  ...
  `;
    ...
    const workAssigneeSortMembers = Object.entries(profilesByUserId).map(([userId, profile2]) => ({ userId, name: profile2?.full_name || teamMemberName(userId) })).filter((member) => member.userId && member.name).sort((a, b) => a.name.localeCompare(b.name));
...
Continue ${actionLabel}?`);
  ...
  var { renderAssetDetail, renderAssetHistoryScreen } = createAssetDetailDisplayHelpers({
    ASSET_TYPE_OPTIONS,
    getAssets: () => assets,
    getActiveAssetId: () => activeAssetId,
    getWorkOrders: () => workOrders,
  ...
  var {
    ...
    getPreventiveSchedules: () => preventiveSchedules,
    getMaintenanceRequests: () => maintenanceRequests,
    getAssetDocumentStoragePaths: (assetId) => (assetDocumentsByAssetId[assetId] || []).map((document2) => document2.storage_path).filter(Boolean),
    ...
    assetDeleteBlockerMessage,
    canDeleteEquipment,
    setAssetPartsReady: (value) => {
      assetPartsReady = value;
    },
  ...
  var {
    ...
    canEditFinancialRecords,
    isMissingTableError,
    setAssetFinancialsReady: (value) => {
      assetFinancialsReady = value;
    },
    loadAssetFinancials,
  ...
  function workOrderDateValue(value) {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(trimmed)) {
      const parsed2 = /* @__PURE__ */ new Date(`${trimmed}T00:00:00`);
      if (Number.isNaN(parsed2.getTime())) throw new Error("Enter a real expected back up / due date as YYYY-MM-DD.");
      const parsedIso = parsed2.toISOString().slice(0, 10);
  ...
  async function updateWithOptionalProcedure(table, payload, id) {
    const response = await supabaseClient.from(table).update(payload).eq("id", id).eq("company_id", activeCompanyId);
  ...
  var { renderMessageCenter } = createMessageCenterDisplayHelpers({
    getMessagesReady: () => messagesReady,
    getMessageThreads: () => messageThreads,
    getActiveMessageThreadId: () => activeMessageThreadId,
    getMessagesByThreadId: () => messagesByThreadId,
  ...
  var {
    ...
    getMessageWorkOrderLinksReady: () => messageWorkOrderLinksReady,
    setMessageWorkOrderLinksReady: (value) => {
      messageWorkOrderLinksReady = value;
    },
    activeLocationDatabaseId,
    ...
    setMessageThreadRead: (threadId, readRow) => {
      messageReadsByThreadId[threadId] = readRow;
    },
    showNotice,
    render: () => render()
  });
  ...
  var {
    ...
    canAdministerTeamRoles,
    getTeamInvitesReady: () => teamInvitesReady,
    setTeamInvitesReady: (value) => {
      teamInvitesReady = value;
    },
    getTeamInviteLinksReady: () => teamInviteLinksReady,
    setTeamInviteLinksReady: (value) => {
      teamInviteLinksReady = value;
    },
    setTeamInviteLinkError: (value) => {
      teamInviteLinkError = value;
    },
    getRequestNotificationRecipientsReady: () => requestNotificationRecipientsReady,
    setRequestNotificationRecipientsReady: (value) => {
      requestNotificationRecipientsReady = value;
    },
    setRequestNotificationRecipientError: (value) => {
      requestNotificationRecipientError = value;
    },
    setPendingCancelInviteId: (value) => {
      pendingCancelInviteId = value;
    },
    setPendingRevokeInviteLinkId: (value) => {
      pendingRevokeInviteLinkId = value;
    },
  ...
  var {
    ...
    setPublicAppUrlOverride: (value) => {
      publicAppUrlOverride = value;
    },
    persistActiveLocationId,
    showNotice,
    render: () => render(),
  ...
  var {
    ...
    getActiveCompanyId: () => activeCompanyId,
    getActiveSection: () => activeSection,
    setAppIssueReportsReady: (value) => {
      appIssueReportsReady = value;
    },
    setAppIssueReports: (value) => {
      appIssueReports = value;
    },
  ...
  var {
    ...
    canAdministerPublicRequestLinks,
    getActiveCompanyId: () => activeCompanyId,
    setPublicRequestLinksReady: (value) => {
      publicRequestLinksReady = value;
    },
    showNotice,
    render: () => render()
  });
  var {
    ...
    setPartSuppliersReady: (value) => {
      partSuppliersReady = value;
    },
    setPartCostsReady: (value) => {
      partCostsReady = value;
    },
    setPartMachineNotesReady: (value) => {
      partMachineNotesReady = value;
    },
    setActivePartId: setActivePartIdState,
  ...
  var { renderCreateWorkOrder } = createCreateWorkOrderDisplayHelpers({
    STATUS_OPTIONS,
    TYPE_OPTIONS,
    getParts: () => parts,
    renderAssetOptions,
  ...
  var { renderQuickFixForm } = createQuickFixDisplayHelpers({
    TYPE_OPTIONS,
  ...
  var { updateWorkOrderQuickView } = createWorkOrderQuickUpdateWorkflow({
    documentRef: document,
    FormDataCtor: FormData,
    consoleRef: console,
    getWorkOrders: () => workOrders,
  ...
  async function openStorageLinkedRecord(section, id, label = "") {
    ...
    if (section === "work" && id) {
      if (!workOrders.some((workOrder) => workOrder.id === id)) {
        const response = await loadWorkspaceResponse("Storage linked work order", fetchWorkOrderById(supabaseClient, activeCompanyId, id, WORK_ORDER_RELATION_SELECT), 12e3);
  ...
  function bindWorkspaceEvents() {
    ...
    bindWorkspaceIssueAdminUiEvents({
      state: {
        setAdminDeleteSqlConfirmed: (value) => {
          adminDeleteSqlConfirmed = value;
        },
        setReportIssueMode: (value) => {
          reportIssueMode = value;
        }
    ...
    bindWorkspaceSearchEvents({
      state: {
        ...
        setSearchQuery: (value) => {
          workspaceUiState.setSearchQuery(value);
        }
      },
      invalidateExactWorkOrderSearchCache,
      reloadRequestQueue,
    ...
    createWorkspaceWorkOrderDeleteEvents({
      alertRef: alert,
      canDeleteWorkOrders,
      deleteWorkOrderRecord: (id) => supabaseClient.from("work_orders").delete().eq("id", id).eq("company_id", activeCompanyId),
      documentRef: document,
      friendlyWorkOrderSaveError,
      getPhotoPathsByWorkOrder: (id) => (photosByWorkOrder[id] || []).map((photo) => photo.storage_path).filter(Boolean),
      removeWorkOrderPhotoStorage: (photoPaths) => supabaseClient.storage.from("work-order-photos").remove(photoPaths),
      render,
      renderWorkspace,
      setActiveAssetId: setActiveAssetIdState,
      setActiveWorkOrderId: setActiveWorkOrderIdState,
      setPendingDeleteWorkOrderId: (id) => {
  ...
  var {
    ...
    recordWorkOrderEvent,
    getAssetDocumentsReady: () => assetDocumentsReady,
    setAssetDocumentsReady: (value) => {
      assetDocumentsReady = value;
    },
    getPartDocumentsReady: () => partDocumentsReady,
    setPartDocumentsReady: (value) => {
      partDocumentsReady = value;
    },
    setPhotosReady: (value) => {
      photosReady = value;
    },
    setRequestPhotosReady: (value) => {
      requestPhotosReady = value;
    },
    getAppIssueReportsReady: () => appIssueReportsReady,
    createAppIssueReportRecord,
    activeLocationDatabaseId,
  ...
  var { uploadCompanyLogo } = createCompanyLogoWorkflow({
    documentRef: document,
    FormDataCtor: FormData,
    cryptoRef: crypto,
    URLRef: URL,
  ...
  var { createQuickFix } = createQuickFixWorkflow({
    documentRef: document,
    FormDataCtor: FormData,
    withOperationTimeout,
    createQuickFixAsset,
  ...
  var { createWorkOrder } = createWorkOrderCreationWorkflow({
    documentRef: document,
    FormDataCtor: FormData,
    alertRef: alert,
    withOperationTimeout,
  ...
  var { updateWorkOrderDetails } = createWorkOrderDetailEditWorkflow({
    documentRef: document,
    FormDataCtor: FormData,
    consoleRef: console,
    getActiveWorkOrderId: () => activeWorkOrderId,
  ...
  var { saveStepResult } = createProcedureChecklistWorkflow({
    blocksProcedureCompletion,
    getActiveCompanyId: () => activeCompanyId,
    getSession: () => session,
    getWorkOrderActionWarningId: () => workOrderActionWarningId,
  ...
  async function assignWorkOrderToMe(id) {
    try {
      ...
      const { error } = await withOperationTimeout(
        supabaseClient.from("work_orders").update({
  ...
  async function assignWorkOrderFromCard(event) {
    ...
    try {
      const { error } = await withOperationTimeout(
        ...
        }).eq("id", workOrder.id).eq("company_id", activeCompanyId),
        "Assignment save timed out. Check your connection and try again.",
        15e3
      );
      if (error) {
      ...
      const summary = assignmentValue === OUTSIDE_VENDOR_VALUE ? "Assigned to outside vendor." : assignmentValue ? `Assigned to ${teamMemberName(assignmentValue)}.` : "Assignment cleared.";
  ...
  function activeCompanyRole() {
    const activeMembership = companyMembers.find(
      (member) => member.company_id === activeCompanyId && member.user_id === session?.user?.id
    );
  ...
  function applySafetyCheckPayload(payload, checked) {
    ...
    payload.safety_devices_checked_at = checked ? (/* @__PURE__ */ new Date()).toISOString() : null;
    return payload;
  }
