...
`),status:"open"}),"Upload failure report timed out.",8e3)}catch(U){s.warn("Could not report upload failure",U)}}}function E(){return{targetBytes:256*1024,passes:[{maxDimension:768,quality:.78},{maxDimension:768,quality:.74},{maxDimension:768,quality:.7}]}}async function C(w,P={}){if(typeof e.optimizePhotoOverride=="function")return e.optimizePhotoOverride(w,P);let R=["image/jpeg","image/png","image/webp","image/heic","image/heif"],O=T(w);if(!R.includes(O))return{blob:w,fileName:e.safeFileName(w.name||"photo"),contentType:O};try{if(!f)throw new Error("Browser image optimization is unavailable.");let q=await f(w),L=Number(P.targetBytes||0)||1*1024*1024,I=P.passes||[{maxDimension:2e3,quality:.82},{maxDimension:1800,quality:.78},{maxDimension:1600,quality:.74}],z=null;for(let U of I){let V=await W(q,U.maxDimension,U.quality);if(z=V,V.size<=L)break}if(q.close&&q.close(),!z)throw new Error("Browser could not optimize this image.");return{blob:z,fileName:`${e.fileBaseName(w.name||"photo")}.jpg`,contentType:"image/jpeg"}}catch(q){return s.warn("Photo optimization failed; uploading original.",q),{blob:w,fileName:e.safeFileName(w.name||"photo"),contentType:O}}}function v(w){return["image/jpeg","image/png","image/webp"].includes(T(w))}function S(w){return!v(w)&&Number(w.size||0)>a}function _(){return"This non-image file is over 25 MB. Compress it or split it before uploading."}function T(w){let P=String(w?.type||"").trim().toLowerCase();if(P)return P;let R=String(w?.name||"").toLowerCase();return/\.(jpe?g)$/.test(R)?"image/jpeg":/\.png$/.test(R)?"image/png":/\.webp$/.test(R)?"image/webp":/\.gif$/.test(R)?"image/gif":/\.heic$/.test(R)?"image/heic":/\.heif$/.test(R)?"image/heif":/\.pdf$/.test(R)?"application/pdf":/\.txt$/.test(R)?"text/plain":/\.csv$/.test(R)?"text/csv":/\.doc$/.test(R)?"application/msword":/\.docx$/.test(R)?"application/vnd.openxmlformats-officedocument.wordprocessingml.document":/\.xls$/.test(R)?"application/vnd.ms-excel":/\.xlsx$/.test(R)?"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet":"application/octet-stream"}function N(w){let P=T(w);return u.has(P)?"":"This upload box accepts photos only. PDF quotes and documents need to be attached in an equipment or part file area."}function D(w){return u.has(String(w?.contentType||"").toLowerCase())?Number(w?.blob?.size||0)>i?"This photo is still over 5 MB after optimization. Try a smaller photo or screenshot it first.":"":"This upload box accepts photos only. PDF quotes and documents need to be attached in an equipment or part file area."}async function W(w,P,R){let O=Math.min(1,P/Math.max(w.width,w.height)),q=Math.max(1,Math.round(w.width*O)),L=Math.max(1,Math.round(w.height*O)),I=n.createElement("canvas");I.width=q,I.height=L,I.getContext("2d",{alpha:!1}).drawImage(w,0,0,q,L);let U=await new Promise(V=>I.toBlob(V,"image/jpeg",R));if(!U)throw new Error("Browser could not optimize this image.");return U}return{addPhotoToMaintenanceRequest:A,addPhotoToWorkOrder:k,optimizePhoto:C,removeUploadedObject:b,reportUploadFailure:$,deleteAssetDocument:p,deleteWorkOrderPhoto:g,uploadAssetDocument:o,uploadPartDocument:c,uploadPhoto:y}}typeof Ae<"u"&&Ae.exports&&(Ae.exports={createMediaStorageWorkflow:l}),window.MaintainOpsMediaStorageWorkflow={createMediaStorageWorkflow:l}})()});var tn=Q((gr,Re)=>{(function(){function l(e={}){let n=e.documentRef||document,t=e.FormDataCtor||FormData,r=e.cryptoRef||crypto,s=e.URLRef||URL,f=e.consoleRef||console,a=e.createImageBitmapRef||(typeof createImageBitmap<"u"?createImageBitmap:null),i=25*1024*1024,u=new Set(["image/jpeg","image/png","image/webp","image/gif","image/heic","image/heif"]);async function m(d){d.preventDefault();let h=d.currentTarget,y=n.querySelector("#company-logo-error"),b=h.querySelector("button[type='submit']"),k=new t(h).get("logo");if(y&&(y.textContent=""),!k||!k.name){y&&(y.textContent="Choose a logo image first.");return}b&&(b.disabled=!0,b.textContent="Uploading...");try{let A=p(k);if(A)throw new Error(A);let $=await c(k),E=g($);if(E)throw new Error(E);let C=`${e.getActiveCompanyId()}/logo-${r.randomUUID()}-${$.fileName}`,v=await e.withOperationTimeout(e.supabaseClient().storage.from("company-logos").upload(C,$.blob,{contentType:$.contentType,upsert:!1}),"Company logo upload timed out. Check your connection and try again.",25e3);if(v.error)throw new Error(v.error.message.includes("Bucket not found")?"Run supabase/step-next-company-logo.sql before uploading a logo.":v.error.message);let{error:S}=await e.withOperationTimeout(e.supabaseClient().rpc("set_company_logo",{target_company_id:e.getActiveCompanyId(),new_logo_path:C}),"Company logo record save timed out. Check your connection and try again.",15e3);if(S)throw await e.removeUploadedObject("company-logos",C),new Error(e.isColumnSchemaError(S,["logo_path"])?"Run supabase/step-next-company-logo.sql before saving a company logo.":S.message.includes("set_company_logo")?"Run supabase/step-next-company-logo.sql, then try uploading the logo again.":S.message);let _=e.getCompanies().find(T=>T.id===e.getActiveCompanyId());_&&(_.logo_path=C,_.logoUrl=s.createObjectURL($.blob)),e.showNotice("Company logo uploaded."),await e.render()}catch(A){y&&(y.textContent=A.message||"Could not upload logo.")}finally{b&&(b.disabled=!1,b.textContent="Upload Logo")}}async function c(d){if(typeof e.optimizeLogoOverride=="function")return e.optimizeLogoOverride(d);let h=o(d);try{if(!a)throw new Error("Browser logo optimization is unavailable.");let y=await a(d),k=Math.min(1,1200/Math.max(y.width,y.height)),A=Math.max(1,Math.round(y.width*k)),$=Math.max(1,Math.round(y.height*k)),E=n.createElement("canvas");E.width=A,E.height=$;let C=E.getContext("2d",{alpha:!0});C.clearRect(0,0,A,$),C.drawImage(y,0,0,A,$),y.close&&y.close();let v=await new Promise(S=>E.toBlob(S,"image/png"));if(!v)throw new Error("Browser could not optimize this logo.");return{blob:v,fileName:`${e.fileBaseName(d.name||"logo")}.png`,contentType:"image/png"}}catch(y){return f.warn("Logo optimization failed; uploading original.",y),{blob:d,fileName:e.safeFileName(d.name||"logo"),contentType:h}}}function o(d){let h=String(d?.type||"").trim().toLowerCase();if(h)return h;let y=String(d?.name||"").toLowerCase();return/\.(jpe?g)$/.test(y)?"image/jpeg":/\.png$/.test(y)?"image/png":/\.webp$/.test(y)?"image/webp":/\.gif$/.test(y)?"image/gif":/\.heic$/.test(y)?"image/heic":/\.heif$/.test(y)?"image/heif":/\.avif$/.test(y)?"image/avif":/\.bmp$/.test(y)?"image/bmp":/\.tiff?$/.test(y)?"image/tiff":"application/octet-stream"}function p(d){let h=o(d);return u.has(h)?"":"Company logos must be JPG, PNG, WebP, GIF, HEIC, or HEIF images."}function g(d){return u.has(String(d?.contentType||"").toLowerCase())?Number(d?.blob?.size||0)>i?"This logo is still over 25 MB after processing. Try a smaller logo image.":"":"Company logos must be JPG, PNG, WebP, GIF, HEIC, or HEIF images."}return{optimizeLogo:c,uploadCompanyLogo:m}}typeof Re<"u"&&Re.exports&&(Re.exports={createCompanyLogoWorkflow:l}),window.MaintainOpsCompanyLogoWorkflow={createCompanyLogoWorkflow:l}})()});var nn=Q((hr,Xe)=>{(function(){function l(e={}){let n=e.documentRef||document,t=e.CSSRef||CSS,r=e.alertUser||alert;function s(i){return e.partUsageRows(i).length?"This part has work order usage history and is kept for traceability.":e.assetPartRows(i).length?"This part is linked to equipment and is kept for traceability.":""}function f(i){if(!e.canDeleteParts()){r("Only company admins and managers can delete parts.");return}if(!e.getParts().find(o=>o.id===i))return;let m=s(i);if(m){r(m);return}let c=!!n.querySelector(`[data-delete-part="${t.escape(i)}"].permanent-delete-button`);if(e.getPendingDeletePartId()===i||c){a(i);return}e.setPendingDeletePartId(i),e.renderWorkspace()}async function a(i){if(!e.canDeleteParts()){r("Only company admins and managers can delete parts.");return}let u=e.getParts().find(p=>p.id===i),m=n.querySelector("#part-delete-error");if(m&&(m.textContent=""),!u)return;let c=s(i);if(c){m&&(m.textContent=c);return}let o=n.querySelector(`[data-delete-part="${t.escape(i)}"].permanent-delete-button`);o&&(o.disabled=!0,o.textContent="Deleting...");try{let p=(e.getPartDocumentsByPartId()[i]||[]).map(y=>y.storage_path).filter(Boolean);if(p.length){let y=await e.withOperationTimeout(e.removePartDocumentStorage(p),"Part document cleanup timed out. Try deleting again.",15e3);if(y.error)throw new Error(`Could not remove filed receipts/invoices: ${y.error.message}`)}let{data:g,error:d}=await e.withOperationTimeout(e.deletePartRecord(i),"Part delete timed out. Check your connection and try again.",15e3);if(d)throw new Error(d.message.includes("violates foreign key constraint")?"This part is linked to work or equipment and cannot be deleted.":d.message);if(!g?.length)throw new Error("Part was not deleted. Check that your company role is admin or manager and that supabase/step-next-part-delete.sql has been run.");let h=await e.withOperationTimeout(e.verifyPartDeleted(i),"Part delete verification timed out. Refresh and check the part list.",15e3);if(h.error)throw new Error(`Part delete verification failed: ${h.error.message}`);if(h.data)throw new Error("Part delete did not persist in Supabase. Run supabase/step-next-part-delete.sql, then try again.");e.setActivePartId(null),e.setPendingDeletePartId(null),e.showNotice("Part deleted."),await e.render()}catch(p){e.showNotice(p.message||"Could not delete part.","warning"),m&&(m.textContent=p.message||"Could not delete part."),o&&(o.disabled=!1,o.textContent="Permanently Delete")}}return{deletePart:a,requestDeletePart:f}}window.MaintainOpsPartDeleteWorkflow={createPartDeleteWorkflow:l},typeof Xe<"u"&&(Xe.exports={createPartDeleteWorkflow:l})})()});var rn=Q((yr,et)=>{(function(){function l(e={}){async function n(t){let r=t.target,s=r.type==="checkbox"?r.checked?"checked":"":r.value;r.disabled=!0;try{let{error:f}=await e.withOperationTimeout(e.upsertStepResult({company_id:e.getActiveCompanyId(),work_order_id:r.dataset.workOrderId,procedure_step_id:r.dataset.stepResult,completed_by:s?e.getSession().user.id:null,value:s,completed_at:s?new Date().toISOString():null}),"Checklist save timed out. Check your connection and try again.",15e3);if(f)throw f;await e.withOperationTimeout(e.recordWorkOrderEvent(r.dataset.workOrderId,"checklist_updated","Procedure checklist updated."),"Activity log timed out.",8e3).catch(()=>null);let a=await e.withOperationTimeout(e.loadStepResults(),"Checklist refresh timed out. Refresh the workspace to confirm the latest checklist state.",1e4).catch(i=>i);if(a){e.showNotice(`Checklist saved, but refresh did not finish: ${a.message||a}`,"warning"),r.disabled=!1;return}if(e.getWorkOrderActionWarningId()===r.dataset.workOrderId){let i=e.getWorkOrders().find(u=>u.id===r.dataset.workOrderId);e.blocksProcedureCompletion(i)||e.setWorkOrderActionWarning("","")}e.renderWorkspace()}catch(f){e.showNotice(`Could not save checklist step: ${f.message||f}`,"warning"),r.disabled=!1}}return{saveStepResult:n}}window.MaintainOpsProcedureChecklistWorkflow={createProcedureChecklistWorkflow:l},typeof et<"u"&&(et.exports={createProcedureChecklistWorkflow:l})})()});var an=Q((br,tt)=>{(function(){function l(e={}){let n=e.documentRef||document,t=e.bodyRef||document.body,r=e.FormDataCtor||FormData;async function s(m,c){let{data:o,error:p}=await e.withOperationTimeout(e.getPublicRequestIntake(m),c);return{data:Array.isArray(o)?o[0]:o,error:p}}async function f(m){t.classList.add("public-qr-mode"),e.setAppHtml(e.loadingQrPage());let c=null;try{let{data:p,error:g}=await s(m,"Request QR lookup timed out.");if(c=p,g||!c){i("This QR code link is inactive or invalid.");return}}catch{i("This QR code link is inactive or invalid.");return}let o=e.publicRequestUrl(m);e.setAppHtml(e.publicRequestQrPage(c,o)),e.bindPublicQrPrintEvents(),typeof e.ensureQrLibrary=="function"&&e.ensureQrLibrary().then(()=>{e.setAppHtml(e.publicRequestQrPage(c,o)),e.bindPublicQrPrintEvents()}).catch(()=>{})}async function a(m){t.classList.remove("public-qr-mode"),e.setAppHtml(e.loadingRequestForm());let c=null;try{let{data:o,error:p}=await s(m,"Request form lookup timed out.");if(p){i("This request link is not ready yet. The company needs to run the public request link setup in Supabase.");return}c=o}catch(o){i(o.message||"This request link could not be loaded.");return}if(!c){i("This request link is inactive or invalid.");return}e.setAppHtml(e.publicRequestForm(c)),n.querySelector("#public-request-form").addEventListener("submit",o=>u(o,m,c))}function i(m){e.setAppHtml(e.publicRequestError(m))}async function u(m,c,o){m.preventDefault();let p=m.currentTarget,g=new r(p),d=n.querySelector("#public-request-error"),h=p.querySelector("button[type='submit']");d&&(d.textContent=""),h&&(h.disabled=!0,h.textContent="Sending...");try{let{data:y,error:b}=await e.withOperationTimeout(e.submitPublicLocationRequest({request_token:c,request_title:e.requiredText(g.get("title"),"Request title"),equipment_note:e.requiredText(g.get("equipment_note"),"Machine / area"),request_description:e.requiredText(g.get("description"),"Request details"),requester_name:e.requiredText(g.get("requester_name"),"Your name"),requester_contact:String(g.get("requester_contact")||"").trim()||null,request_priority:g.get("priority")||"medium"}),"Request send timed out.");if(b)throw b;let k=g.get("photo"),A="";if(k&&k.name){let E=await e.addPhotoToMaintenanceRequest(y,k);E&&(A=`Request sent, but the photo did not upload: ${E.message||E}`)}let $=await e.notifyRequestEmailer(y);$.error&&e.warn("Request email notification did not send",$.error),e.setAppHtml(e.publicRequestSuccess(o,A)),n.querySelector("#public-request-another").addEventListener("click",()=>a(c))}catch(y){d&&(d.textContent=y.message||"Could not send the request.")}finally{h?.isConnected&&(h.disabled=!1,h.textContent="Send Request")}}return{renderPublicRequestError:i,renderPublicRequestIntake:a,renderPublicRequestQrPage:f,submitPublicRequest:u}}window.MaintainOpsPublicRequestIntakeWorkflow={createPublicRequestIntakeWorkflow:l},typeof tt<"u"&&(tt.exports={createPublicRequestIntakeWorkflow:l})})()});var on=Q((wr,nt)=>{(function(){function l(e={}){let n=e.documentRef||document,t=e.FormDataCtor||FormData;function r(){e.setAppHtml(e.companyCreateForm(e.getAppError())),n.querySelector("#company-form").addEventListener("submit",s),n.querySelector("#sign-out").addEventListener("click",()=>e.signOut())}async function s(f){f.preventDefault();let a=f.target,i=a.querySelector("button[type='submit']"),u=n.querySelector("#company-error"),m=String(new t(a).get("name")||"").trim();i.disabled=!0,i.textContent="Creating...",u.textContent="";try{if(!m)throw new Error("Company name is required.");let c=e.getCompanies().find(d=>d.name.trim().toLowerCase()===m.trim().toLowerCase());if(c){e.setActiveCompanyId(c.id),e.persistActiveCompanyId(c.id),await e.render();return}let{data:o,error:p}=await e.withOperationTimeout(e.createCompanyRecord(m),"Company creation timed out.");if(p){u.textContent=p.message.includes("create_company")?"Database setup is not finished. Run supabase/schema.sql in the Supabase SQL editor, then wait a few seconds and try again.":p.message;return}if(e.setActiveCompanyId(o),e.persistActiveCompanyId(o),!await e.ensureProfileForActiveCompany(m))throw new Error(e.getAppError()||"Could not create your company profile.");await e.seedStarterAssets(),await e.render()}catch(c){u.textContent=c.message||"Could not create company."}finally{i?.isConnected&&(i.disabled=!1,i.textContent="Create Company")}}return{createCompany:s,renderCompanyCreate:r}}window.MaintainOpsCompanySetupWorkflow={createCompanySetupWorkflow:l},typeof nt<"u"&&(nt.exports={createCompanySetupWorkflow:l})})()});var sn=Q((vr,rt)=>{(function(){function l(e={}){async function n(r){let s=e.getWorkOrders().find(f=>f.id===e.getActiveWorkOrderId());r.target.disabled=!0;try{await t(e.getActiveWorkOrderId(),r.target.value)||(r.target.value=s?.status||"open")}catch(f){r.target.value=s?.status||"open",e.showNotice(`Could not update status: ${f.message||f}`,"warning")}finally{r.target.disabled=!1}}async function t(r,s){let f=e.getWorkOrders().find(c=>c.id===r);if(s==="completed"){let c=e.productionActionCompletionMessage?.(f)||"";if(c)return e.setActiveWorkOrderId(r),e.setWorkOrderActionWarning(r,c),e.showNotice(c,"warning"),await e.render(),!1;let o=e.blocksProcedureCompletion(f);if(o)return e.setActiveWorkOrderId(r),e.setWorkOrderActionWarning(r,o),e.showNotice(o,"warning"),await e.render(),!1}let a=e.currentSafetyCheckboxCheckedForWorkOrder(r),i=e.hasCompletedSafetyDeviceCheck(f)||a;if(s==="completed"&&e.requiresSafetyDeviceCheck(f)&&!i){e.setActiveWorkOrderId(r);let c="Safety devices must be checked before completing equipment work. Open the work order and use Complete Work.";return e.setWorkOrderActionWarning(r,c),e.showNotice(c,"warning"),await e.render(),!1}let u={status:s,asset_id:f?.asset_id||null,completed_at:s==="completed"?new Date().toISOString():null};e.applySafetyRequirementPayload(u),s==="completed"?e.applySafetyCheckPayload(u,u.safety_check_required&&i):s!=="completed"&&e.applySafetyCheckPayload(u,!1),delete u.asset_id;let{error:m}=await e.withOperationTimeout(e.updateWorkOrderSafely(u,r),"Status save timed out. Check your connection and try again.",15e3);return m?(e.showNotice(`Could not update status: ${e.friendlyWorkOrderSaveError(m)}`,"warning"),!1):(e.setActiveWorkOrderId(r),e.setWorkOrderActionWarning("",""),await e.recordWorkOrderEvent(r,"status_changed",`Status changed to ${e.statusLabel(s)}.`),e.showNotice(`Status changed to ${e.statusLabel(s)}.`),await e.render(),!0)}return{setWorkOrderStatus:t,updateWorkOrderStatus:n}}window.MaintainOpsWorkOrderStatusWorkflow={createWorkOrderStatusWorkflow:l},typeof rt<"u"&&(rt.exports={createWorkOrderStatusWorkflow:l})})()});var cn=Q((kr,Oe)=>{(function(){function l(e={}){let n=e.FormDataCtor||FormData,t=e.confirmRef||confirm;function r(u,m){return u?.closest?.("[data-production-action-control]")?.querySelector?.(`[data-production-action-error="${m}"]`)||e.documentRef?.querySelector?.(`[data-production-action-error="${m}"]`)||null}async function s({workOrderId:u,payload:m,source:c,busyText:o,successMessage:p}){let g=c?.querySelector?.("button[type='submit']")||c,d=g?.textContent||"",h=r(c,u);g&&(g.disabled=!0,g.textContent=o),h&&(h.textContent="");try{let y=await e.withOperationTimeout(e.updateProductionActionRecord(u,m),"Production Action save timed out. Check your connection and try again.",15e3);if(y.error){let b=e.friendlyWorkOrderSaveError(y.error);return h?h.textContent=`Could not save Production Action: ${b}`:e.showNotice(`Could not save Production Action: ${b}`,"warning"),!1}return e.showNotice(p,"success"),await e.afterProductionActionMutation(y.data,u),!0}catch(y){let b=y.message||String(y);return h?h.textContent=`Could not save Production Action: ${b}`:e.showNotice(`Could not save Production Action: ${b}`,"warning"),!1}finally{g?.isConnected&&(g.disabled=!1,g.textContent=d)}}async function f(u){u.preventDefault(),u.stopPropagation();let m=u.currentTarget,c=m.dataset.productionActionForm,o=new n(m),p=String(o.get("production_action")||"").trim(),g=String(o.get("production_action_assigned_to")||"").trim(),d=r(m,c);if(!p||!g){d&&(d.textContent="Enter an action and choose a Production owner.");return}let h=e.getWorkOrderById(c);await s({workOrderId:c,payload:{production_action:p,production_action_assigned_to:g},source:m,busyText:"Saving...",successMessage:h?.production_action?"Production Action updated.":"Production Action assigned."})}async function a(u){u.preventDefault(),u.stopPropagation();let m=u.currentTarget,c=m.dataset.workOrderId,o=m.dataset.productionActionStatus;await s({workOrderId:c,payload:{production_action_status:o},source:m,busyText:o==="completed"?"Completing...":"Reopening...",successMessage:o==="completed"?"Production Action completed.":"Production Action reopened."})}async function i(u){u.preventDefault(),u.stopPropagation();let m=u.currentTarget,c=m.dataset.productionActionRemove;t("Remove this Production Action? Work Order History will keep a record of the removal.")&&await s({workOrderId:c,payload:{production_action:null},source:m,busyText:"Removing...",successMessage:"Production Action removed."})}return{saveProductionAction:f,setProductionActionStatus:a,removeProductionAction:i}}window.MaintainOpsProductionActionWorkflow={createProductionActionWorkflow:l},typeof Oe<"u"&&Oe.exports&&(Oe.exports={createProductionActionWorkflow:l})})()});var ln=Q((_r,at)=>{(function(){function l(e={}){async function n(s,f={}){let a=s.filter(c=>c.id&&!c.read_at);if(!a.length)return!0;let i=new Map(a.map(c=>[c.id,c])),u=new Date().toISOString(),m=a.map(c=>c.id);e.setNotifications(e.getNotifications().map(c=>i.has(c.id)?{...c,read_at:u}:c)),f.render!==!1&&e.renderWorkspace();try{let c=await e.withOperationTimeout(e.markWorkOrderNotificationsRead(e.getSupabaseClient(),e.getSession().user.id,m,u),"Work notification update timed out.",1e4);if(c.error)throw c.error;return!0}catch(c){return e.setNotifications(e.getNotifications().map(o=>i.get(o.id)||o)),e.showNotice(`Could not mark the work notification read: ${c.message||c}`,"warning"),f.render!==!1&&e.renderWorkspace(),!1}}function t(s,f={}){let a=e.getNotifications().find(i=>i.id===s);return a?.read_at?Promise.resolve(!0):n([a||{id:s,read_at:null}],f)}function r(s,f={}){return n(e.getNotifications().filter(a=>a.work_order_id===s),f)}return{markWorkOrderNotificationRead:t,markWorkOrderNotificationsReadForOrder:r}}window.MaintainOpsWorkOrderNotificationWorkflow={createWorkOrderNotificationWorkflow:l},typeof at<"u"&&(at.exports={createWorkOrderNotificationWorkflow:l})})()});var un=Q((Sr,ot)=>{(function(){function l(e){async function n(t,r){if(!e.canEditOperationalRecords())return e.showNotice("This account can view Planning but cannot change work orders.","warning"),{saved:!1,reason:"read_only"};let s=e.getPlanningWorkOrders().find(f=>f.id===t);if(!s||s.status==="completed")return e.showNotice("That work order is no longer available in the no-due-date queue.","warning"),{saved:!1,reason:"not_available"};try{let f=e.workOrderDateValue(r);if(!f)throw new Error("Choose a due date.");let a=await e.withOperationTimeout(e.updateWorkOrderSafely({due_at:f},t),"Due date save timed out. Check your connection and try again.");if(a.error)throw a.error;return e.setPlanningWorkOrders(e.getPlanningWorkOrders().map(i=>i.id===t?{...i,due_at:f}:i)),e.setWorkOrders(e.getWorkOrders().map(i=>i.id===t?{...i,due_at:f}:i)),e.resetNoDuePage(),await e.recordWorkOrderEvent(t,"updated",`Due date set to ${f} from Planning.`),e.showNotice("Due date set. The order moved out of No Due Date."),e.renderWorkspace(),{saved:!0,dueAt:f}}catch(f){return e.showNotice(`Could not set due date: ${f.message||f}`,"warning"),{saved:!1,reason:"save_failed",error:f}}}return{savePlanningDueDate:n}}window.MaintainOpsPlanningDueDateWorkflow={createPlanningDueDateWorkflow:l},typeof ot<"u"&&(ot.exports={createPlanningDueDateWorkflow:l})})()});var dn=Q((qr,it)=>{(function(){function l(n,t,r,s=50){let f=Math.min(Math.max(Number(s)||50,1),100);return n.from("work_order_notifications").select("id, company_id, work_order_id, recipient_id, actor_id, source_event_id, kind, title, body, read_at, created_at").eq("company_id",t).eq("recipient_id",r).order("created_at",{ascending:!1}).limit(f)}function e(n,t,r,s){let f=[...new Set((r||[]).filter(Boolean))];return f.length?n.from("work_order_notifications").update({read_at:s}).eq("recipient_id",t).in("id",f).select("id, read_at"):Promise.resolve({data:[],error:null})}window.MaintainOpsWorkOrderNotificationsService={listWorkOrderNotifications:l,markWorkOrderNotificationsRead:e},typeof it<"u"&&(it.exports={listWorkOrderNotifications:l,markWorkOrderNotificationsRead:e})})()});var pn=Q((Cr,st)=>{(function(){async function l(e,n){if(!e?.functions?.invoke||!n)return{data:null,error:null,skipped:!0};try{let{data:t,error:r}=await e.functions.invoke("request-emailer",{body:{request_id:n}});return{data:t,error:r||null,skipped:!1}}catch(t){return{data:null,error:t,skipped:!1}}}window.MaintainOpsRequestEmailNotificationService={notifyRequestEmailer:l},typeof st<"u"&&(st.exports={notifyRequestEmailer:l})})()});var mn=Q(($r,ct)=>{(function(){async function l(n,t,r=[],s={}){let f=s.pathKey||"storage_path",a=s.urlKey||"signedUrl",i=s.expiresIn||600,u=s.onError;await Promise.all(r.map(async m=>{let c=m?.[f];if(!c)return;let{data:o,error:p}=await n.storage.from(t).createSignedUrl(c,i);if(p){m[a]="",typeof u=="function"&&u(m,p);return}m[a]=o?.signedUrl||""}))}function e(n={}){function t(r){if(!r||!n.getReady())return;let f=(n.getRows(r)||[]).filter(i=>i.storage_path&&!i.signedUrl),a=n.getSigningMap();!f.length||a[r]||(a[r]=!0,n.withOperationTimeout(l(n.supabaseClient(),n.bucketName,f),n.timeoutMessage||"Signed file link load timed out.",n.timeoutMs||1e4).catch(i=>{n.warn("Could not load signed file links",i)}).finally(()=>{delete a[r],n.getActiveGroupId()===r&&n.renderWorkspace()}))}return{ensureGroupSignedUrls:t}}window.MaintainOpsSignedUrlService={addSignedUrlsToRows:l,createDeferredSignedUrlLoader:e},typeof ct<"u"&&(ct.exports={addSignedUrlsToRows:l,createDeferredSignedUrlLoader:e})})()});var fn=Q((Pr,lt)=>{(function(){function l(t,r){if(t[r]===void 0)throw new Error(`workspaceQueueLoadersService missing dependency: ${r}`);return t[r]}function e(t={}){let r=l(t,"supabaseClient"),s=l(t,"workspaceUiState"),f=l(t,"applyRequestQueryFilters"),a=l(t,"applyWorkOrderListFilters"),i=l(t,"applyWorkOrderFilters"),u=l(t,"selectWorkOrders"),m=l(t,"countWorkOrdersQuery"),c=l(t,"fetchExactSearchedWorkOrderPage"),o=l(t,"isColumnSchemaError"),p=t.warn||(()=>{}),g=l(t,"LIST_ITEMS_PER_PAGE"),d=l(t,"WORK_ORDERS_PER_PAGE"),h=l(t,"REQUEST_RELATION_SELECT"),y=l(t,"REQUEST_ASSET_FALLBACK_SELECT"),b=l(t,"REQUEST_FALLBACK_SELECT"),k=l(t,"WORK_ORDER_RELATION_SELECT"),A=l(t,"WORK_ORDER_FALLBACK_SELECT");function $(){return typeof r=="function"?r():r}async function E(D=s.getRequestViewFilter(),W={}){let w=Math.max(1,s.getRequestsPage()),P=(w-1)*g,R=P+g-1,O=W.includeRelations===!1?b:W.includeLocationRelation===!1?y:h,q=await f($().from("maintenance_requests").select(O,{count:"exact"}),D).order("created_at",{ascending:!1}).range(P,R);return q.error&&W.includeLocationRelation!==!1&&o(q.error,["location_id","locations"])?E(D,{includeLocationRelation:!1}):q.error&&W.includeRelations!==!1?E(D,{includeRelations:!1}):!q.error&&q.count&&w>1&&P>=q.count?(s.setRequestsPage(Math.max(1,Math.ceil(q.count/g))),E(D,W)):q}async function C(D){let W=await f($().from("maintenance_requests").select("id",{count:"exact",head:!0}),D);return W.error?(p("Request count failed",W.error),0):W.count||0}async function v(){let[D,W,w]=await Promise.all([C("active"),C("converted"),C("all")]);return{active:D,converted:W,all:w}}async function S(D={}){if(s.getWorkOrderSearchMode()&&s.getSearchQuery().trim())return c(D);let W=Math.max(1,s.getWorkOrderPage()),w=(W-1)*d,P=w+d-1,R=D.includeLocationRelation===!1?A:k,O=await a(u($(),R,{count:"exact"})).range(w,P);return!O.error&&O.count&&W>1&&w>=O.count?(s.setWorkOrderPage(Math.max(1,Math.ceil(O.count/d))),S(D)):O}async function _(D={}){let W=await i(m($()),D);return W.error?(p("Work order count failed",W.error),0):W.count||0}async function T(){let[D,W,w,P,R,O,q,L]=await Promise.all([_({statusFilter:"active",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1}),_({statusFilter:"open",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1}),_({statusFilter:"in_progress",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1}),_({statusFilter:"blocked",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1}),_({statusFilter:"overdue",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1}),_({statusFilter:"completed",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1}),_({statusFilter:"completed_month",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1}),_({statusFilter:"completed_week",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1})]);return{activeWork:D,newWork:W,inProgress:w,blocked:P,overdue:R,completedAll:O,completedMonth:q,completedWeek:L}}async function N(){let[D,W,w,P,R,O,q,L]=await Promise.all([_({statusFilter:"active",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1}),_({statusFilter:"open",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1}),_({statusFilter:"in_progress",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1}),_({statusFilter:"blocked",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1}),_({statusFilter:"overdue",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1}),_({statusFilter:"completed",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1}),_({statusFilter:"completed_month",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1}),_({statusFilter:"completed_week",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1})]);return{activeWork:D,newWork:W,inProgress:w,blocked:P,overdue:R,completedAll:O,completedMonth:q,completedWeek:L}}return{fetchRequestPage:E,countRequests:C,loadRequestDashboardCounts:v,fetchWorkOrderPage:S,countWorkOrders:_,loadWorkOrderDashboardCounts:T,loadMyWorkDashboardCounts:N}}let n={createWorkspaceQueueLoaders:e};typeof window<"u"&&(window.MaintainOpsWorkspaceQueueLoadersService=n),typeof lt<"u"&&(lt.exports=n)})()});var gn=Q((Ar,ut)=>{(function(){function l(e={}){let n=e.windowRef||window,t=e.documentRef||document,r=e.app;function s(){return n.MaintainOpsAuthRedirects.authCallbackUrl(n.location,n.PUBLIC_APP_URL)}function f(){return n.MaintainOpsAuthRedirects.cleanAuthUrl(n.location)}function a(){n.history.replaceState({},t.title,n.MaintainOpsAuthRedirects.cleanAuthUrl(n.location))}async function i(g){u("Verifying your account...");try{if(g.error||g.errorDescription)throw new Error(g.errorDescription||g.error||"This verification link is invalid or expired.");let d=null;if(g.code){let{data:h,error:y}=await e.supabaseClient.auth.exchangeCodeForSession(g.code);if(y)throw y;d=h?.session||null}else if(g.accessToken&&g.refreshToken){let{data:h,error:y}=await e.supabaseClient.auth.setSession({access_token:g.accessToken,refresh_token:g.refreshToken});if(y)throw y;d=h?.session||null}if(!d){let{data:h,error:y}=await e.supabaseClient.auth.getSession();if(y)throw y;d=h?.session||null}if(!d)throw new Error("The verification link did not create a session. Request a new verification email and try again.");e.setSession(d),a(),u("Verification complete. Loading workspace..."),await e.render()}catch(d){a(),m(d.message||"This verification link is invalid or expired.")}}function u(g){t.body.classList.remove("public-qr-mode"),r.innerHTML=e.authCallback(g)}function m(g){t.body.classList.remove("public-qr-mode"),r.innerHTML=e.authCallbackError(g),t.querySelector("#auth-back-to-login").addEventListener("click",()=>e.renderAuth("login"))}async function c(g=e.passwordRecoveryParamsFromUrl()){let d=!1,h="";if(g.accessToken&&g.refreshToken){let{data:y,error:b}=await e.supabaseClient.auth.setSession({access_token:g.accessToken,refresh_token:g.refreshToken});d=!!(y?.session&&!b),b&&(h="This reset link is expired or invalid. Send a new password reset email and use the newest link.")}else h="This reset link is missing the secure session. Send a new password reset email and use the newest link.";p({ready:d,initialError:h})}function o(g="",d=""){t.body.classList.remove("public-qr-mode"),r.innerHTML=e.passwordResetRequest(g,d),t.querySelector("#auth-back-to-login").addEventListener("click",()=>e.renderAuth("login")),t.querySelector("#auth-reset").addEventListener("click",e.resetLoginState),t.querySelector("#password-reset-request-form").addEventListener("submit",async h=>{h.preventDefault();let y=h.target,b=y.querySelector("button[type='submit']"),k=t.querySelector("#auth-error"),A=t.querySelector("#auth-status"),$=String(new FormData(y).get("email")||"").trim();k.textContent="",A.textContent="Sending reset link...",b.disabled=!0,b.textContent="Sending...";try{let{error:E}=await e.withOperationTimeout(e.supabaseClient.auth.resetPasswordForEmail($,{redirectTo:f()}),"Password reset email timed out. Check your connection and try again.",2e4);if(E){A.textContent="",k.textContent=E.message;return}A.textContent="If that email exists in Supabase, a reset link has been sent."}catch(E){A.textContent="",k.textContent=E.message||"Could not send reset link."}finally{t.body.contains(b)&&(b.disabled=!1,b.textContent="Send Reset Link")}})}function p({ready:g=!1,initialError:d=""}={}){t.body.classList.remove("public-qr-mode"),r.innerHTML=e.passwordRecovery({ready:g,initialError:d}),t.querySelector("#auth-back-to-login").addEventListener("click",()=>{a(),e.renderAuth("login")}),t.querySelector("#auth-send-new-reset").addEventListener("click",()=>{a(),o()}),t.querySelector("#password-recovery-form").addEventListener("submit",async h=>{if(h.preventDefault(),!g)return;let y=h.target,b=y.querySelector("button[type='submit']"),k=new FormData(y),A=String(k.get("password")||""),$=String(k.get("confirmPassword")||""),E=t.querySelector("#auth-error"),C=t.querySelector("#auth-status");if(E.textContent="",A.length<8){E.textContent="Password must be at least 8 characters.";return}if(A!==$){E.textContent="Passwords do not match.";return}C.textContent="Updating password...",b.disabled=!0,b.textContent="Updating...";try{let{error:v}=await e.withOperationTimeout(e.supabaseClient.auth.updateUser({password:A}),"Password update timed out. Try the newest reset link again.",2e4);if(v){C.textContent="",E.textContent=v.message;return}a();let{data:S}=await e.supabaseClient.auth.getSession();if(e.setSession(S.session),C.textContent=S.session?"Password updated. Loading workspace...":"Password updated. Sign in with your new password.",S.session){await e.render();return}e.renderAuth("login","Password updated. Sign in with your new password.")}catch(v){C.textContent="",E.textContent=v.message||"Could not update password."}finally{t.body.contains(b)&&(b.disabled=!1,b.textContent="Update Password")}})}return{authCallbackRedirectUrl:s,passwordResetRedirectUrl:f,clearPasswordRecoveryUrl:a,startAuthCallback:i,renderAuthCallback:u,renderAuthCallbackError:m,startPasswordRecovery:c,renderPasswordResetRequest:o,renderPasswordRecovery:p}}window.MaintainOpsAuthSessionFlow={createAuthSessionFlow:l},typeof ut<"u"&&(ut.exports={createAuthSessionFlow:l})})()});var hn=Q((Rr,Ee)=>{(function(){function l(f,a){let i=a.getProfilesByUserId();if(f.type==="comment")return`
  ...
  `}function r(f){return{asset:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7l8-4 8 4-8 4-8-4z"></path><path d="M4 7v10l8 4 8-4V7"></path><path d="M12 11v10"></path></svg>',procedure:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 6h11"></path><path d="M9 12h11"></path><path d="M9 18h11"></path><path d="M4 6l1 1 2-2"></path><path d="M4 12l1 1 2-2"></path><path d="M4 18l1 1 2-2"></path></svg>',parts:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 7l3 3"></path><path d="M5 19l8-8"></path><path d="M15 5l4 4-4 4-4-4 4-4z"></path></svg>',comment:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 5h14v10H8l-3 3V5z"></path></svg>',message:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5h16v11H7l-3 3V5z"></path><path d="M8 9h8"></path><path d="M8 13h5"></path></svg>',photo:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 6h16v12H4V6z"></path><path d="M8 14l3-3 2 2 2-3 3 4"></path><path d="M8 9h.01"></path></svg>'}[f]||""}function s(f){return Object.freeze({renderActivityItem:a=>l(a,f),renderRelationshipChips:a=>e(a,f),relationshipChip:(a,i,u)=>n(a,i,u,f),photoJumpChip:(a,i)=>t(a,i,f),relationshipIcon:r})}window.MaintainOpsRelationshipDisplay=Object.freeze({createRelationshipDisplayHelpers:s}),typeof Ee<"u"&&Ee.exports&&(Ee.exports={createRelationshipDisplayHelpers:s})})()});var yn=Q((Or,dt)=>{(function(){function l(e){let n=e.segmentIcon,t=e.escapeHtml,r=e.renderAssetOptions,s=e.renderMaintenanceRequestPhoto,f=e.isConvertedRequest,a=e.canDeleteOperationalRecords,i=e.canEditOperationalRecords||(()=>!0),u=e.getPendingDeleteRequestId,m=e.getProfilesByUserId;function c(d,h){return d==="converted"?`${h} converted`:d==="all"?`${h} total`:`${h} active`}function o(d,h,y={}){return`
    ...
    `}function ae(M){return`
        ...
        <article class="part-card part-tile ${q?"low-stock":""}" data-open-part="${w.id}" tabindex="0" role="button" aria-label="Open ${e(w.name)}">
          <div class="part-card-main">
            ...
            <div class="part-card-meta">
              ...
              <span>${m()?`${n(O)} listed cost`:"Cost reference not active yet"}</span>
            </div>
            ${q&&R>0?`<small>Need ${L} to reach reorder point.</small>`:""}
          </div>
          <span class="part-tile-open">Open</span>
        </article>
      `}function T(){let w=s().filter(r),P=w.filter(t).length,R=c();return[["All Parts",w.length,"all"],["Low Stock",P,"low"]].map(([O,q,L])=>`
      ...
      `}function D(w){let P=Number(w.quantity_on_hand)||0,R=Number(w.reorder_point)||0,O=Number(w.unit_cost)||0,q=f()[w.id]||[],L=S(q),I=d();return`
      ...
      `}function W(w){let P=p(w.id).length,R=f()[w.id]||[],O=i()===w.id;return g()?`
    ...
    `}return{renderWorkOrderDetail:P}}window.MaintainOpsWorkOrderDetailDisplay={createWorkOrderDetailDisplayHelpers:l},typeof yt<"u"&&(yt.exports={createWorkOrderDetailDisplayHelpers:l})})()});var $n=Q((Lr,bt)=>{(function(){function l(){function e(){return`
      ...
      `}function A(){if(!u())return"";let E=c(),C=r(),v=f();return`
      ...
      `}function $(E){let C=s().find(D=>D.location_id===E.id),v=!!(C&&C.is_active!==!1),S=m(),_=v?o(C.token):"",T=v?p(C.token):"",N=!!(_&&T);return`
        ...
        <details class="planning-group" data-planning-group="${e(h)}" ${C?"open":""}>
          ...
          <div class="planning-group-body">
            <div class="planning-list">
              ${E.map(o).join("")||'<p class="muted">Nothing here.</p>'}
            </div>
...
//# sourceMappingURL=runtime.a862490882.js.map
