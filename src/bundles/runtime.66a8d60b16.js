...
`),status:"open"}),"Upload failure report timed out.",8e3)}catch(U){s.warn("Could not report upload failure",U)}}}function O(){return{targetBytes:256*1024,passes:[{maxDimension:768,quality:.78},{maxDimension:768,quality:.74},{maxDimension:768,quality:.7}]}}async function C(w,P={}){if(typeof e.optimizePhotoOverride=="function")return e.optimizePhotoOverride(w,P);let R=["image/jpeg","image/png","image/webp","image/heic","image/heif"],E=T(w);if(!R.includes(E))return{blob:w,fileName:e.safeFileName(w.name||"photo"),contentType:E};try{if(!f)throw new Error("Browser image optimization is unavailable.");let q=await f(w),L=Number(P.targetBytes||0)||1*1024*1024,I=P.passes||[{maxDimension:2e3,quality:.82},{maxDimension:1800,quality:.78},{maxDimension:1600,quality:.74}],z=null;for(let U of I){let V=await W(q,U.maxDimension,U.quality);if(z=V,V.size<=L)break}if(q.close&&q.close(),!z)throw new Error("Browser could not optimize this image.");return{blob:z,fileName:`${e.fileBaseName(w.name||"photo")}.jpg`,contentType:"image/jpeg"}}catch(q){return s.warn("Photo optimization failed; uploading original.",q),{blob:w,fileName:e.safeFileName(w.name||"photo"),contentType:E}}}function v(w){return["image/jpeg","image/png","image/webp"].includes(T(w))}function S(w){return!v(w)&&Number(w.size||0)>a}function _(){return"This non-image file is over 25 MB. Compress it or split it before uploading."}function T(w){let P=String(w?.type||"").trim().toLowerCase();if(P)return P;let R=String(w?.name||"").toLowerCase();return/\.(jpe?g)$/.test(R)?"image/jpeg":/\.png$/.test(R)?"image/png":/\.webp$/.test(R)?"image/webp":/\.gif$/.test(R)?"image/gif":/\.heic$/.test(R)?"image/heic":/\.heif$/.test(R)?"image/heif":/\.pdf$/.test(R)?"application/pdf":/\.txt$/.test(R)?"text/plain":/\.csv$/.test(R)?"text/csv":/\.doc$/.test(R)?"application/msword":/\.docx$/.test(R)?"application/vnd.openxmlformats-officedocument.wordprocessingml.document":/\.xls$/.test(R)?"application/vnd.ms-excel":/\.xlsx$/.test(R)?"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet":"application/octet-stream"}function N(w){let P=T(w);return u.has(P)?"":"This upload box accepts photos only. PDF quotes and documents need to be attached in an equipment or part file area."}function D(w){return u.has(String(w?.contentType||"").toLowerCase())?Number(w?.blob?.size||0)>i?"This photo is still over 5 MB after optimization. Try a smaller photo or screenshot it first.":"":"This upload box accepts photos only. PDF quotes and documents need to be attached in an equipment or part file area."}async function W(w,P,R){let E=Math.min(1,P/Math.max(w.width,w.height)),q=Math.max(1,Math.round(w.width*E)),L=Math.max(1,Math.round(w.height*E)),I=n.createElement("canvas");I.width=q,I.height=L,I.getContext("2d",{alpha:!1}).drawImage(w,0,0,q,L);let U=await new Promise(V=>I.toBlob(V,"image/jpeg",R));if(!U)throw new Error("Browser could not optimize this image.");return U}return{addPhotoToMaintenanceRequest:A,addPhotoToWorkOrder:k,optimizePhoto:C,removeUploadedObject:b,reportUploadFailure:$,deleteAssetDocument:p,deleteWorkOrderPhoto:g,uploadAssetDocument:o,uploadPartDocument:c,uploadPhoto:y}}typeof Ae<"u"&&Ae.exports&&(Ae.exports={createMediaStorageWorkflow:l}),window.MaintainOpsMediaStorageWorkflow={createMediaStorageWorkflow:l}})()});var tn=Q((gr,Re)=>{(function(){function l(e={}){let n=e.documentRef||document,t=e.FormDataCtor||FormData,r=e.cryptoRef||crypto,s=e.URLRef||URL,f=e.consoleRef||console,a=e.createImageBitmapRef||(typeof createImageBitmap<"u"?createImageBitmap:null),i=25*1024*1024,u=new Set(["image/jpeg","image/png","image/webp","image/gif","image/heic","image/heif"]);async function m(d){d.preventDefault();let h=d.currentTarget,y=n.querySelector("#company-logo-error"),b=h.querySelector("button[type='submit']"),k=new t(h).get("logo");if(y&&(y.textContent=""),!k||!k.name){y&&(y.textContent="Choose a logo image first.");return}b&&(b.disabled=!0,b.textContent="Uploading...");try{let A=p(k);if(A)throw new Error(A);let $=await c(k),O=g($);if(O)throw new Error(O);let C=`${e.getActiveCompanyId()}/logo-${r.randomUUID()}-${$.fileName}`,v=await e.withOperationTimeout(e.supabaseClient().storage.from("company-logos").upload(C,$.blob,{contentType:$.contentType,upsert:!1}),"Company logo upload timed out. Check your connection and try again.",25e3);if(v.error)throw new Error(v.error.message.includes("Bucket not found")?"Run supabase/step-next-company-logo.sql before uploading a logo.":v.error.message);let{error:S}=await e.withOperationTimeout(e.supabaseClient().rpc("set_company_logo",{target_company_id:e.getActiveCompanyId(),new_logo_path:C}),"Company logo record save timed out. Check your connection and try again.",15e3);if(S)throw await e.removeUploadedObject("company-logos",C),new Error(e.isColumnSchemaError(S,["logo_path"])?"Run supabase/step-next-company-logo.sql before saving a company logo.":S.message.includes("set_company_logo")?"Run supabase/step-next-company-logo.sql, then try uploading the logo again.":S.message);let _=e.getCompanies().find(T=>T.id===e.getActiveCompanyId());_&&(_.logo_path=C,_.logoUrl=s.createObjectURL($.blob)),e.showNotice("Company logo uploaded."),await e.render()}catch(A){y&&(y.textContent=A.message||"Could not upload logo.")}finally{b&&(b.disabled=!1,b.textContent="Upload Logo")}}async function c(d){if(typeof e.optimizeLogoOverride=="function")return e.optimizeLogoOverride(d);let h=o(d);try{if(!a)throw new Error("Browser logo optimization is unavailable.");let y=await a(d),k=Math.min(1,1200/Math.max(y.width,y.height)),A=Math.max(1,Math.round(y.width*k)),$=Math.max(1,Math.round(y.height*k)),O=n.createElement("canvas");O.width=A,O.height=$;let C=O.getContext("2d",{alpha:!0});C.clearRect(0,0,A,$),C.drawImage(y,0,0,A,$),y.close&&y.close();let v=await new Promise(S=>O.toBlob(S,"image/png"));if(!v)throw new Error("Browser could not optimize this logo.");return{blob:v,fileName:`${e.fileBaseName(d.name||"logo")}.png`,contentType:"image/png"}}catch(y){return f.warn("Logo optimization failed; uploading original.",y),{blob:d,fileName:e.safeFileName(d.name||"logo"),contentType:h}}}function o(d){let h=String(d?.type||"").trim().toLowerCase();if(h)return h;let y=String(d?.name||"").toLowerCase();return/\.(jpe?g)$/.test(y)?"image/jpeg":/\.png$/.test(y)?"image/png":/\.webp$/.test(y)?"image/webp":/\.gif$/.test(y)?"image/gif":/\.heic$/.test(y)?"image/heic":/\.heif$/.test(y)?"image/heif":/\.avif$/.test(y)?"image/avif":/\.bmp$/.test(y)?"image/bmp":/\.tiff?$/.test(y)?"image/tiff":"application/octet-stream"}function p(d){let h=o(d);return u.has(h)?"":"Company logos must be JPG, PNG, WebP, GIF, HEIC, or HEIF images."}function g(d){return u.has(String(d?.contentType||"").toLowerCase())?Number(d?.blob?.size||0)>i?"This logo is still over 25 MB after processing. Try a smaller logo image.":"":"Company logos must be JPG, PNG, WebP, GIF, HEIC, or HEIF images."}return{optimizeLogo:c,uploadCompanyLogo:m}}typeof Re<"u"&&Re.exports&&(Re.exports={createCompanyLogoWorkflow:l}),window.MaintainOpsCompanyLogoWorkflow={createCompanyLogoWorkflow:l}})()});var nn=Q((hr,Xe)=>{(function(){function l(e={}){let n=e.documentRef||document,t=e.CSSRef||CSS,r=e.alertUser||alert;function s(i){return e.partUsageRows(i).length?"This part has work order usage history and is kept for traceability.":e.assetPartRows(i).length?"This part is linked to equipment and is kept for traceability.":""}function f(i){if(!e.canDeleteParts()){r("Only company admins and managers can delete parts.");return}if(!e.getParts().find(o=>o.id===i))return;let m=s(i);if(m){r(m);return}let c=!!n.querySelector(`[data-delete-part="${t.escape(i)}"].permanent-delete-button`);if(e.getPendingDeletePartId()===i||c){a(i);return}e.setPendingDeletePartId(i),e.renderWorkspace()}async function a(i){if(!e.canDeleteParts()){r("Only company admins and managers can delete parts.");return}let u=e.getParts().find(p=>p.id===i),m=n.querySelector("#part-delete-error");if(m&&(m.textContent=""),!u)return;let c=s(i);if(c){m&&(m.textContent=c);return}let o=n.querySelector(`[data-delete-part="${t.escape(i)}"].permanent-delete-button`);o&&(o.disabled=!0,o.textContent="Deleting...");try{let p=(e.getPartDocumentsByPartId()[i]||[]).map(y=>y.storage_path).filter(Boolean);if(p.length){let y=await e.withOperationTimeout(e.removePartDocumentStorage(p),"Part document cleanup timed out. Try deleting again.",15e3);if(y.error)throw new Error(`Could not remove filed receipts/invoices: ${y.error.message}`)}let{data:g,error:d}=await e.withOperationTimeout(e.deletePartRecord(i),"Part delete timed out. Check your connection and try again.",15e3);if(d)throw new Error(d.message.includes("violates foreign key constraint")?"This part is linked to work or equipment and cannot be deleted.":d.message);if(!g?.length)throw new Error("Part was not deleted. Check that your company role is admin or manager and that supabase/step-next-part-delete.sql has been run.");let h=await e.withOperationTimeout(e.verifyPartDeleted(i),"Part delete verification timed out. Refresh and check the part list.",15e3);if(h.error)throw new Error(`Part delete verification failed: ${h.error.message}`);if(h.data)throw new Error("Part delete did not persist in Supabase. Run supabase/step-next-part-delete.sql, then try again.");e.setActivePartId(null),e.setPendingDeletePartId(null),e.showNotice("Part deleted."),await e.render()}catch(p){e.showNotice(p.message||"Could not delete part.","warning"),m&&(m.textContent=p.message||"Could not delete part."),o&&(o.disabled=!1,o.textContent="Permanently Delete")}}return{deletePart:a,requestDeletePart:f}}window.MaintainOpsPartDeleteWorkflow={createPartDeleteWorkflow:l},typeof Xe<"u"&&(Xe.exports={createPartDeleteWorkflow:l})})()});var rn=Q((yr,et)=>{(function(){function l(e={}){async function n(t){let r=t.target,s=r.type==="checkbox"?r.checked?"checked":"":r.value;r.disabled=!0;try{let{error:f}=await e.withOperationTimeout(e.upsertStepResult({company_id:e.getActiveCompanyId(),work_order_id:r.dataset.workOrderId,procedure_step_id:r.dataset.stepResult,completed_by:s?e.getSession().user.id:null,value:s,completed_at:s?new Date().toISOString():null}),"Checklist save timed out. Check your connection and try again.",15e3);if(f)throw f;await e.withOperationTimeout(e.recordWorkOrderEvent(r.dataset.workOrderId,"checklist_updated","Procedure checklist updated."),"Activity log timed out.",8e3).catch(()=>null);let a=await e.withOperationTimeout(e.loadStepResults(),"Checklist refresh timed out. Refresh the workspace to confirm the latest checklist state.",1e4).catch(i=>i);if(a){e.showNotice(`Checklist saved, but refresh did not finish: ${a.message||a}`,"warning"),r.disabled=!1;return}if(e.getWorkOrderActionWarningId()===r.dataset.workOrderId){let i=e.getWorkOrders().find(u=>u.id===r.dataset.workOrderId);e.blocksProcedureCompletion(i)||e.setWorkOrderActionWarning("","")}e.renderWorkspace()}catch(f){e.showNotice(`Could not save checklist step: ${f.message||f}`,"warning"),r.disabled=!1}}return{saveStepResult:n}}window.MaintainOpsProcedureChecklistWorkflow={createProcedureChecklistWorkflow:l},typeof et<"u"&&(et.exports={createProcedureChecklistWorkflow:l})})()});var an=Q((br,tt)=>{(function(){function l(e={}){let n=e.documentRef||document,t=e.bodyRef||document.body,r=e.FormDataCtor||FormData;async function s(m,c){let{data:o,error:p}=await e.withOperationTimeout(e.getPublicRequestIntake(m),c);return{data:Array.isArray(o)?o[0]:o,error:p}}async function f(m){t.classList.add("public-qr-mode"),e.setAppHtml(e.loadingQrPage());let c=null;try{let{data:p,error:g}=await s(m,"Request QR lookup timed out.");if(c=p,g||!c){i("This QR code link is inactive or invalid.");return}}catch{i("This QR code link is inactive or invalid.");return}let o=e.publicRequestUrl(m);e.setAppHtml(e.publicRequestQrPage(c,o)),e.bindPublicQrPrintEvents(),typeof e.ensureQrLibrary=="function"&&e.ensureQrLibrary().then(()=>{e.setAppHtml(e.publicRequestQrPage(c,o)),e.bindPublicQrPrintEvents()}).catch(()=>{})}async function a(m){t.classList.remove("public-qr-mode"),e.setAppHtml(e.loadingRequestForm());let c=null;try{let{data:o,error:p}=await s(m,"Request form lookup timed out.");if(p){i("This request link is not ready yet. The company needs to run the public request link setup in Supabase.");return}c=o}catch(o){i(o.message||"This request link could not be loaded.");return}if(!c){i("This request link is inactive or invalid.");return}e.setAppHtml(e.publicRequestForm(c)),n.querySelector("#public-request-form").addEventListener("submit",o=>u(o,m,c))}function i(m){e.setAppHtml(e.publicRequestError(m))}async function u(m,c,o){m.preventDefault();let p=m.currentTarget,g=new r(p),d=n.querySelector("#public-request-error"),h=p.querySelector("button[type='submit']");d&&(d.textContent=""),h&&(h.disabled=!0,h.textContent="Sending...");try{let{data:y,error:b}=await e.withOperationTimeout(e.submitPublicLocationRequest({request_token:c,request_title:e.requiredText(g.get("title"),"Request title"),equipment_note:e.requiredText(g.get("equipment_note"),"Machine / area"),request_description:e.requiredText(g.get("description"),"Request details"),requester_name:e.requiredText(g.get("requester_name"),"Your name"),requester_contact:String(g.get("requester_contact")||"").trim()||null,request_priority:g.get("priority")||"medium"}),"Request send timed out.");if(b)throw b;let k=g.get("photo"),A="";if(k&&k.name){let O=await e.addPhotoToMaintenanceRequest(y,k);O&&(A=`Request sent, but the photo did not upload: ${O.message||O}`)}let $=await e.notifyRequestEmailer(y);$.error&&e.warn("Request email notification did not send",$.error),e.setAppHtml(e.publicRequestSuccess(o,A)),n.querySelector("#public-request-another").addEventListener("click",()=>a(c))}catch(y){d&&(d.textContent=y.message||"Could not send the request.")}finally{h?.isConnected&&(h.disabled=!1,h.textContent="Send Request")}}return{renderPublicRequestError:i,renderPublicRequestIntake:a,renderPublicRequestQrPage:f,submitPublicRequest:u}}window.MaintainOpsPublicRequestIntakeWorkflow={createPublicRequestIntakeWorkflow:l},typeof tt<"u"&&(tt.exports={createPublicRequestIntakeWorkflow:l})})()});var on=Q((wr,nt)=>{(function(){function l(e={}){let n=e.documentRef||document,t=e.FormDataCtor||FormData;function r(){e.setAppHtml(e.companyCreateForm(e.getAppError())),n.querySelector("#company-form").addEventListener("submit",s),n.querySelector("#sign-out").addEventListener("click",()=>e.signOut())}async function s(f){f.preventDefault();let a=f.target,i=a.querySelector("button[type='submit']"),u=n.querySelector("#company-error"),m=String(new t(a).get("name")||"").trim();i.disabled=!0,i.textContent="Creating...",u.textContent="";try{if(!m)throw new Error("Company name is required.");let c=e.getCompanies().find(d=>d.name.trim().toLowerCase()===m.trim().toLowerCase());if(c){e.setActiveCompanyId(c.id),e.persistActiveCompanyId(c.id),await e.render();return}let{data:o,error:p}=await e.withOperationTimeout(e.createCompanyRecord(m),"Company creation timed out.");if(p){u.textContent=p.message.includes("create_company")?"Database setup is not finished. Run supabase/schema.sql in the Supabase SQL editor, then wait a few seconds and try again.":p.message;return}if(e.setActiveCompanyId(o),e.persistActiveCompanyId(o),!await e.ensureProfileForActiveCompany(m))throw new Error(e.getAppError()||"Could not create your company profile.");await e.seedStarterAssets(),await e.render()}catch(c){u.textContent=c.message||"Could not create company."}finally{i?.isConnected&&(i.disabled=!1,i.textContent="Create Company")}}return{createCompany:s,renderCompanyCreate:r}}window.MaintainOpsCompanySetupWorkflow={createCompanySetupWorkflow:l},typeof nt<"u"&&(nt.exports={createCompanySetupWorkflow:l})})()});var sn=Q((vr,rt)=>{(function(){function l(e={}){async function n(r){let s=e.getWorkOrders().find(f=>f.id===e.getActiveWorkOrderId());r.target.disabled=!0;try{await t(e.getActiveWorkOrderId(),r.target.value)||(r.target.value=s?.status||"open")}catch(f){r.target.value=s?.status||"open",e.showNotice(`Could not update status: ${f.message||f}`,"warning")}finally{r.target.disabled=!1}}async function t(r,s){let f=e.getWorkOrders().find(c=>c.id===r);if(s==="completed"){let c=e.productionActionCompletionMessage?.(f)||"";if(c)return e.setActiveWorkOrderId(r),e.setWorkOrderActionWarning(r,c),e.showNotice(c,"warning"),await e.render(),!1;let o=e.blocksProcedureCompletion(f);if(o)return e.setActiveWorkOrderId(r),e.setWorkOrderActionWarning(r,o),e.showNotice(o,"warning"),await e.render(),!1}let a=e.currentSafetyCheckboxCheckedForWorkOrder(r),i=e.hasCompletedSafetyDeviceCheck(f)||a;if(s==="completed"&&e.requiresSafetyDeviceCheck(f)&&!i){e.setActiveWorkOrderId(r);let c="Safety devices must be checked before completing equipment work. Open the work order and use Complete Work.";return e.setWorkOrderActionWarning(r,c),e.showNotice(c,"warning"),await e.render(),!1}let u={status:s,asset_id:f?.asset_id||null,completed_at:s==="completed"?new Date().toISOString():null};e.applySafetyRequirementPayload(u),s==="completed"?e.applySafetyCheckPayload(u,u.safety_check_required&&i):s!=="completed"&&e.applySafetyCheckPayload(u,!1),delete u.asset_id;let{error:m}=await e.withOperationTimeout(e.updateWorkOrderSafely(u,r),"Status save timed out. Check your connection and try again.",15e3);return m?(e.showNotice(`Could not update status: ${e.friendlyWorkOrderSaveError(m)}`,"warning"),!1):(e.setActiveWorkOrderId(r),e.setWorkOrderActionWarning("",""),await e.recordWorkOrderEvent(r,"status_changed",`Status changed to ${e.statusLabel(s)}.`),e.showNotice(`Status changed to ${e.statusLabel(s)}.`),await e.render(),!0)}return{setWorkOrderStatus:t,updateWorkOrderStatus:n}}window.MaintainOpsWorkOrderStatusWorkflow={createWorkOrderStatusWorkflow:l},typeof rt<"u"&&(rt.exports={createWorkOrderStatusWorkflow:l})})()});var cn=Q((kr,Ee)=>{(function(){function l(e={}){let n=e.FormDataCtor||FormData,t=e.confirmRef||confirm;function r(u,m){return u?.closest?.("[data-production-action-control]")?.querySelector?.(`[data-production-action-error="${m}"]`)||e.documentRef?.querySelector?.(`[data-production-action-error="${m}"]`)||null}async function s({workOrderId:u,payload:m,source:c,busyText:o,successMessage:p}){let g=c?.querySelector?.("button[type='submit']")||c,d=g?.textContent||"",h=r(c,u);g&&(g.disabled=!0,g.textContent=o),h&&(h.textContent="");try{let y=await e.withOperationTimeout(e.updateProductionActionRecord(u,m),"Production Action save timed out. Check your connection and try again.",15e3);if(y.error){let b=e.friendlyWorkOrderSaveError(y.error);return h?h.textContent=`Could not save Production Action: ${b}`:e.showNotice(`Could not save Production Action: ${b}`,"warning"),!1}return e.showNotice(p,"success"),await e.afterProductionActionMutation(y.data,u),!0}catch(y){let b=y.message||String(y);return h?h.textContent=`Could not save Production Action: ${b}`:e.showNotice(`Could not save Production Action: ${b}`,"warning"),!1}finally{g?.isConnected&&(g.disabled=!1,g.textContent=d)}}async function f(u){u.preventDefault(),u.stopPropagation();let m=u.currentTarget,c=m.dataset.productionActionForm,o=new n(m),p=String(o.get("production_action")||"").trim(),g=String(o.get("production_action_assigned_to")||"").trim(),d=r(m,c);if(!p||!g){d&&(d.textContent="Enter an action and choose a Production owner.");return}let h=e.getWorkOrderById(c);await s({workOrderId:c,payload:{production_action:p,production_action_assigned_to:g},source:m,busyText:"Saving...",successMessage:h?.production_action?"Production Action updated.":"Production Action assigned."})}async function a(u){u.preventDefault(),u.stopPropagation();let m=u.currentTarget,c=m.dataset.workOrderId,o=m.dataset.productionActionStatus;await s({workOrderId:c,payload:{production_action_status:o},source:m,busyText:o==="completed"?"Completing...":"Reopening...",successMessage:o==="completed"?"Production Action completed.":"Production Action reopened."})}async function i(u){u.preventDefault(),u.stopPropagation();let m=u.currentTarget,c=m.dataset.productionActionRemove;t("Remove this Production Action? Work Order History will keep a record of the removal.")&&await s({workOrderId:c,payload:{production_action:null},source:m,busyText:"Removing...",successMessage:"Production Action removed."})}return{saveProductionAction:f,setProductionActionStatus:a,removeProductionAction:i}}window.MaintainOpsProductionActionWorkflow={createProductionActionWorkflow:l},typeof Ee<"u"&&Ee.exports&&(Ee.exports={createProductionActionWorkflow:l})})()});var ln=Q((_r,at)=>{(function(){function l(e={}){async function n(s,f={}){let a=s.filter(c=>!c.read_at);if(!a.length)return!0;let i=new Map(a.map(c=>[c.id,c])),u=new Date().toISOString(),m=a.map(c=>c.id);e.setNotifications(e.getNotifications().map(c=>i.has(c.id)?{...c,read_at:u}:c)),f.render!==!1&&e.renderWorkspace();try{let c=await e.withOperationTimeout(e.markWorkOrderNotificationsRead(e.getSupabaseClient(),e.getSession().user.id,m,u),"Work notification update timed out.",1e4);if(c.error)throw c.error;return!0}catch(c){return e.setNotifications(e.getNotifications().map(o=>i.get(o.id)||o)),e.showNotice(`Could not mark the work notification read: ${c.message||c}`,"warning"),f.render!==!1&&e.renderWorkspace(),!1}}function t(s,f={}){return n(e.getNotifications().filter(a=>a.id===s),f)}function r(s,f={}){return n(e.getNotifications().filter(a=>a.work_order_id===s),f)}return{markWorkOrderNotificationRead:t,markWorkOrderNotificationsReadForOrder:r}}window.MaintainOpsWorkOrderNotificationWorkflow={createWorkOrderNotificationWorkflow:l},typeof at<"u"&&(at.exports={createWorkOrderNotificationWorkflow:l})})()});var un=Q((Sr,ot)=>{(function(){function l(e){async function n(t,r){if(!e.canEditOperationalRecords())return e.showNotice("This account can view Planning but cannot change work orders.","warning"),{saved:!1,reason:"read_only"};let s=e.getPlanningWorkOrders().find(f=>f.id===t);if(!s||s.status==="completed")return e.showNotice("That work order is no longer available in the no-due-date queue.","warning"),{saved:!1,reason:"not_available"};try{let f=e.workOrderDateValue(r);if(!f)throw new Error("Choose a due date.");let a=await e.withOperationTimeout(e.updateWorkOrderSafely({due_at:f},t),"Due date save timed out. Check your connection and try again.");if(a.error)throw a.error;return e.setPlanningWorkOrders(e.getPlanningWorkOrders().map(i=>i.id===t?{...i,due_at:f}:i)),e.setWorkOrders(e.getWorkOrders().map(i=>i.id===t?{...i,due_at:f}:i)),e.resetNoDuePage(),await e.recordWorkOrderEvent(t,"updated",`Due date set to ${f} from Planning.`),e.showNotice("Due date set. The order moved out of No Due Date."),e.renderWorkspace(),{saved:!0,dueAt:f}}catch(f){return e.showNotice(`Could not set due date: ${f.message||f}`,"warning"),{saved:!1,reason:"save_failed",error:f}}}return{savePlanningDueDate:n}}window.MaintainOpsPlanningDueDateWorkflow={createPlanningDueDateWorkflow:l},typeof ot<"u"&&(ot.exports={createPlanningDueDateWorkflow:l})})()});var dn=Q((qr,it)=>{(function(){function l(n,t,r,s=50){let f=Math.min(Math.max(Number(s)||50,1),100);return n.from("work_order_notifications").select("id, company_id, work_order_id, recipient_id, actor_id, source_event_id, kind, title, body, read_at, created_at").eq("company_id",t).eq("recipient_id",r).order("created_at",{ascending:!1}).limit(f)}function e(n,t,r,s){let f=[...new Set((r||[]).filter(Boolean))];return f.length?n.from("work_order_notifications").update({read_at:s}).eq("recipient_id",t).in("id",f).select("id, read_at"):Promise.resolve({data:[],error:null})}window.MaintainOpsWorkOrderNotificationsService={listWorkOrderNotifications:l,markWorkOrderNotificationsRead:e},typeof it<"u"&&(it.exports={listWorkOrderNotifications:l,markWorkOrderNotificationsRead:e})})()});var pn=Q((Cr,st)=>{(function(){async function l(e,n){if(!e?.functions?.invoke||!n)return{data:null,error:null,skipped:!0};try{let{data:t,error:r}=await e.functions.invoke("request-emailer",{body:{request_id:n}});return{data:t,error:r||null,skipped:!1}}catch(t){return{data:null,error:t,skipped:!1}}}window.MaintainOpsRequestEmailNotificationService={notifyRequestEmailer:l},typeof st<"u"&&(st.exports={notifyRequestEmailer:l})})()});var mn=Q(($r,ct)=>{(function(){async function l(n,t,r=[],s={}){let f=s.pathKey||"storage_path",a=s.urlKey||"signedUrl",i=s.expiresIn||600,u=s.onError;await Promise.all(r.map(async m=>{let c=m?.[f];if(!c)return;let{data:o,error:p}=await n.storage.from(t).createSignedUrl(c,i);if(p){m[a]="",typeof u=="function"&&u(m,p);return}m[a]=o?.signedUrl||""}))}function e(n={}){function t(r){if(!r||!n.getReady())return;let f=(n.getRows(r)||[]).filter(i=>i.storage_path&&!i.signedUrl),a=n.getSigningMap();!f.length||a[r]||(a[r]=!0,n.withOperationTimeout(l(n.supabaseClient(),n.bucketName,f),n.timeoutMessage||"Signed file link load timed out.",n.timeoutMs||1e4).catch(i=>{n.warn("Could not load signed file links",i)}).finally(()=>{delete a[r],n.getActiveGroupId()===r&&n.renderWorkspace()}))}return{ensureGroupSignedUrls:t}}window.MaintainOpsSignedUrlService={addSignedUrlsToRows:l,createDeferredSignedUrlLoader:e},typeof ct<"u"&&(ct.exports={addSignedUrlsToRows:l,createDeferredSignedUrlLoader:e})})()});var fn=Q((Pr,lt)=>{(function(){function l(t,r){if(t[r]===void 0)throw new Error(`workspaceQueueLoadersService missing dependency: ${r}`);return t[r]}function e(t={}){let r=l(t,"supabaseClient"),s=l(t,"workspaceUiState"),f=l(t,"applyRequestQueryFilters"),a=l(t,"applyWorkOrderListFilters"),i=l(t,"applyWorkOrderFilters"),u=l(t,"selectWorkOrders"),m=l(t,"countWorkOrdersQuery"),c=l(t,"fetchExactSearchedWorkOrderPage"),o=l(t,"isColumnSchemaError"),p=t.warn||(()=>{}),g=l(t,"LIST_ITEMS_PER_PAGE"),d=l(t,"WORK_ORDERS_PER_PAGE"),h=l(t,"REQUEST_RELATION_SELECT"),y=l(t,"REQUEST_ASSET_FALLBACK_SELECT"),b=l(t,"REQUEST_FALLBACK_SELECT"),k=l(t,"WORK_ORDER_RELATION_SELECT"),A=l(t,"WORK_ORDER_FALLBACK_SELECT");function $(){return typeof r=="function"?r():r}async function O(D=s.getRequestViewFilter(),W={}){let w=Math.max(1,s.getRequestsPage()),P=(w-1)*g,R=P+g-1,E=W.includeRelations===!1?b:W.includeLocationRelation===!1?y:h,q=await f($().from("maintenance_requests").select(E,{count:"exact"}),D).order("created_at",{ascending:!1}).range(P,R);return q.error&&W.includeLocationRelation!==!1&&o(q.error,["location_id","locations"])?O(D,{includeLocationRelation:!1}):q.error&&W.includeRelations!==!1?O(D,{includeRelations:!1}):!q.error&&q.count&&w>1&&P>=q.count?(s.setRequestsPage(Math.max(1,Math.ceil(q.count/g))),O(D,W)):q}async function C(D){let W=await f($().from("maintenance_requests").select("id",{count:"exact",head:!0}),D);return W.error?(p("Request count failed",W.error),0):W.count||0}async function v(){let[D,W,w]=await Promise.all([C("active"),C("converted"),C("all")]);return{active:D,converted:W,all:w}}async function S(D={}){if(s.getWorkOrderSearchMode()&&s.getSearchQuery().trim())return c(D);let W=Math.max(1,s.getWorkOrderPage()),w=(W-1)*d,P=w+d-1,R=D.includeLocationRelation===!1?A:k,E=await a(u($(),R,{count:"exact"})).range(w,P);return!E.error&&E.count&&W>1&&w>=E.count?(s.setWorkOrderPage(Math.max(1,Math.ceil(E.count/d))),S(D)):E}async function _(D={}){let W=await i(m($()),D);return W.error?(p("Work order count failed",W.error),0):W.count||0}async function T(){let[D,W,w,P,R,E,q,L]=await Promise.all([_({statusFilter:"active",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1}),_({statusFilter:"open",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1}),_({statusFilter:"in_progress",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1}),_({statusFilter:"blocked",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1}),_({statusFilter:"overdue",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1}),_({statusFilter:"completed",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1}),_({statusFilter:"completed_month",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1}),_({statusFilter:"completed_week",includeQueue:!1,includeSearch:!1,includeAttributeFilters:!1})]);return{activeWork:D,newWork:W,inProgress:w,blocked:P,overdue:R,completedAll:E,completedMonth:q,completedWeek:L}}async function N(){let[D,W,w,P,R,E,q,L]=await Promise.all([_({statusFilter:"active",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1}),_({statusFilter:"open",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1}),_({statusFilter:"in_progress",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1}),_({statusFilter:"blocked",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1}),_({statusFilter:"overdue",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1}),_({statusFilter:"completed",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1}),_({statusFilter:"completed_month",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1}),_({statusFilter:"completed_week",section:"mywork",includeQueue:!0,includeSearch:!0,includeAttributeFilters:!1})]);return{activeWork:D,newWork:W,inProgress:w,blocked:P,overdue:R,completedAll:E,completedMonth:q,completedWeek:L}}return{fetchRequestPage:O,countRequests:C,loadRequestDashboardCounts:v,fetchWorkOrderPage:S,countWorkOrders:_,loadWorkOrderDashboardCounts:T,loadMyWorkDashboardCounts:N}}let n={createWorkspaceQueueLoaders:e};typeof window<"u"&&(window.MaintainOpsWorkspaceQueueLoadersService=n),typeof lt<"u"&&(lt.exports=n)})()});var gn=Q((Ar,ut)=>{(function(){function l(e={}){let n=e.windowRef||window,t=e.documentRef||document,r=e.app;function s(){return n.MaintainOpsAuthRedirects.authCallbackUrl(n.location,n.PUBLIC_APP_URL)}function f(){return n.MaintainOpsAuthRedirects.cleanAuthUrl(n.location)}function a(){n.history.replaceState({},t.title,n.MaintainOpsAuthRedirects.cleanAuthUrl(n.location))}async function i(g){u("Verifying your account...");try{if(g.error||g.errorDescription)throw new Error(g.errorDescription||g.error||"This verification link is invalid or expired.");let d=null;if(g.code){let{data:h,error:y}=await e.supabaseClient.auth.exchangeCodeForSession(g.code);if(y)throw y;d=h?.session||null}else if(g.accessToken&&g.refreshToken){let{data:h,error:y}=await e.supabaseClient.auth.setSession({access_token:g.accessToken,refresh_token:g.refreshToken});if(y)throw y;d=h?.session||null}if(!d){let{data:h,error:y}=await e.supabaseClient.auth.getSession();if(y)throw y;d=h?.session||null}if(!d)throw new Error("The verification link did not create a session. Request a new verification email and try again.");e.setSession(d),a(),u("Verification complete. Loading workspace..."),await e.render()}catch(d){a(),m(d.message||"This verification link is invalid or expired.")}}function u(g){t.body.classList.remove("public-qr-mode"),r.innerHTML=e.authCallback(g)}function m(g){t.body.classList.remove("public-qr-mode"),r.innerHTML=e.authCallbackError(g),t.querySelector("#auth-back-to-login").addEventListener("click",()=>e.renderAuth("login"))}async function c(g=e.passwordRecoveryParamsFromUrl()){let d=!1,h="";if(g.accessToken&&g.refreshToken){let{data:y,error:b}=await e.supabaseClient.auth.setSession({access_token:g.accessToken,refresh_token:g.refreshToken});d=!!(y?.session&&!b),b&&(h="This reset link is expired or invalid. Send a new password reset email and use the newest link.")}else h="This reset link is missing the secure session. Send a new password reset email and use the newest link.";p({ready:d,initialError:h})}function o(g="",d=""){t.body.classList.remove("public-qr-mode"),r.innerHTML=e.passwordResetRequest(g,d),t.querySelector("#auth-back-to-login").addEventListener("click",()=>e.renderAuth("login")),t.querySelector("#auth-reset").addEventListener("click",e.resetLoginState),t.querySelector("#password-reset-request-form").addEventListener("submit",async h=>{h.preventDefault();let y=h.target,b=y.querySelector("button[type='submit']"),k=t.querySelector("#auth-error"),A=t.querySelector("#auth-status"),$=String(new FormData(y).get("email")||"").trim();k.textContent="",A.textContent="Sending reset link...",b.disabled=!0,b.textContent="Sending...";try{let{error:O}=await e.withOperationTimeout(e.supabaseClient.auth.resetPasswordForEmail($,{redirectTo:f()}),"Password reset email timed out. Check your connection and try again.",2e4);if(O){A.textContent="",k.textContent=O.message;return}A.textContent="If that email exists in Supabase, a reset link has been sent."}catch(O){A.textContent="",k.textContent=O.message||"Could not send reset link."}finally{t.body.contains(b)&&(b.disabled=!1,b.textContent="Send Reset Link")}})}function p({ready:g=!1,initialError:d=""}={}){t.body.classList.remove("public-qr-mode"),r.innerHTML=e.passwordRecovery({ready:g,initialError:d}),t.querySelector("#auth-back-to-login").addEventListener("click",()=>{a(),e.renderAuth("login")}),t.querySelector("#auth-send-new-reset").addEventListener("click",()=>{a(),o()}),t.querySelector("#password-recovery-form").addEventListener("submit",async h=>{if(h.preventDefault(),!g)return;let y=h.target,b=y.querySelector("button[type='submit']"),k=new FormData(y),A=String(k.get("password")||""),$=String(k.get("confirmPassword")||""),O=t.querySelector("#auth-error"),C=t.querySelector("#auth-status");if(O.textContent="",A.length<8){O.textContent="Password must be at least 8 characters.";return}if(A!==$){O.textContent="Passwords do not match.";return}C.textContent="Updating password...",b.disabled=!0,b.textContent="Updating...";try{let{error:v}=await e.withOperationTimeout(e.supabaseClient.auth.updateUser({password:A}),"Password update timed out. Try the newest reset link again.",2e4);if(v){C.textContent="",O.textContent=v.message;return}a();let{data:S}=await e.supabaseClient.auth.getSession();if(e.setSession(S.session),C.textContent=S.session?"Password updated. Loading workspace...":"Password updated. Sign in with your new password.",S.session){await e.render();return}e.renderAuth("login","Password updated. Sign in with your new password.")}catch(v){C.textContent="",O.textContent=v.message||"Could not update password."}finally{t.body.contains(b)&&(b.disabled=!1,b.textContent="Update Password")}})}return{authCallbackRedirectUrl:s,passwordResetRedirectUrl:f,clearPasswordRecoveryUrl:a,startAuthCallback:i,renderAuthCallback:u,renderAuthCallbackError:m,startPasswordRecovery:c,renderPasswordResetRequest:o,renderPasswordRecovery:p}}window.MaintainOpsAuthSessionFlow={createAuthSessionFlow:l},typeof ut<"u"&&(ut.exports={createAuthSessionFlow:l})})()});var hn=Q((Rr,Oe)=>{(function(){function l(f,a){let i=a.getProfilesByUserId();if(f.type==="comment")return`
  ...
  `}function e(f,a){let i=a.getProcedureTemplates(),u=a.getPartsUsedByWorkOrder(),m=a.getCommentsByWorkOrder(),c=a.getPhotosByWorkOrder(),o=a.getMessageThreads(),p=i.find(A=>A.id===f.procedure_template_id),g=p?a.checklistProgress(f,p):null,d=(u[f.id]||[]).length,h=(m[f.id]||[]).length,y=(c[f.id]||[]).length,b=o.filter(A=>A.work_order_id===f.id).length,k=[];return f.asset_id&&k.push(n("asset","Equipment",f.assets?.name||"Linked",a)),p&&g&&k.push(n("procedure","Procedure checklist",`${g.done}/${g.total}`,a)),d&&k.push(n("parts","Parts",String(d),a)),h&&k.push(n("comment","Comments",String(h),a)),b&&k.push(n("message","Messages",String(b),a)),y&&k.push(t(f.id,String(y),a)),k.length?`<div class="relationship-row">${k.join("")}</div>`:""}function n(f,a,i,u){return`
    <span class="relationship-chip ${f}" title="${u.escapeHtml(a)}">
      ${r(f)}
  ...
  `}function r(f){return{asset:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7l8-4 8 4-8 4-8-4z"></path><path d="M4 7v10l8 4 8-4V7"></path><path d="M12 11v10"></path></svg>',procedure:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 6h11"></path><path d="M9 12h11"></path><path d="M9 18h11"></path><path d="M4 6l1 1 2-2"></path><path d="M4 12l1 1 2-2"></path><path d="M4 18l1 1 2-2"></path></svg>',parts:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 7l3 3"></path><path d="M5 19l8-8"></path><path d="M15 5l4 4-4 4-4-4 4-4z"></path></svg>',comment:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 5h14v10H8l-3 3V5z"></path></svg>',message:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5h16v11H7l-3 3V5z"></path><path d="M8 9h8"></path><path d="M8 13h5"></path></svg>',photo:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 6h16v12H4V6z"></path><path d="M8 14l3-3 2 2 2-3 3 4"></path><path d="M8 9h.01"></path></svg>'}[f]||""}function s(f){return Object.freeze({renderActivityItem:a=>l(a,f),renderRelationshipChips:a=>e(a,f),relationshipChip:(a,i,u)=>n(a,i,u,f),photoJumpChip:(a,i)=>t(a,i,f),relationshipIcon:r})}window.MaintainOpsRelationshipDisplay=Object.freeze({createRelationshipDisplayHelpers:s}),typeof Oe<"u"&&Oe.exports&&(Oe.exports={createRelationshipDisplayHelpers:s})})()});var yn=Q((Er,dt)=>{(function(){function l(e){let n=e.segmentIcon,t=e.escapeHtml,r=e.renderAssetOptions,s=e.renderMaintenanceRequestPhoto,f=e.isConvertedRequest,a=e.canDeleteOperationalRecords,i=e.canEditOperationalRecords||(()=>!0),u=e.getPendingDeleteRequestId,m=e.getProfilesByUserId;function c(d,h){return d==="converted"?`${h} converted`:d==="all"?`${h} total`:`${h} active`}function o(d,h,y={}){return`
      ...
      `}function p(d){let h=f(d),y=i(),b=u()===d.id,k=m(),A=d.created_at?new Date(d.created_at):null,$=A&&!Number.isNaN(A.getTime())?A.toLocaleString():"date unavailable",O=d.assets?.name||d.locations?.name||"No equipment",C=d.requested_by_name||k[d.requested_by]?.full_name||"Requester",v=d.converted_by||d.reviewed_by||"",S=k[v]?.full_name||"",_=S?`Converted to work order by ${S}`:v?"Converted to work order; converter name unavailable":"Converted to work order; converter not recorded",T=y&&a()?b?`
        <button class="secondary-button" data-cancel-delete-request type="button">Cancel</button>
        <button class="danger-action-button confirm-delete-button" data-confirm-delete-request="${t(d.id)}" type="button">Permanently Delete</button>
      `:`
        <button class="danger-action-button" data-delete-request="${t(d.id)}" type="button">Delete</button>
      `:"";return`
        <article class="request-card ${h?"converted-request":"active-request"}">
          <div class="request-card-main">
            <div class="request-card-header">
              <div class="chip-row">
                <span class="chip ${d.priority}">${t(d.priority)}</span>
                <span class="chip ${h?"completed":"open"}">${h?"converted":t(d.status)}</span>
              </div>
              <span class="request-source-pill">Public intake</span>
            </div>
            <h3>${t(d.title)}</h3>
            <p>${t(d.description||"No description.")}</p>
            ${s(d)}
            <div class="meta-row">
          ...
          ${y&&!h&&d.status==="submitted"?`
            <div class="request-actions">
              <button class="secondary-button request-action-button" data-quick-fix-request="${d.id}" type="button">Quick Fix</button>
              <button class="secondary-button work-action-button" data-convert-request="${d.id}" type="button">Convert to Work Order</button>
              ${T}
            </div>
          `:h?`
      ...
      `}return{requestPanelSubtitle:c,renderRequestFilterBar:o,renderMaintenanceRequest:p,renderRequestFormContent:g}}window.MaintainOpsRequestDisplay={createRequestDisplayHelpers:l},typeof dt<"u"&&(dt.exports={createRequestDisplayHelpers:l})})()});var bn=Q((Or,pt)=>{(function(){function l({statusLabel:e,workOrderTypeLabel:n=R=>String(R||"corrective").replace(/\b\w/g,E=>E.toUpperCase()),teamMemberName:t,getWorkOrderAssigneeFilter:r,getWorkOrderFilter:s,getWorkOrderTypeFilter:f=()=>"all",getWorkOrderPriorityFilter:a=()=>"all",getWorkSort:i=()=>"newest",getWorkGroup:u=()=>"none",getActiveStatusFilter:m,getMyWorkFilter:c,getActiveSection:o,getDueState:p,getProcedureTemplates:g,getActiveWorkOrderId:d,getProfilesByUserId:h,getSession:y,STATUS_OPTIONS:b,TYPE_OPTIONS:k=[],OUTSIDE_VENDOR_VALUE:A,escapeHtml:$,cleanWorkOrderDescription:O,relationshipIcon:C,segmentIcon:v,isVendorAssigned:S,assignmentLabel:_,renderRelationshipChips:T,canAssignWorkOrderToMe:N,canManageTeam:D,renderProductionActionCard:W=()=>"",hasOpenProductionAction:w=()=>!1,hasUnreadProductionReady:P=()=>!1}){function R(){let M=r(),j=s(),H=m(),J=M?`${t(M)} Work`:j==="unassigned"?"Unassigned Work Orders":j==="vendor"?"Outside Vendor Work":j==="assigned"?"Assigned Work Orders":"Work Orders";return H==="active"||H==="all"?J==="Work Orders"?"Active Work Orders":`Active - ${J}`:`${e(H)} - ${J}`}function E(){let M=m();return M==="active"||M==="all"?"My Work":`${e(M)} - My Work`}function q(){return o()==="mywork"?E():R()}function L(M){let j=o(),H=c();return j==="mywork"?`${M} shown - ${j==="mywork"?H==="created"?"Created By Me":"Assigned To Me":"shown"}`:`${M} shown`}function I(M,j,H){return`<option value="${$(M)}" ${M===H?"selected":""}>${$(j)}</option>`}function z(M){return{all:"Any assignment",assigned:"Team member",vendor:"Outside vendor",unassigned:"Unassigned"}[M]||"Any assignment"}function U(M){return M?M.charAt(0).toUpperCase()+M.slice(1):""}function V(M=[]){let j=m(),H=j==="all"?"active":j,J=s(),X=r(),K=f(),Y=a(),ne=i(),ee=u(),de=["completed","completed_month","completed_week"].includes(j),oe=H==="active"&&J==="all"&&!X&&K==="all"&&Y==="all"&&ne==="newest"&&ee==="none",F=M.find(te=>te.userId===X),pe=[`Status: ${e(H)}`,`Assignment: ${z(J)}`,...F?[`Person: ${F.name}`]:[],...K!=="all"?[`Type: ${n(K)}`]:[],...Y!=="all"?[`Priority: ${U(Y)}`]:[]],he=[["active","Active work"],["open","New"],["in_progress","In progress"],["blocked","Blocked"],["overdue","Overdue"],["completed","All completed"],["completed_month","Completed this month"],["completed_week","Completed this week"]],ue=[["all","Any assignment"],["assigned","Team member"],["vendor","Outside vendor"],["unassigned","Unassigned"]],x=[["newest","Recently created"],["due","Due date soonest"],["priority","Highest priority"],["type","Work type A-Z"],["assigned","Assigned person A-Z"]],ie=[["none","No grouping"],["assignee","Assigned person"],["status","Status"],["priority","Priority"],["type","Work type"]];return`
      ...
      `}function Z(M){let j=p(M),H=g().find(ee=>ee.id===M.procedure_template_id),J=M.created_at?new Date(M.created_at):null,X=J&&!Number.isNaN(J.getTime())?J.toLocaleDateString():"",K=M.status==="completed",Y=K?"Completed":e(M.status),ne=ee=>ee==="completed"?"Complete":e(ee);return`
        <article class="work-card status-card status-${M.status} ${M.id===d()?"selected":""}" data-id="${M.id}" tabindex="0">
    ...
    `}function ae(M){return`
      ...
      `).join("")}function f(c,o={}){let p=n(),g=o.compact?" compact":"";if(!p.length)return'<p class="warning-text production-action-empty">Add a Production user in Team before assigning a Production Action.</p>';let d=p.some(h=>h.userId===c.production_action_assigned_to)?c.production_action_assigned_to:p[0].userId;return`
      ...
      `}return{productionMembers:n,productionAssigneeName:t,renderProductionActionCard:u,renderProductionActionDetail:m}}window.MaintainOpsProductionActionDisplay={createProductionActionDisplayHelpers:l},typeof We<"u"&&We.exports&&(We.exports={createProductionActionDisplayHelpers:l})})()});var vn=Q((xr,mt)=>{(function(){function l(e={}){let n=e.getNotifications||(()=>[]),t=e.escapeHtml||(u=>String(u||"")),r=e.formatMessageTime||(u=>String(u||"")),s=Math.max(Number(e.visibleLimit)||12,1);function f(){return n().filter(u=>!u.read_at).length}function a(u){return n().some(m=>!m.read_at&&m.kind==="production_action_completed"&&m.work_order_id===u)}function i(){if(!e.getReady?.())return"";let u=n();if(!u.length)return"";let m=f(),c=u.slice(0,s);return`
        <details class="work-notification-panel" ${m?"open":""}>
          <summary>
            <span>Work notifications</span>
            <span>${m?`${m} new`:"Recent"}</span>
          </summary>
      ...
      `}function D(w){let P=Number(w.quantity_on_hand)||0,R=Number(w.reorder_point)||0,E=Number(w.unit_cost)||0,q=f()[w.id]||[],L=S(q),I=d();return`
      ...
      `}function i(m){let c={open:0,reviewing:1,resolved:2};return[...m].sort((o,p)=>{let g=(c[o.status||"open"]??1)-(c[p.status||"open"]??1);return g||new Date(p.created_at||0)-new Date(o.created_at||0)})}function u(){if(!e())return"";let m=s(),c=f(),o=i(c);return`
      ...
      `}return{renderAppIssueReportForm:a,renderAppIssueReportsPanel:u,sortedAppIssueReports:i}}window.MaintainOpsAppIssuePanelDisplay={createAppIssuePanelDisplayHelpers:l},typeof gt<"u"&&(gt.exports={createAppIssuePanelDisplayHelpers:l})})()});var qn=Q((Ir,ht)=>{(function(){function l(e){let n=e.escapeHtml,t=e.getDueState,r=e.procedureDeleteBlockerMessage,s=e.canDeleteOperationalRecords,f=e.canEditOperationalRecords||(()=>!0);function a(){return e.getPreventiveSchedules().filter(c=>e.matchesActiveLocation(c)&&e.matchesSearch([c.title,c.frequency,c.next_due_at,c.assets?.name]))}function i(){return e.getProcedureTemplates().filter(c=>e.matchesSearch([c.name,c.description,...(c.procedure_steps||[]).map(o=>o.prompt)]))}function u(c){let o=t({due_at:c.next_due_at,status:"open"}),p=e.getPendingDeleteScheduleId()===c.id,g=f();return`
      ...
      `}function m(c){let o=e.getWorkOrders().filter(y=>y.procedure_template_id===c.id).length,p=e.getPreventiveSchedules().filter(y=>y.procedure_template_id===c.id).length,g=r({workOrders:o,schedules:p}),d=e.getPendingDeleteProcedureId()===c.id,h=f();return`
      ...
      <div class="detail-stack">
        <div>
          ...
          ${q.asset_id&&d(q)?'<p class="completion-note">Safety devices identified before completion.</p>':""}
        ...
        <form class="form-grid" id="edit-work-order-form">
          ...
          <label>Work type
            <select name="type">
              ${le.map(F=>`<option value="${F}" ${F===p(q.type)?"selected":""}>${g(F)}</option>`).join("")}
            </select>
          </label>
          ${k(q)}
        ...
        ${oe&&q.status!=="completed"?`
          ...
          <form class="completion-box" id="complete-work-order-form">
            ...
            ${A(q)?`
              <label class="check-row safety-check-row">
                <input name="safety_devices_checked" type="checkbox" required ${d(q)?"checked":""}>
    ...
    `}return{renderWorkOrderDetail:P}}window.MaintainOpsWorkOrderDetailDisplay={createWorkOrderDetailDisplayHelpers:l},typeof yt<"u"&&(yt.exports={createWorkOrderDetailDisplayHelpers:l})})()});var $n=Q((Lr,bt)=>{(function(){function l(){function e(){return`
      ...
      `}return{renderMessageCenter:d}}window.MaintainOpsMessageCenterDisplay={createMessageCenterDisplayHelpers:l},typeof vt<"u"&&(vt.exports={createMessageCenterDisplayHelpers:l})})()});var Rn=Q((Qr,kt)=>{(function(){function l(e={}){let{STATUS_OPTIONS:n=[],TYPE_OPTIONS:t=[],renderAssetOptions:r,statusLabel:s,workOrderTypeLabel:f=o=>String(o||"corrective").replace(/\b\w/g,p=>p.toUpperCase()),renderAssignmentSelect:a,renderProcedureOptions:i,escapeHtml:u}=e;function m(){let o=new Date;return new Date(o.getTime()-o.getTimezoneOffset()*6e4).toISOString().slice(0,10)}function c(){let o=e.getParts();return`
            ...
            <div class="form-grid">
              <label>Part used
                <select name="part_id">
                  ...
                  ${o.map(p=>`<option value="${p.id}">${u(p.name)} (${p.quantity_on_hand} on hand)</option>`).join("")}
                </select>
              </label>
              <label>Quantity used<input name="quantity_used" type="number" min="1" step="1" value="1"></label>
      ...
      `}return{renderCreateWorkOrder:c}}window.MaintainOpsCreateWorkOrderDisplay={createCreateWorkOrderDisplayHelpers:l},typeof kt<"u"&&(kt.exports={createCreateWorkOrderDisplayHelpers:l})})()});var En=Q((Br,_t)=>{(function(){function l(e={}){let{TYPE_OPTIONS:n=[],renderAssetOptions:t,assetLocationRoutingMessage:r,escapeHtml:s,renderAssignmentSelect:f,renderProcedureOptions:a,assetStatusLabel:i,workOrderTypeLabel:u=o=>String(o||"corrective").replace(/\b\w/g,p=>p.toUpperCase())}=e;function m(){let o=new Date;return new Date(o.getTime()-o.getTimezoneOffset()*6e4).toISOString().slice(0,10)}function c(){let o=e.getQuickFixAssetId(),p=e.getQuickFixRequestId(),g=e.getMaintenanceRequests(),d=e.getSession(),h=e.getParts(),y=o||"",b=g.find(k=>k.id===p);return`
        <form class="form-grid quick-fix-form relationship-detail comment" id="quick-fix-form">
          ...
          <details class="quick-fix-more">
            ...
            <div class="form-grid">
              ...
              <label>Assign to
                <select name="assigned_to">
                  ${f(d.user.id,{selfLabel:"Assign to me"})}
                </select>
              </label>
              <label>Procedure checklist
                <select name="procedure_template_id">
      ...
      `}return{renderQuickFixForm:c}}window.MaintainOpsQuickFixDisplay={createQuickFixDisplayHelpers:l},typeof _t<"u"&&(_t.exports={createQuickFixDisplayHelpers:l})})()});var On=Q((jr,St)=>{(function(){function l(e={}){let n=e.escapeHtml;function t(c){return`
      ...
      `}function u(c={}){let o=!!c.ready,p=c.initialError||"";return`
        <section class="auth-shell">
      ...
      `}return{workspaceLoading:t,workspaceLoadError:r,authForm:s,authCallback:f,authCallbackError:a,passwordResetRequest:i,passwordRecovery:u,companyCreate:m}}window.MaintainOpsAuthDisplay={createAuthDisplayHelpers:l},typeof St<"u"&&(St.exports={createAuthDisplayHelpers:l})})()});var Wn=Q((zr,qt)=>{(function(){function l(e={}){let n=e.escapeHtml,t=e.qrSvgFor,r=e.getLocations||(()=>[]),s=e.getPublicRequestLinks||(()=>[]),f=e.getPublicRequestLinksReady||(()=>!0),a=e.getPublicAppUrlOverride||(()=>""),i=e.getWindowPublicAppUrl||(()=>""),u=e.canManageTeam||(()=>!1),m=e.canAdministerPublicRequestLinks||(()=>!1),c=e.publicAppBaseUrl,o=e.publicRequestUrl,p=e.publicRequestQrUrl;function g(){return`
      ...
      `}function A(){if(!u())return"";let O=c(),C=r(),v=f();return`
      ...
      `}function $(O){let C=s().find(D=>D.location_id===O.id),v=!!(C&&C.is_active!==!1),S=m(),_=v?o(C.token):"",T=v?p(C.token):"",N=!!(_&&T);return`
...
${c.dataset.quickReply}`:c.dataset.quickReply,p.focus(),typeof f=="function"&&f(p)})})}window.MaintainOpsWorkspaceMessageUiEvents={bindWorkspaceMessageUiEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.state,r=e.renderWorkspace,s=e.resetPartsPage;if(!t||typeof r!="function"||typeof s!="function")return;let f=n.querySelector("#part-search-form");if(!f)return;let a=u=>{t.setPartSearchQuery(u||""),s(),r()},i=f.querySelector("input[name='part_search']");i&&i.addEventListener("input",()=>{a(i.value||"");let u=n.querySelector("#part-search");if(!u)return;u.focus();let m=u.value.length;u.setSelectionRange(m,m)}),f.addEventListener("submit",u=>{u.preventDefault();let m=e.FormDataRef||FormData,c=new m(f).get("part_search")||"";a(c),n.querySelector("#parts-list")?.scrollIntoView({behavior:"smooth",block:"start"})})}window.MaintainOpsWorkspacePartSearchEvents={bindWorkspacePartSearchEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.state;if(!t||typeof e.renderWorkspace!="function")return;let r=e.storage||localStorage,s=typeof e.scrollToSectionTop=="function"?e.scrollToSectionTop:()=>{};n.querySelectorAll("[data-section]").forEach(f=>{f.addEventListener("click",async()=>{let a=performance.now(),i=f.dataset.section;e.visibleNavItems().some(([u])=>u===i)&&(t.setActiveSection(i),t.setActiveWorkOrderId(null),t.setActiveAssetId(null),t.setActivePartId(null),t.setShowPartSourceManager(!1),t.setCreateWorkOrderMode(!1),t.setQuickFixMode(!1),t.setReportIssueMode(!1),t.setQuickFixAssetId(null),t.setQuickFixRequestId(null),i!=="work"&&e.setWorkOrderSearchMode(!1),e.resetWorkOrderPage(),r.setItem("maintainops.activeSection",i),e.renderWorkspace(),s(),["work","mywork","messages"].includes(i)&&typeof e.loadWorkOrderNotifications=="function"&&await e.loadWorkOrderNotifications(),i==="messages"&&typeof e.loadMessageCenter=="function"&&(await e.loadMessageCenter(),e.renderWorkspace()),(i==="work"||i==="mywork")&&await e.reloadWorkOrderQueue(),i==="planning"&&typeof e.reloadPlanningWorkOrderQueue=="function"&&await e.reloadPlanningWorkOrderQueue(),i==="requests"&&await e.reloadRequestQueue(),i==="team"&&typeof e.reloadTeamWorkloads=="function"&&await e.reloadTeamWorkloads(),i==="setup"&&typeof e.loadSetupStorageDashboard=="function"&&(await e.loadSetupStorageDashboard(),e.renderWorkspace()),i==="manager"&&typeof e.loadManagerDashboardCompletedWork=="function"&&(await e.loadManagerDashboardCompletedWork(),e.renderWorkspace()),i==="performance"&&typeof e.loadPlatformPerformance=="function"&&await e.loadPlatformPerformance(),typeof e.onSectionNavigation=="function"&&e.onSectionNavigation(i,a))})})}window.MaintainOpsWorkspaceSectionNavigationEvents={bindWorkspaceSectionNavigationEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.state;if(!t||typeof e.renderWorkspace!="function"||typeof e.markMessageThreadRead!="function")return;let r=e.storage||localStorage;n.querySelectorAll("[data-message-thread]").forEach(s=>{s.addEventListener("click",async()=>{let f=s.dataset.messageThread;t.setActiveMessageThreadId(f),r.setItem("maintainops.activeMessageThreadId",f),typeof e.loadActiveMessageThreadMessages=="function"&&await e.loadActiveMessageThreadMessages(f),await e.markMessageThreadRead(f),e.renderWorkspace()})}),n.querySelectorAll("[data-open-work-message-thread]").forEach(s=>{s.addEventListener("click",async()=>{let f=s.dataset.openWorkMessageThread;t.setActiveMessageThreadId(f),t.setMessageComposerOpen(!1),t.setActiveSection("messages"),r.setItem("maintainops.activeMessageThreadId",f),r.setItem("maintainops.activeSection","messages"),typeof e.loadActiveMessageThreadMessages=="function"&&await e.loadActiveMessageThreadMessages(f),await e.markMessageThreadRead(f),e.renderWorkspace()})})}window.MaintainOpsWorkspaceMessageThreadEvents={bindWorkspaceMessageThreadEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.state;if(!t||typeof e.renderWorkspace!="function")return;let r=e.storage||localStorage;n.querySelectorAll("[data-cancel-app-issue-report]").forEach(s=>{s.addEventListener("click",()=>{t.setReportIssueMode(!1),e.renderWorkspace()})}),n.querySelectorAll("[data-setup-action]").forEach(s=>{s.addEventListener("click",()=>{s.dataset.setupAction==="confirm-admin-delete-sql"&&(t.setAdminDeleteSqlConfirmed(!0),r.setItem("maintainops.adminDeleteSqlConfirmed","true"),typeof e.showNotice=="function"&&e.showNotice("Admin delete SQL marked as applied."),e.renderWorkspace())})})}window.MaintainOpsWorkspaceIssueAdminUiEvents={bindWorkspaceIssueAdminUiEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.state;typeof e.requestDeletePart=="function"&&(n.querySelectorAll("[data-delete-part]:not(.permanent-delete-button)").forEach(r=>{r.addEventListener("click",()=>{e.requestDeletePart(r.dataset.deletePart)})}),n.querySelectorAll("[data-delete-part].permanent-delete-button").forEach(r=>{r.addEventListener("click",()=>{e.requestDeletePart(r.dataset.deletePart)})})),!(!t||typeof e.renderWorkspace!="function")&&n.querySelectorAll("[data-cancel-delete-part]").forEach(r=>{r.addEventListener("click",()=>{t.setPendingDeletePartId(null),e.renderWorkspace()})})}window.MaintainOpsWorkspacePartDeleteCancelEvents={bindWorkspacePartDeleteCancelEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.state;if(!t||typeof e.renderWorkspace!="function")return;let r=e.storage||localStorage;n.querySelectorAll("[data-start-work-message]").forEach(s=>{s.addEventListener("click",()=>{let f=s.dataset.startWorkMessage;t.setMessageComposerWorkOrderId(f),t.setMessageComposerOpen(!0),t.setActiveMessageThreadId(""),t.setActiveSection("messages"),r.setItem("maintainops.messageComposerWorkOrderId",f),r.setItem("maintainops.activeSection","messages"),r.setItem("maintainops.activeMessageThreadId",""),e.renderWorkspace()})})}window.MaintainOpsWorkspaceWorkMessageStartEvents={bindWorkspaceWorkMessageStartEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.state;!t||typeof e.renderWorkspace!="function"||n.querySelectorAll('[data-command-action="report-issue"]').forEach(r=>{r.addEventListener("click",()=>{t.setActiveWorkOrderId(null),t.setActiveAssetId(null),t.setActivePartId(null),t.setCreateWorkOrderMode(!1),t.setQuickFixMode(!1),t.setReportIssueMode(!0),e.renderWorkspace()})})}window.MaintainOpsWorkspaceReportIssueCommandEvents={bindWorkspaceReportIssueCommandEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.state;if(!t||typeof e.reloadRequestQueue!="function"||typeof e.resetRequestsPage!="function")return;let r=e.storage||localStorage;n.querySelectorAll('[data-command-action="request"]').forEach(s=>{s.addEventListener("click",async()=>{t.setActiveWorkOrderId(null),t.setActiveAssetId(null),t.setCreateWorkOrderMode(!1),t.setQuickFixMode(!1),t.setReportIssueMode(!1),t.setQuickFixAssetId(null),t.setQuickFixRequestId(null),t.setActiveSection("requests"),e.setWorkOrderSearchMode(!1),r.setItem("maintainops.activeSection","requests"),e.resetRequestsPage(),await e.reloadRequestQueue()})})}window.MaintainOpsWorkspaceSubmitRequestCommandEvents={bindWorkspaceSubmitRequestCommandEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.state;if(!t||typeof e.renderWorkspace!="function")return;let r=e.storage||localStorage;n.querySelectorAll('[data-command-action="create-work-order"]').forEach(s=>{s.addEventListener("click",()=>{t.setActiveWorkOrderId(null),t.setActiveAssetId(null),t.setCreateWorkOrderMode(!0),t.setQuickFixMode(!1),t.setReportIssueMode(!1),t.setQuickFixAssetId(null),t.setQuickFixRequestId(null),t.setActiveSection("work"),e.setWorkOrderSearchMode(!1),r.setItem("maintainops.activeSection","work"),e.renderWorkspace()})})}window.MaintainOpsWorkspaceNewWorkOrderCommandEvents={bindWorkspaceNewWorkOrderCommandEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document;typeof e.exportActiveSectionCsv=="function"&&n.querySelectorAll('[data-command-action="export-csv"]').forEach(t=>{t.addEventListener("click",()=>{e.exportActiveSectionCsv()})})}window.MaintainOpsWorkspaceExportCsvCommandEvents={bindWorkspaceExportCsvCommandEvents:l}})();var Ma=B(xt());(function(){function l(e={}){let n=e.documentRef||document,t=e.state;typeof e.requestDeleteAsset=="function"&&n.querySelectorAll("[data-delete-asset]").forEach(r=>{r.addEventListener("click",async s=>{s&&typeof s.stopPropagation=="function"&&s.stopPropagation(),await e.requestDeleteAsset(r.dataset.deleteAsset)})}),!(!t||typeof e.renderWorkspace!="function")&&(n.querySelectorAll("[data-cancel-delete-asset]").forEach(r=>{r.addEventListener("click",s=>{s&&typeof s.stopPropagation=="function"&&s.stopPropagation(),t.setPendingDeleteAssetId(null),e.renderWorkspace()})}),typeof e.deleteAsset=="function"&&n.querySelectorAll("[data-confirm-delete-asset]").forEach(r=>{r.addEventListener("click",async s=>{s&&typeof s.stopPropagation=="function"&&s.stopPropagation(),await e.deleteAsset(r.dataset.confirmDeleteAsset)})}))}window.MaintainOpsWorkspaceAssetDeleteCancelEvents={bindWorkspaceAssetDeleteCancelEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.state;typeof e.requestDeleteMaintenanceRequest=="function"&&n.querySelectorAll("[data-delete-request]").forEach(r=>{r.addEventListener("click",()=>{e.requestDeleteMaintenanceRequest(r.dataset.deleteRequest)})}),!(!t||typeof e.renderWorkspace!="function")&&(n.querySelectorAll("[data-cancel-delete-request]").forEach(r=>{r.addEventListener("click",()=>{t.setPendingDeleteRequestId(null),e.renderWorkspace()})}),typeof e.deleteMaintenanceRequest=="function"&&n.querySelectorAll("[data-confirm-delete-request]").forEach(r=>{r.addEventListener("click",()=>{e.deleteMaintenanceRequest(r.dataset.confirmDeleteRequest)})}))}window.MaintainOpsWorkspaceRequestDeleteCancelEvents={bindWorkspaceRequestDeleteCancelEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.state;typeof e.requestDeletePreventiveSchedule=="function"&&n.querySelectorAll("[data-delete-schedule]").forEach(r=>{r.addEventListener("click",()=>{e.requestDeletePreventiveSchedule(r.dataset.deleteSchedule)})}),!(!t||typeof e.renderWorkspace!="function")&&(n.querySelectorAll("[data-cancel-delete-schedule]").forEach(r=>{r.addEventListener("click",()=>{t.setPendingDeleteScheduleId(null),e.renderWorkspace()})}),typeof e.deletePreventiveSchedule=="function"&&n.querySelectorAll("[data-confirm-delete-schedule]").forEach(r=>{r.addEventListener("click",()=>{e.deletePreventiveSchedule(r.dataset.confirmDeleteSchedule)})}))}window.MaintainOpsWorkspaceScheduleDeleteCancelEvents={bindWorkspaceScheduleDeleteCancelEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.state;typeof e.requestDeleteProcedureTemplate=="function"&&n.querySelectorAll("[data-delete-procedure]").forEach(r=>{r.addEventListener("click",async()=>{await e.requestDeleteProcedureTemplate(r.dataset.deleteProcedure)})}),!(!t||typeof e.renderWorkspace!="function")&&(n.querySelectorAll("[data-cancel-delete-procedure]").forEach(r=>{r.addEventListener("click",()=>{t.setPendingDeleteProcedureId(null),e.renderWorkspace()})}),typeof e.deleteProcedureTemplate=="function"&&n.querySelectorAll("[data-confirm-delete-procedure]").forEach(r=>{r.addEventListener("click",async()=>{await e.deleteProcedureTemplate(r.dataset.confirmDeleteProcedure)})}))}window.MaintainOpsWorkspaceProcedureDeleteCancelEvents={bindWorkspaceProcedureDeleteCancelEvents:l}})();(function(){function l(n){!n||!n.style||(n.style.height="auto",n.style.height=`${n.scrollHeight}px`)}function e(n={}){(n.documentRef||document).querySelectorAll("textarea").forEach(r=>{l(r),r.addEventListener("input",()=>l(r))})}window.MaintainOpsWorkspaceTextareaAutoGrow={autoGrowTextarea:l,bindWorkspaceTextareaAutoGrow:e}})();var Na=B(Mt());(function(){function l(e={}){let n=e.documentRef||document,t=e.state;!t||typeof e.renderWorkspace!="function"||(n.querySelectorAll("[data-cancel-invite]").forEach(r=>{r.addEventListener("click",()=>{t.setTeamInviteCancelError(""),t.setPendingCancelInviteId(r.dataset.cancelInvite),e.renderWorkspace()})}),n.querySelectorAll("[data-cancel-invite-cancel]").forEach(r=>{r.addEventListener("click",()=>{t.setTeamInviteCancelError(""),t.setPendingCancelInviteId(null),e.renderWorkspace()})}),typeof e.cancelTeamInvite=="function"&&n.querySelectorAll("[data-confirm-cancel-invite]").forEach(r=>{r.addEventListener("click",()=>{e.cancelTeamInvite(r.dataset.confirmCancelInvite)})}))}window.MaintainOpsWorkspaceTeamInviteCancelEvents={bindWorkspaceTeamInviteCancelEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.copyTextToClipboard,r=e.setTimeoutRef||setTimeout,s=Number.isFinite(e.resetDelayMs)?e.resetDelayMs:1600;typeof t=="function"&&n.querySelectorAll("[data-copy-team-invite]").forEach(f=>{f.addEventListener("click",async()=>{let a=await t(f.dataset.copyTeamInvite||"");f.textContent=a?"Copied":"Copy failed",r(()=>{f.textContent="Copy Invite"},s)})})}window.MaintainOpsWorkspaceTeamInviteCopyEvents={bindWorkspaceTeamInviteCopyEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.state,r=typeof e.scrollToQuickFixForm=="function"?e.scrollToQuickFixForm:()=>{};if(!t||typeof e.renderWorkspace!="function")return;let s=e.storage||localStorage;n.querySelectorAll('[data-command-action="quick-fix"]').forEach(f=>{f.addEventListener("click",()=>{t.setActiveWorkOrderId(null),t.setActiveAssetId(null),t.setCreateWorkOrderMode(!1),t.setQuickFixMode(!0),t.setReportIssueMode(!1),t.setQuickFixAssetId(null),t.setQuickFixRequestId(null),t.setActiveSection("mywork"),e.setWorkOrderSearchMode(!1),s.setItem("maintainops.activeSection","mywork"),e.renderWorkspace(),r()})})}window.MaintainOpsWorkspaceQuickFixCommandEvents={bindWorkspaceQuickFixCommandEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.state,r=typeof e.scrollToQuickFixForm=="function"?e.scrollToQuickFixForm:()=>{};if(!t||typeof e.renderWorkspace!="function")return;let s=e.storage||localStorage;n.querySelectorAll("[data-quick-fix-asset]").forEach(f=>{f.addEventListener("click",()=>{t.setQuickFixAssetId(f.dataset.quickFixAsset),t.setQuickFixRequestId(null),t.setActiveAssetId(null),t.setActiveWorkOrderId(null),t.setCreateWorkOrderMode(!1),t.setQuickFixMode(!0),t.setActiveSection("mywork"),s.setItem("maintainops.activeSection","mywork"),e.renderWorkspace(),r()})})}window.MaintainOpsWorkspaceAssetQuickFixEvents={bindWorkspaceAssetQuickFixEvents:l}})();var za=B(Tt());(function(){function l(e={}){let n=e.documentRef||document,t=e.copyTextToClipboard,r=e.setTimeoutRef||setTimeout,s=Number.isFinite(e.resetDelayMs)?e.resetDelayMs:1600;typeof t=="function"&&n.querySelectorAll("[data-copy-public-request-link]").forEach(f=>{f.addEventListener("click",async()=>{let a=await t(f.dataset.copyPublicRequestLink);f.textContent=a?"Copied":"Copy failed",r(()=>{f.textContent="Copy QR Link"},s)})})}window.MaintainOpsWorkspacePublicRequestLinkCopyEvents={bindWorkspacePublicRequestLinkCopyEvents:l}})();var Ha=B(Dt());(function(){function l(e={}){let n=e.documentRef||document,t=e.convertRequestToWorkOrder;typeof t=="function"&&n.querySelectorAll("[data-convert-request]").forEach(r=>{r.addEventListener("click",()=>{t(r.dataset.convertRequest)})})}window.MaintainOpsWorkspaceRequestConversionEvents={bindWorkspaceRequestConversionEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.generatePreventiveWorkOrder;typeof t=="function"&&n.querySelectorAll("[data-generate-pm]").forEach(r=>{r.addEventListener("click",()=>{t(r.dataset.generatePm)})})}window.MaintainOpsWorkspacePmGenerationEvents={bindWorkspacePmGenerationEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.createFollowUpWorkOrder;typeof t=="function"&&n.querySelectorAll("[data-create-follow-up]").forEach(r=>{r.addEventListener("click",()=>{let f=r.closest?.("[data-follow-up-create]")?.querySelector?.("[name='follow_up_days']");t(r.dataset.createFollowUp,f?.value)})})}window.MaintainOpsWorkspaceFollowUpWorkEvents={bindWorkspaceFollowUpWorkEvents:l}})();var Ja=B(It());(function(){function l(e={}){let n=e.documentRef||document,t=e.createComment,r=n.querySelector("#comment-form");!r||typeof t!="function"||r.addEventListener("submit",t)}window.MaintainOpsWorkspaceCommentEvents={bindWorkspaceCommentEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.updateWorkOrderQuickView,r=n.querySelector("#quick-update-work-order-form");!r||typeof t!="function"||r.addEventListener("submit",t)}window.MaintainOpsWorkspaceQuickUpdateEvents={bindWorkspaceQuickUpdateEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.updateWorkOrderDetails,r=n.querySelector("#edit-work-order-form");!r||typeof t!="function"||r.addEventListener("submit",t)}window.MaintainOpsWorkspaceWorkOrderEditEvents={bindWorkspaceWorkOrderEditEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.openQuickFixForRequest;typeof t=="function"&&n.querySelectorAll("[data-quick-fix-request]").forEach(r=>{r.addEventListener("click",()=>t(r.dataset.quickFixRequest))})}window.MaintainOpsWorkspaceRequestQuickFixEvents={bindWorkspaceRequestQuickFixEvents:l}})();(function(){function l(e={}){let n=e.documentRef||document,t=e.updateAssetLocationWarning;typeof t=="function"&&n.querySelectorAll("[data-location-sensitive-asset]").forEach(r=>{t(r),r.addEventListener("change",()=>t(r))})}window.MaintainOpsWorkspaceAssetLocationWarningEvents={bindWorkspaceAssetLocationWarningEvents:l}})();var ro=B(Ft()),ao=B(Lt()),oo=B(Nt()),io=B(Ut()),so=B(Qt()),co=B(Bt()),lo=B(jt()),uo=B(zt()),po=B(Gt()),mo=B(Ht()),fo=B(Vt()),go=B(Yt()),ho=B(Kt()),yo=B(Jt()),bo=B(Zt()),wo=B(Xt()),vo=B(en()),ko=B(tn()),_o=B(nn()),So=B(rn()),qo=B(an()),Co=B(on()),$o=B(sn()),Po=B(cn()),Ao=B(ln()),Ro=B(un());(function(){function l(e){function n(r){return e[r]()}function t(r,s=n("requestViewFilter")){let f=r.eq("company_id",n("activeCompanyId"));n("locationsReady")&&n("activeLocationId")&&(f=f.eq("location_id",n("activeLocationId"))),s==="converted"?f=f.or("status.eq.converted,converted_work_order_id.not.is.null"):s!=="all"&&(f=f.eq("status","submitted").is("converted_work_order_id",null));let a=e.postgrestSearchTerm(n("searchQuery"));if(a){let i=`%${a}%`,u=n("assets").filter(e.matchesActiveLocation).filter(m=>e.matchesQuery([m.name,m.asset_code,m.manufacturer,m.model,m.location,m.status,m.asset_type,e.parentAssetFor()(m)?.name],a)).map(m=>m.id).slice(0,e.SEARCH_ID_PAGE_SIZE);f=f.or([`title.ilike.${i}`,`description.ilike.${i}`,`status.ilike.${i}`,`priority.ilike.${i}`,`requested_by_name.ilike.${i}`,`requested_by_contact.ilike.${i}`,...u.length?[`asset_id.in.(${u.join(",")})`]:[]].join(","))}return f}return{applyRequestQueryFilters:t}}window.MaintainOpsRequestQueryFilters={createRequestQueryFilterHelpers:l}})();(function(){function l(e){function n(p){return e[p]()}async function t(){let p=n("searchQuery").trim();if(!p||n("workOrderSearchMode")){e.setWorkOrderRelatedSearch({assetIds:[],workOrderIds:[],procedureIds:[]});return}let g=n("assets").filter(e.matchesActiveLocation).filter(b=>e.matchesQuery([b.name,b.asset_code,b.manufacturer,b.model,b.location,b.status,b.asset_type,e.parentAssetFor()(b)?.name],p)).map(b=>b.id),d=n("procedureTemplates").filter(b=>e.matchesQuery([b.name,b.description,...(b.procedure_steps||[]).map(k=>k.prompt)],p)).map(b=>b.id),h=n("parts").filter(e.matchesActiveLocation).filter(b=>e.matchesQuery([b.name,b.sku,b.supplier_name,b.quantity_on_hand,b.reorder_point,b.unit_cost],p)).map(b=>b.id),y=new Set;await Promise.all([r(y,h),s(y,"work_order_comments",["body"],p),s(y,"work_order_events",["event_type","summary"],p),s(y,"work_order_photos",["file_name"],p),s(y,"work_order_step_results",["value"],p)]),e.setWorkOrderRelatedSearch({assetIds:g.slice(0,200),procedureIds:d.slice(0,200),workOrderIds:[...y].slice(0,300)})}async function r(p,g,d={}){if(!g.length)return;let y=d.maxRows??300;for(let b of e.chunkArray(g,e.SEARCH_ID_CHUNK_SIZE)){if(y<=0)break;try{await e.fetchPagedSearchRows(()=>n("supabaseClient").from("work_order_parts").select("work_order_id").eq("company_id",n("activeCompanyId")).in("part_id",b),k=>{k.forEach(A=>{A.work_order_id&&p.add(A.work_order_id)}),y-=k.length},y)}catch(k){e.warn("Part-linked work order search failed",k);return}}}async function s(p,g,d,h,y={}){let b=e.postgrestSearchTerm(h);if(!b)return;let k=d.map($=>`${$}.ilike.%${b}%`).join(","),A=y.maxRows??300;try{await e.fetchPagedSearchRows(()=>n("supabaseClient").from(g).select("work_order_id").eq("company_id",n("activeCompanyId")).or(k),$=>{$.forEach(O=>{O.work_order_id&&p.add(O.work_order_id)})},A)}catch($){e.warn(`${g} work order search failed`,$)}}async function f(p={}){let g=await a(),d=g.length,h=Math.max(1,Math.ceil(d/e.WORK_ORDERS_PER_PAGE));n("workOrderPage")>h&&e.setWorkOrderPage(h),n("workOrderPage")<1&&e.setWorkOrderPage(1);let y=(n("workOrderPage")-1)*e.WORK_ORDERS_PER_PAGE,b=g.slice(y,y+e.WORK_ORDERS_PER_PAGE).map(O=>O.id);if(!b.length)return{data:[],error:null,count:d};let k=p.includeLocationRelation===!1?e.WORK_ORDER_FALLBACK_SELECT():e.WORK_ORDER_RELATION_SELECT(),A=await e.fetchWorkOrdersByIds(n("supabaseClient"),{companyId:n("activeCompanyId"),locationId:n("activeLocationId"),locationsReady:n("locationsReady"),selectClause:k,ids:b});if(A.error)return A;let $=new Map((A.data||[]).map(O=>[O.id,O]));return{...A,data:b.map(O=>$.get(O)).filter(Boolean),count:d}}async function a(){let p=[n("activeCompanyId")||"",n("locationsReady")?n("activeLocationId")||"":"all-locations",n("workSort"),n("searchQuery").trim().toLowerCase()].join("|"),g=n("exactWorkOrderSearchCache");if(g.key===p)return g.rows;let d=n("searchQuery").trim(),h=new Map;await i(h,d);let y=n("assets").filter(e.matchesActiveLocation).filter(O=>e.matchesQuery([O.name,O.asset_code,O.manufacturer,O.model,O.location,O.status,O.asset_type,e.parentAssetFor()(O)?.name],d)).map(O=>O.id),b=n("procedureTemplates").filter(O=>e.matchesQuery([O.name,O.description,...(O.procedure_steps||[]).map(C=>C.prompt)],d)).map(O=>O.id),k=n("parts").filter(e.matchesActiveLocation).filter(O=>e.matchesQuery([O.name,O.sku,O.supplier_name,O.quantity_on_hand,O.reorder_point,O.unit_cost],d)).map(O=>O.id);await Promise.all([u(h,"asset_id",y),u(h,"procedure_template_id",b)]);let A=new Set;await Promise.all([r(A,k,{maxRows:1/0}),s(A,"work_order_comments",["body"],d,{maxRows:1/0}),s(A,"work_order_events",["event_type","summary"],d,{maxRows:1/0}),s(A,"work_order_photos",["file_name"],d,{maxRows:1/0}),s(A,"work_order_step_results",["value"],d,{maxRows:1/0})]),await m(h,[...A]);let $=[...h.values()].sort(e.compareWorkOrders);return e.setExactWorkOrderSearchCache({key:p,rows:$}),$}async function i(p,g){let d=e.postgrestSearchTerm(g);if(!d)return;let h=["title","description","priority","type","status","failure_cause","resolution_summary","completion_notes"].map(y=>`${y}.ilike.%${d}%`).join(",");await e.fetchPagedSearchRows(()=>c().or(h),y=>o(p,y))}async function u(p,g,d){if(d.length)for(let h of e.chunkArray(d,e.SEARCH_ID_CHUNK_SIZE))await e.fetchPagedSearchRows(()=>c().in(g,h),y=>o(p,y))}async function m(p,g){if(g.length)for(let d of e.chunkArray(g,e.SEARCH_ID_CHUNK_SIZE))await e.fetchPagedSearchRows(()=>c().in("id",d),h=>o(p,h))}function c(){return e.buildScopedWorkOrderSearchQuery(n("supabaseClient"),{companyId:n("activeCompanyId"),locationId:n("activeLocationId"),locationsReady:n("locationsReady")})}function o(p,g){(g||[]).forEach(d=>{d?.id&&p.set(d.id,{...p.get(d.id)||{},...d})})}return{refreshWorkOrderRelatedSearch:t,fetchExactSearchedWorkOrderPage:f,exactWorkOrderSearchRows:a,addRelatedWorkOrderIdsFromParts:r,addRelatedWorkOrderIdsFromTable:s}}window.MaintainOpsWorkOrderSearch={createWorkOrderSearchHelpers:l}})();(function(){function l(e){function n(a){return e[a]()}function t(){let a=n("searchQuery").trim(),i=n("workOrders").filter(e.matchesActiveLocation).sort(e.compareWorkOrders).slice(0,e.SEARCH_PREVIEW_LIMIT),u=n("assets").filter(e.matchesActiveLocation).filter(g=>e.matchesQuery([g.name,g.asset_code,g.manufacturer,g.model,g.location,g.status],a)).sort((g,d)=>g.name.localeCompare(d.name)).slice(0,e.SEARCH_PREVIEW_LIMIT),m=n("parts").filter(e.matchesActiveLocation).filter(g=>e.matchesQuery([g.name,g.sku,g.supplier_name,g.quantity_on_hand,g.reorder_point],a)).sort((g,d)=>g.name.localeCompare(d.name)).slice(0,e.SEARCH_PREVIEW_LIMIT),c=n("maintenanceRequests").filter(e.matchesActiveLocation).filter(g=>e.matchesQuery([g.title,g.description,g.status,g.priority,g.assets?.name,n("profilesByUserId")[g.requested_by]?.full_name],a)).sort((g,d)=>new Date(d.created_at)-new Date(g.created_at)).slice(0,e.SEARCH_PREVIEW_LIMIT),o=n("preventiveSchedules").filter(e.matchesActiveLocation).filter(g=>e.matchesQuery([g.title,g.frequency,g.next_due_at,g.assets?.name],a)).sort((g,d)=>String(g.next_due_at||"").localeCompare(String(d.next_due_at||""))).slice(0,e.SEARCH_PREVIEW_LIMIT),p=n("procedureTemplates").filter(g=>e.matchesQuery([g.name,g.description,...(g.procedure_steps||[]).map(d=>d.prompt)],a)).sort((g,d)=>g.name.localeCompare(d.name)).slice(0,e.SEARCH_PREVIEW_LIMIT);return{work:i,assets:u,parts:m,requests:c,pm:o,procedures:p}}function r(a="all"){let i=e.startOfToday(),u=new Date(i);return u.setDate(u.getDate()+7),n("planningWorkOrders").filter(e.matchesActiveLocation).filter(m=>m.status!=="completed").filter(m=>e.matchesSearch([m.title,m.description,m.priority,m.status,m.assets?.name,e.assignmentLabel(m)])).filter(m=>a==="no_due"?!m.due_at:!!m.due_at).map(m=>{let c=m.due_at?new Date(`${m.due_at}T00:00:00`):null;return{kind:a==="no_due"?"no_due":"work",id:m.id,title:m.title,priority:m.priority,status:m.status,assetName:m.assets?.name||"No equipment",dueAt:m.due_at,due:c,createdAt:m.created_at||"",assignedTo:e.assignmentLabel(m),workOrder:m}}).filter(m=>a==="no_due"?!0:a==="overdue"?m.due<i:a==="today"?m.due.getTime()===i.getTime():a==="soon"?m.due>i&&m.due<=u:!0).sort((m,c)=>{if(a==="no_due"){let o={critical:4,high:3,medium:2,low:1};return(o[c.priority]||0)-(o[m.priority]||0)||new Date(m.createdAt||0)-new Date(c.createdAt||0)}return m.due-c.due})}function s(){let a=e.startOfToday(),i=new Date(a);return i.setDate(i.getDate()+7),n("preventiveSchedules").filter(e.matchesActiveLocation).filter(u=>{let m=new Date(`${u.next_due_at}T00:00:00`);return m>=a&&m<=i}).filter(u=>e.matchesSearch([u.title,u.frequency,u.next_due_at,u.assets?.name])).map(u=>({kind:"pm",id:u.id,title:u.title,assetName:u.assets?.name||"No equipment",dueAt:u.next_due_at,due:new Date(`${u.next_due_at}T00:00:00`)})).sort((u,m)=>u.due-m.due)}function f(){return n("planningWorkOrders").filter(e.matchesActiveLocation).filter(a=>a.follow_up_needed).filter(a=>e.matchesSearch([a.title,a.description,a.failure_cause,a.resolution_summary,a.assets?.name,a.assigned_profile?.full_name])).map(a=>({kind:"follow_up",id:a.id,title:a.title,assetName:a.assets?.name||"No equipment",completedAt:a.completed_at?new Date(a.completed_at).toLocaleDateString():"not completed",resolution:a.resolution_summary||a.completion_notes||"",workOrder:a})).sort((a,i)=>a.title.localeCompare(i.title))}return{globalSearchResults:t,planningItems:r,planningPmItems:s,followUpItems:f}}window.MaintainOpsWorkspaceListBuilders={createWorkspaceListBuilders:l}})();(function(){function l(n,t){return n.from("locations").select("*").eq("company_id",t).order("name")}function e(n,t,r){return n.from("locations").insert({company_id:t,name:r}).select("id").single()}window.MaintainOpsLocationsService={listLocations:l,createLocation:e}})();(function(){function l(f,a){return f.from("profiles").select("user_id, full_name, mobile_tech").eq("company_id",a)}function e(f,a){return f.from("company_members").select("*").eq("company_id",a).order("created_at",{ascending:!0})}function n(f,a){return f.from("company_invites").select("id, email, role, invited_by, accepted_at, created_at, default_location_id").eq("company_id",a).order("created_at",{ascending:!1})}function t(f,a){return f.from("company_invites").select("id, email, role, invited_by, accepted_at, created_at").eq("company_id",a).order("created_at",{ascending:!1})}function r(f,a){return f.from("company_invite_links").select("id, token, role, default_location_id, created_by, created_at, expires_at, used_at, used_by, revoked_at").eq("company_id",a).order("created_at",{ascending:!1})}function s(f,a){return f.from("request_notification_recipients").select("id, company_id, location_id, email, label, is_active, created_at").eq("company_id",a).order("created_at",{ascending:!1})}window.MaintainOpsProfilesService={listProfiles:l,listCompanyMembers:e,listTeamInvites:n,listTeamInvitesLegacy:t,listTeamInviteLinks:r,listRequestNotificationRecipients:s}})();(function(){function l(e,n){return e.from("parts").select("*").eq("company_id",n).order("name")}window.MaintainOpsPartsService={listParts:l}})();(function(){function l(n,t){return n.from("assets").select("*").eq("company_id",t).order("name")}function e(n,t){return n.from("asset_financials").select("*").eq("company_id",t).order("updated_at",{ascending:!1})}window.MaintainOpsAssetsService={listAssets:l,listAssetFinancials:e}})();(function(){function l(i,u,m={}){return i.from("work_orders").select(u,m)}function e(i){return i.from("work_orders").select("id",{count:"exact",head:!0})}function n(i,u,m,c){return i.from("work_orders").select(c).eq("company_id",u).eq("id",m).maybeSingle()}function t(i,u,m,c){return i.from("work_orders").select(c).eq("company_id",u).eq("asset_id",m).order("completed_at",{ascending:!1,nullsFirst:!1}).order("created_at",{ascending:!1})}async function r(i,u){let{companyId:m,locationId:c,locationsReady:o,selectClause:p,ids:g}=u,d=i.from("work_orders").select(p).eq("company_id",m).in("id",g);return o&&c&&(d=d.eq("location_id",c)),d}function s(i,u){let{companyId:m,locationId:c,locationsReady:o}=u,p=i.from("work_orders").select("id, created_at, due_at, completed_at, priority, status").eq("company_id",m);return o&&c&&(p=p.eq("location_id",c)),p}function f(i,u){let{companyId:m,locationId:c,locationsReady:o}=u,p=i.from("work_orders").select("id, assigned_to, production_action_assigned_to, production_action_status, status, due_at, location_id").eq("company_id",m).in("status",["open","in_progress","blocked","completed"]).or("assigned_to.not.is.null,and(production_action_assigned_to.not.is.null,production_action_status.eq.open)");return o&&c&&(p=p.eq("location_id",c)),p.order("id",{ascending:!0})}async function a(i,u,m=1/0,c=1e3){let o=0,p=0;for(;p<m;){let g=Math.min(c,m-p),{data:d,error:h}=await i().range(o,o+g-1);if(h)throw h;let y=d||[];if(u(y),p+=y.length,y.length<g)break;o+=g}}window.MaintainOpsWorkOrdersService={selectWorkOrders:l,countWorkOrdersQuery:e,fetchWorkOrderById:n,fetchWorkOrdersByAsset:t,fetchWorkOrdersByIds:r,scopedWorkOrderSearchQuery:s,scopedTeamWorkloadQuery:f,fetchPagedSearchRows:a}})();var Fo=B(dn());(function(){function l(s){return s.rpc("get_my_companies")}function e(s,f){return s.from("company_members").select("company_id, role, default_location_id").eq("user_id",f).order("created_at",{ascending:!0})}function n(s,f){return s.from("company_members").select("company_id, role").eq("user_id",f).order("created_at",{ascending:!0})}function t(s,f){return s.from("companies").select("id, name, logo_path, created_at").in("id",f).order("created_at",{ascending:!0})}function r(s,f){return s.from("companies").select("id, name, created_at").in("id",f).order("created_at",{ascending:!0})}window.MaintainOpsCompanyService={getMyCompanies:l,listUserCompanyMemberships:e,listUserCompanyMembershipsLegacy:n,listCompaniesByIds:t,listCompaniesByIdsLegacy:r}})();(function(){function l(r,s){return r.from("app_issue_reports").select("*").eq("company_id",s).order("created_at",{ascending:!1})}function e(r,s){return r.from("app_issue_reports").insert(s)}function n(r,s,f,a){return r.from("app_issue_reports").update({status:a,resolved_at:a==="resolved"?new Date().toISOString():null}).eq("company_id",s).eq("id",f)}function t(r,s,f){return r.from("app_issue_reports").delete().eq("company_id",s).eq("id",f)}window.MaintainOpsAppIssueReportsService={listAppIssueReports:l,createAppIssueReportRecord:e,updateAppIssueReportStatusRecord:n,deleteAppIssueReportRecord:t}})();(function(){let l="user_id, shop_reference_favorites, updated_at";function e(t,r){return t.from("user_preferences").select(l).eq("user_id",r).maybeSingle()}function n(t,r,s){return t.from("user_preferences").upsert({user_id:r,shop_reference_favorites:Array.isArray(s)?s.filter(Boolean):[],updated_at:new Date().toISOString()},{onConflict:"user_id"}).select(l).single()}window.MaintainOpsUserPreferencesService={getUserPreferences:e,saveShopReferenceFavorites:n}})();var Qo=B(pn()),Bo=B(mn()),jo=B(fn()),zo=B(gn());(function(){function l(t,r,s="neutral"){return`<article class="metric dashboard-card tone-${s}"><span>${t}</span><strong>${r}</strong></article>`}function e(t,r,s,f="neutral"){return`
  ...
  `}window.MaintainOpsRenderDisplayHelpers=Object.freeze({renderMetric:l,renderInsight:e,renderRoleGuide:n})})();var Ho=B(hn());(function(){function l(c,o,p="active",g={},d){let h=d.getActiveStatusFilter(),y=g.filter||g.section,b=y?"button":"article",k=g.filter&&h===g.filter?" selected":"",A=p.includes("overdue")&&Number(o)>=3,$=A?" alert-blink":"",O=[y?'type="button"':"",g.filter?`data-status-filter="${g.filter}" aria-pressed="${h===g.filter}"`:"",g.section?`data-section="${g.section}"`:""].filter(Boolean).join(" "),C=O?` ${O}`:"";return`
  ...
  `}function e(c){let o=c.getWorkOrderDashboardCounts()||{},p=o.activeWork||0,g=o.newWork||0,d=o.inProgress||0,h=o.blocked||0,y=o.overdue||0,b=o.completedAll||0,k=o.completedMonth||0,A=o.completedWeek||0,$=c.getRequestsReady()?c.openMaintenanceRequests().filter(c.matchesActiveLocation).length:0;return`
    <div class="summary-gauge-grid">
      ${l("Active Work",p,"active",{filter:"active"},c)}
      ${l("New",g,"new",{filter:"open"},c)}
      ${l("In Progress",d,"in_progress",{filter:"in_progress"},c)}
  ...
  `}function n(c,o){let p=c||{},g=p.newWork||0,d=p.inProgress||0,h=p.blocked||0,y=p.activeWork??g+d+h,b=p.overdue||0,k=p.completedAll||0,A=p.completedMonth||0,$=p.completedWeek||0;return`
    <div class="workload-strip" aria-label="Active work summary">
      ...
      ${l("In Progress",d,"in_progress workload-pill",{filter:"in_progress"},o)}
  ...
  `}function t(c){return c.getWorkOrders().filter(o=>c.getDueState(o)?.className==="overdue")}function r(c){return c.getWorkOrders().filter(o=>s(o,c))}function s(c,o,p=new Date){if(!c.completed_at)return!1;let g=new Date(c.completed_at),d=o.sundayWeekRange(p);return Number.isFinite(g.getTime())&&g>=d.start&&g<d.end}function f(c){return c.getWorkOrders().filter(a)}function a(c){let o=new Date,p=new Date(o.getFullYear(),o.getMonth(),1);return!!(c.completed_at&&new Date(c.completed_at)>=p)}function i(c){let o=c.filter(g=>g.status==="completed"&&Number(g.actual_minutes)>0);if(!o.length)return 0;let p=o.reduce((g,d)=>g+Number(d.actual_minutes||0),0);return Math.round(p/o.length)}function u(c){let o=new Date;o.setHours(0,0,0,0);let p=new Date(o);return p.setDate(p.getDate()+7),c.getPreventiveSchedules().filter(g=>{let d=new Date(`${g.next_due_at}T00:00:00`);return d>=o&&d<=p})}function m(c){return Object.freeze({renderGaugeReadout:(o,p,g="active",d={})=>l(o,p,g,d,c),renderWorkOrderGaugeDashboard:()=>e(c),renderWorkloadStrip:o=>n(o,c),overdueWorkOrders:()=>t(c),completedThisWeek:()=>r(c),isCompletedThisWeek:(o,p)=>s(o,c,p),completedThisMonth:()=>f(c),isCompletedThisMonth:a,averageCompletionMinutes:(o=c.getWorkOrders())=>i(o),preventiveDueSoon:()=>u(c)})}window.MaintainOpsDashboardDisplay=Object.freeze({createDashboardDisplayHelpers:m})})();(function(){function l(n){let t={active:'<path d="M4 12h5l2-6 4 12 2-6h3"></path>',all:'<path d="M4 6h16"></path><path d="M4 12h16"></path><path d="M4 18h16"></path>',mine:'<path d="M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8z"></path><path d="M4 21a8 8 0 0 1 16 0"></path>',created:'<path d="M5 4h10l4 4v12H5z"></path><path d="M15 4v5h5"></path><path d="M8 14h8"></path><path d="M8 17h5"></path>',vendor:'<path d="M3 16h2l3-7h8l3 7h2"></path><path d="M7 16h10"></path><path d="M8 20a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"></path><path d="M16 20a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"></path>',unassigned:'<path d="M12 5v14"></path><path d="M5 12h14"></path>',open:'<path d="M5 7h14v12H5z"></path><path d="M8 7V5h8v2"></path>',in_progress:'<path d="M12 3v4"></path><path d="M12 17v4"></path><path d="M4.2 7.5l3.5 2"></path><path d="M16.3 14.5l3.5 2"></path><path d="M19.8 7.5l-3.5 2"></path><path d="M7.7 14.5l-3.5 2"></path>',blocked:'<path d="M5 5l14 14"></path><circle cx="12" cy="12" r="8"></circle>',completed:'<path d="M4 12l5 5L20 6"></path>',overdue:'<path d="M12 8v5"></path><path d="M12 17h.01"></path><circle cx="12" cy="12" r="9"></circle>',newest:'<path d="M12 5v7l4 2"></path><circle cx="12" cy="12" r="8"></circle>',due:'<path d="M7 3v4"></path><path d="M17 3v4"></path><path d="M4 8h16"></path><path d="M5 5h14v15H5z"></path>',priority:'<path d="M12 3l8 18H4z"></path><path d="M12 9v4"></path><path d="M12 17h.01"></path>'};return`<svg class="segment-icon" viewBox="0 0 24 24" aria-hidden="true">${t[n]||t.all}</svg>`}function e(n){let t={mywork:'<path d="M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8z"></path><path d="M4 21a8 8 0 0 1 16 0"></path>',work:'<path d="M5 7h14v12H5z"></path><path d="M8 7V5h8v2"></path>',planning:'<path d="M7 3v4"></path><path d="M17 3v4"></path><path d="M4 8h16"></path><path d="M5 5h14v15H5z"></path>',requests:'<path d="M5 5h14v10H8l-3 3V5z"></path>',assets:'<path d="M4 7l8-4 8 4-8 4-8-4z"></path><path d="M4 7v10l8 4 8-4V7"></path><path d="M12 11v10"></path>',financial:'<path d="M4 19h16"></path><path d="M6 19V8"></path><path d="M12 19V5"></path><path d="M18 19v-9"></path><path d="M8 11h8"></path><path d="M10 8h4"></path>',pm:'<path d="M12 3v4"></path><path d="M12 17v4"></path><path d="M4.2 7.5l3.5 2"></path><path d="M16.3 14.5l3.5 2"></path><path d="M19.8 7.5l-3.5 2"></path><path d="M7.7 14.5l-3.5 2"></path>',procedures:'<path d="M9 6h11"></path><path d="M9 12h11"></path><path d="M9 18h11"></path><path d="M4 6l1 1 2-2"></path><path d="M4 12l1 1 2-2"></path><path d="M4 18l1 1 2-2"></path>',parts:'<path d="M14 7l3 3"></path><path d="M5 19l8-8"></path><path d="M15 5l4 4-4 4-4-4 4-4z"></path>',conversions:'<path d="M7 7h10"></path><path d="M14 4l3 3-3 3"></path><path d="M17 17H7"></path><path d="M10 14l-3 3 3 3"></path><path d="M5 12h14"></path>',performance:'<path d="M4 18h16"></path><path d="M6 15l4-4 3 2 5-6"></path><path d="M18 7h-4"></path><path d="M18 7v4"></path>',messages:'<path d="M4 5h16v11H7l-3 3V5z"></path><path d="M8 9h8"></path><path d="M8 13h5"></path>',team:'<path d="M8 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"></path><path d="M16 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"></path><path d="M3 21a5 5 0 0 1 10 0"></path><path d="M11 21a5 5 0 0 1 10 0"></path>',manager:'<path d="M4 19V5"></path><path d="M4 19h16"></path><path d="M8 15v-4"></path><path d="M12 15V8"></path><path d="M16 15v-6"></path><path d="M19 6l-4 4-3-2-4 4"></path>',setup:'<path d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"></path><path d="M19.4 15a8 8 0 0 0 .1-2l2-1.5-2-3.4-2.4 1a8 8 0 0 0-1.7-1l-.3-2.6h-4l-.3 2.6a8 8 0 0 0-1.7 1l-2.4-1-2 3.4L4.5 13a8 8 0 0 0 .1 2l-2 1.5 2 3.4 2.4-1a8 8 0 0 0 1.7 1l.3 2.6h4l.3-2.6a8 8 0 0 0 1.7-1l2.4 1 2-3.4-2-1.5z"></path>',settings:'<path d="M4 7h16"></path><path d="M4 17h16"></path><path d="M8 7v10"></path><path d="M16 7v10"></path>'};return`<svg class="nav-icon" viewBox="0 0 24 24" aria-hidden="true">${t[n]||t.work}</svg>`}window.MaintainOpsIconDisplay=Object.freeze({segmentIcon:l,navIcon:e})})();(function(){function l(n){let t={machine:"Primary",forklift:"Forklift",secondary_machine:"Sub Equipment",tooling:"Tooling / Setup",component:"Component",shop_item:"Shop Item"};return t[n]?t[n]:String(n||"machine").replaceAll("_"," ").replace(/\b\w/g,r=>r.toUpperCase())}function e(n){return n==="offline"?"Offline / Down":String(n||"running").replaceAll("_"," ").replace(/\b\w/g,t=>t.toUpperCase())}window.MaintainOpsEquipmentLabels=Object.freeze({assetTypeLabel:l,assetStatusLabel:e})})();(function(){function l({getSearchQuery:e,getAssetStatusFilter:n,getAssetTypeFilter:t,getPartSearchQuery:r,getPartInventoryFilter:s,assetTypeLabel:f,assetStatusLabel:a}){function i(c){return e().trim()?"No requests match this search.":c==="converted"?"No converted requests at this location.":c==="all"?"No requests at this location yet.":"No active requests waiting for review."}function u(){let c=n(),o=t?t():"all";return e().trim()?"No equipment matches this search.":c!=="all"?`No ${a(c).toLowerCase()} equipment found.`:o!=="all"?`No ${f(o).toLowerCase()} equipment found.`:"No equipment added yet."}function m(){return r().trim()?"No parts match this search.":s()==="low"?"No low stock parts right now.":"No parts added yet."}return{requestEmptyStateText:i,assetEmptyStateText:u,partEmptyStateText:m}}window.MaintainOpsEmptyStateText={createEmptyStateTextHelpers:l}})();var Zo=B(yn());(function(){function l({escapeHtml:e,statusLabel:n,assignmentLabel:t,activeLocationName:r,getSearchQuery:s}){function f(d){return`
      ...
      `}function i(d){return`
        <button class="global-result-item" data-search-work-order="${d.id}" type="button">
          <strong>${e(d.title)}</strong>
          <span>${n(d.status)} - ${e(d.assets?.name||"No equipment")} - ${e(t(d))}</span>
        </button>
      `}function u(d){return`
        <button class="global-result-item" data-search-asset="${d.id}" type="button">
          <strong>${e(d.name)}</strong>
          <span>${e(d.asset_code||"No serial")} - ${e(d.status)} - ${e(d.location||r())}</span>
        </button>
      `}function m(d){let h=Number(d.quantity_on_hand)||0;return`
        <button class="global-result-item" data-search-part="${d.id}" type="button">
          <strong>${e(d.name)}</strong>
          <span>${e(d.sku||"No SKU")} - ${h} on hand${d.supplier_name?` - ${e(d.supplier_name)}`:""}</span>
        </button>
      `}function c(d){return`
        <button class="global-result-item" data-search-request="${d.id}" type="button">
          <strong>${e(d.title)}</strong>
          <span>${e(d.status)} - ${e(d.assets?.name||"No equipment")}</span>
        </button>
      `}function o(d){return`
        <button class="global-result-item" data-search-section="pm" data-search-label="${e(d.title)}" type="button">
          <strong>${e(d.title)}</strong>
          <span>${e(d.assets?.name||"No equipment")} - due ${e(d.next_due_at||"unset")}</span>
        </button>
      `}function p(d){return`
        <button class="global-result-item" data-search-section="procedures" data-search-label="${e(d.name)}" type="button">
          <strong>${e(d.name)}</strong>
          <span>${(d.procedure_steps||[]).length} steps</span>
        </button>
      `}function g(d){return Object.values(d).reduce((h,y)=>h+y.length,0)}return{renderGlobalSearchResults:f,renderGlobalResultGroup:a,renderGlobalWorkResult:i,renderGlobalAssetResult:u,renderGlobalPartResult:m,renderGlobalRequestResult:c,renderGlobalPmResult:o,renderGlobalProcedureResult:p,globalResultCount:g}}window.MaintainOpsGlobalSearchDisplay={createGlobalSearchDisplayHelpers:l}})();var ei=B(bn()),ti=B(wn()),ni=B(vn());(function(){function l({escapeHtml:e,LIST_ITEMS_PER_PAGE:n,getPlanningPage:t,getPlanningGroupOpen:r=(u,m)=>m,renderListPagination:s,statusLabel:f,renderRelationshipChips:a,canEditOperationalRecords:i=()=>!0}){function u(p,g,d,h,y={}){let b=n||12,k=typeof t=="function"?t(h):1,A=Math.max(1,Math.ceil(g.length/b)),$=Math.min(Math.max(k,1),A),O=g.slice(($-1)*b,$*b),C=r(h,!!(y.defaultOpen&&g.length));return`
        <details class="planning-group" data-planning-group="${e(h)}" ${C?"open":""}>
          <summary class="planning-group-summary">
            ...
            <span class="chip ${d}">${g.length}</span>
          </summary>
      ...
      `}function c(p){return`
        <div class="planning-grid">
          ${m("Needs action","Unscheduled work and completed items that still need follow-up.",`
            ${u("No Due Date",p.noDue,"blocked","no-due",{defaultOpen:!0,description:"Set a date here to move work into the schedule."})}
            ${u("Follow-up Needed",p.followUp,"blocked","follow-up",{description:"Close the loop or create the next order."})}
          `,"planning-lane-action")}
          ${m("Current schedule","Work requiring attention now.",`
            ${u("Overdue",p.overdue,"overdue","overdue",{defaultOpen:!0})}
            ${u("Due Today",p.today,"due_today","today",{defaultOpen:!0})}
          `)}
          ${m("Upcoming","Near-term maintenance and preventive work.",`
            ${u("Next 7 Days",p.soon,"in_progress","soon")}
            ${u("PM Due Soon",p.pm,"open","pm")}
          `)}
        </div>
      `}function o(p){if(p.kind==="follow_up")return`
        ...
        `;if(p.kind==="no_due"){let g=p.createdAt?new Date(p.createdAt):null,d=g&&!Number.isNaN(g.getTime())?g.toLocaleDateString():"Unknown";return`
      ...
      `}function m(o,p){if(o<=t)return"";let g=a(),d=(g-1)*t+1,h=Math.min(o,g*t);return`
        <div class="pagination-bar">
      ...
      `}function c(o,p,g,d){if(p<=r)return"";let h=(g-1)*r+1,y=Math.min(p,g*r);return`
        <div class="pagination-bar">
          ...
          <span>Showing ${h}-${y} of ${p} - Page ${g} of ${d}</span>
          <button class="secondary-button page-action-button" data-list-page="${o}" data-page-direction="next" type="button" ${g>=d?"disabled":""}>Next</button>
        </div>
      `}return{renderWorkPagination:i,renderPartsPagination:u,renderAssetsPagination:m,renderListPagination:c}}window.MaintainOpsPaginationDisplay={createPaginationDisplayHelpers:l}})();var ii=B(_n());(function(){function l({escapeHtml:e,getLocations:n,getActiveLocationId:t,getAssets:r,matchesActiveLocation:s,isAssetDescendantOf:f,parentAssetFor:a}){function i(g=t()){return n().map(d=>`<option value="${d.id}" ${d.id===g?"selected":""}>${e(d.name)}</option>`).join("")}function u(g){let d=a(g);return d?`${g.name} - part of ${d.name}`:g.name}function m(g=""){let d=r().filter(s).sort((b,k)=>u(b).localeCompare(u(k))),h=g?r().find(b=>b.id===g):null;return(h&&!d.some(b=>b.id===h.id)?[h,...d]:d).map(b=>`<option value="${b.id}" ${b.id===g?"selected":""}>${e(u(b))}</option>`).join("")}function c(g="",d=""){return r().filter(s).filter(h=>h.id!==d&&!f(h.id,d)).sort((h,y)=>u(h).localeCompare(u(y))).map(h=>`<option value="${h.id}" ${h.id===g?"selected":""}>${e(u(h))}</option>`).join("")}function o(g=""){let d=[...new Set(r().filter(s).map(y=>String(y.location||"").trim()).filter(Boolean))].sort((y,b)=>y.localeCompare(b)),h=String(g||"").trim();return h&&!d.includes(h)?[h,...d]:d}function p(g=""){return o(g).map(d=>`<option value="${e(d)}" ${d===g?"selected":""}>${e(d)}</option>`).join("")}return{renderLocationOptions:i,renderAssetOptions:m,renderParentAssetOptions:c,renderAssetAreaOptions:p,assetOptionLabel:u}}window.MaintainOpsOptionDisplay={createOptionDisplayHelpers:l}})();(function(){function l({escapeHtml:e,requestPhotoMetaText:n,getRequestPhotosReady:t}){function r(s){if(!s.photo_storage_path)return"";let f=s.photo_file_name||s.photo_original_file_name||"Request photo",a=n(s);return`
      ...
      `}return{renderAppIssueReport:r}}window.MaintainOpsAppIssueDisplay={createAppIssueDisplayHelpers:l}})();(function(){function l({escapeHtml:e,formatMessageTime:n,messageThreadScopeLabel:t,getMessageThreads:r,getMessagesByThreadId:s,getMessageWorkOrderLinksReady:f}){function a(u){let m=s()[u.id]||[],c=m[m.length-1];return`
      ...
      `}function i(u){let m=r().filter(c=>c.work_order_id===u.id);return`
      ...
      `}return{renderPartSourceOptions:r,renderPartSourceManager:s}}window.MaintainOpsPartSourceDisplay={createPartSourceDisplayHelpers:l}})();(function(){function l({escapeHtml:e,assetTypeLabel:n,getWorkOrders:t,getActiveAssetId:r,parentAssetFor:s,childAssetsFor:f}){function a(i){let u=t().filter(o=>o.asset_id===i.id&&o.status!=="completed").length,m=s(i),c=f(i.id);return`
...
${i}`:i}return{cleanWorkOrderDescription:n,descriptionWithAssignmentNote:t,descriptionWithRequestPhotoNote:r}}window.MaintainOpsWorkOrderDescriptionDisplay={createWorkOrderDescriptionDisplayHelpers:l}})();(function(){function l(){function e(n,t){if(!n)return"Work order updated.";let r=[];return n.title!==t.title&&r.push("title"),(n.description||"")!==(t.description||"")&&r.push("description"),(n.due_at||"")!==(t.due_at||"")&&r.push("due date"),n.priority!==t.priority&&r.push("priority"),(n.type||"corrective")!==t.type&&r.push("type"),(n.assigned_to||"")!==(t.assigned_to||"")&&r.push("assignment"),(n.procedure_template_id||"")!==(t.procedure_template_id||"")&&r.push("procedure"),String(n.actual_minutes||0)!==String(t.actual_minutes||0)&&r.push("actual minutes"),r.length?`Updated ${r.join(", ")}.`:"Work order saved."}return{describeWorkOrderChanges:e}}window.MaintainOpsWorkOrderChangeDisplay={createWorkOrderChangeDisplayHelpers:l}})();(function(){function l(){function e(n,t,r,s=[]){return[...n.map(f=>({...f,type:"comment"})),...t.map(f=>({...f,type:"photo"})),...s.map(f=>({...f,type:"part"})),...r.map(f=>({...f,type:"event"}))].sort((f,a)=>new Date(a.created_at)-new Date(f.created_at))}return{buildActivityFeed:e}}window.MaintainOpsActivityFeedDisplay={createActivityFeedDisplayHelpers:l}})();(function(){function l(e){function n(a){return Number(a.quantity_on_hand)<=Number(a.reorder_point)}function t(){return e.getParts().filter(n)}function r(a){let i=e.getPartSearchQuery().trim().toLowerCase();return i?a.some(u=>String(u??"").toLowerCase().includes(i)):!0}function s(){let a=e.getParts().filter(i=>!e.matchesActiveLocation(i)||e.getPartInventoryFilter()==="low"&&!n(i)?!1:r([i.name,i.sku,i.supplier_name,i.machine_note,i.quantity_on_hand,i.reorder_point,i.unit_cost]));return e.getPartSort&&e.getPartSort()==="source"?[...a].sort((i,u)=>{let m=String(i.supplier_name||"zzzzzz").localeCompare(String(u.supplier_name||"zzzzzz"),void 0,{sensitivity:"base"});return m||String(i.name||"").localeCompare(String(u.name||""),void 0,{sensitivity:"base"})}):a}function f(){return[...new Set(e.getParts().filter(e.matchesActiveLocation).map(a=>String(a.supplier_name||"").trim()).filter(Boolean))].sort((a,i)=>a.localeCompare(i))}return{isLowStockPart:n,lowStockParts:t,filteredParts:s,matchesPartSearch:r,partSourceOptions:f}}window.MaintainOpsPartInventoryDisplay={createPartInventoryDisplayHelpers:l}})();(function(){function l(e){function n(t){return Object.values(e.getPartsUsedByWorkOrder()).flat().filter(r=>r.part_id===t)}return{partUsageRows:n}}window.MaintainOpsPartUsageDisplay={createPartUsageDisplayHelpers:l}})();(function(){function l(e){function n(){return e.getMaintenanceRequests().filter(i=>i.status==="submitted")}function t(i){return e.matchesActiveLocation(i)&&e.matchesSearch([i.title,i.description,i.status,i.priority,i.assets?.name,e.getProfilesByUserId()[i.requested_by]?.full_name])}function r(i){return i.status==="converted"||!!i.converted_work_order_id}function s(i,u=e.getRequestViewFilter()){return u==="converted"?r(i):u==="all"?!0:!r(i)&&i.status==="submitted"}function f(i=e.getRequestViewFilter()){return e.getMaintenanceRequests().filter(u=>t(u)&&s(u,i))}function a(){return e.getRequestDashboardCounts()||{active:0,converted:0,all:0}}return{openMaintenanceRequests:n,requestMatchesBaseFilters:t,isConvertedRequest:r,requestMatchesViewFilter:s,filteredRequests:f,requestFilterCounts:a}}window.MaintainOpsRequestQueueDisplay={createRequestQueueDisplayHelpers:l}})();(function(){function l(){function e(t){let r=[t.workOrders?`${t.workOrders} work order${t.workOrders===1?"":"s"}`:"",t.children?`${t.children} linked equipment item${t.children===1?"":"s"}`:"",t.schedules?`${t.schedules} PM schedule${t.schedules===1?"":"s"}`:"",t.requests?`${t.requests} request${t.requests===1?"":"s"}`:""].filter(Boolean);return r.length?`This equipment is kept for traceability because it has ${r.join(", ")}.`:""}function n(t){let r=[t.workOrders?`${t.workOrders} work order${t.workOrders===1?"":"s"}`:"",t.schedules?`${t.schedules} PM schedule${t.schedules===1?"":"s"}`:""].filter(Boolean);return r.length?`This procedure is kept for traceability because it is linked to ${r.join(", ")}.`:""}return{assetDeleteBlockerMessage:e,procedureDeleteBlockerMessage:n}}window.MaintainOpsDeleteBlockerDisplay={createDeleteBlockerDisplayHelpers:l}})();(function(){function l(e){function n(f){return e.getAssets().find(a=>a.id===f?.parent_asset_id)||null}function t(f){return e.getAssets().filter(a=>a.parent_asset_id===f).sort((a,i)=>a.name.localeCompare(i.name))}function r(f,a){if(!f||!a)return!1;let i=e.getAssets().find(m=>m.id===f),u=new Set;for(;i?.parent_asset_id&&!u.has(i.id);){if(i.parent_asset_id===a)return!0;u.add(i.id),i=e.getAssets().find(m=>m.id===i.parent_asset_id)}return!1}function s(){return e.getAssets().filter(f=>!e.matchesActiveLocation(f)||e.getAssetStatusFilter()!=="all"&&f.status!==e.getAssetStatusFilter()||e.getAssetTypeFilter&&e.getAssetTypeFilter()!=="all"&&(f.asset_type||"machine")!==e.getAssetTypeFilter()||e.getAssetAreaFilter&&e.getAssetAreaFilter()!=="all"&&(f.location||"")!==e.getAssetAreaFilter()?!1:e.matchesSearch([f.name,f.asset_code,f.manufacturer,f.model,f.location,f.status,f.asset_type,n(f)?.name]))}return{filteredAssets:s,parentAssetFor:n,childAssetsFor:t,isAssetDescendantOf:r}}window.MaintainOpsAssetHierarchyDisplay={createAssetHierarchyDisplayHelpers:l}})();var Li=B(qn());(function(){function l(e){function n(r){let s=e.getSearchQuery().trim().toLowerCase();return s?r.some(f=>String(f??"").toLowerCase().includes(s)):!0}function t(r,s=e.getSearchQuery()){let f=s.trim().toLowerCase();return f?r.some(a=>String(a??"").toLowerCase().includes(f)):!0}return{matchesSearch:n,matchesQuery:t}}window.MaintainOpsSearchFilterDisplay={createSearchFilterDisplayHelpers:l}})();(function(){function l(e){function n(a){return a.due_at?new Date(`${a.due_at}T00:00:00`).getTime():Number.MAX_SAFE_INTEGER}function t(a){return{low:1,medium:2,high:3,critical:4}[a]||0}function r(a){return a.completed_at?new Date(a.completed_at).getTime():0}function s(a){return typeof e.assignmentLabel=="function"?e.assignmentLabel(a):a.assigned_profile?.full_name||a.assigned_to||"Unassigned"}function f(a,i){return["completed","completed_month","completed_week"].includes(e.getActiveStatusFilter())?r(i)-r(a)||new Date(i.created_at)-new Date(a.created_at):e.getWorkSort()==="due"?n(a)-n(i)||new Date(i.created_at)-new Date(a.created_at):e.getWorkSort()==="priority"?t(i.priority)-t(a.priority)||n(a)-n(i):e.getWorkSort()==="type"?String(a.type||"").localeCompare(String(i.type||""))||new Date(i.created_at)-new Date(a.created_at):e.getWorkSort()==="assigned"?s(a).localeCompare(s(i))||new Date(i.created_at)-new Date(a.created_at):new Date(i.created_at)-new Date(a.created_at)}return{compareWorkOrders:f,dueSortValue:n,prioritySortValue:t,completedSortValue:r,assigneeSortLabel:s}}window.MaintainOpsWorkOrderSortDisplay={createWorkOrderSortDisplayHelpers:l}})();(function(){function l(e){function n(r){return r?.location_id||r?.assets?.location_id||null}function t(r){return!e.getLocationsReady()||!e.getActiveLocationId()?!0:n(r)===e.getActiveLocationId()}return{recordLocationId:n,matchesActiveLocation:t}}window.MaintainOpsLocationFilterDisplay={createLocationFilterDisplayHelpers:l}})();(function(){function l(e){function n(){return e.getWorkOrders().filter(i=>e.matchesActiveLocation(i)&&i.status!=="completed").slice(0,8)}function t(){let i=e.getMessageThreadFilter();return e.getMessageThreads().filter(u=>(i==="all"||i==="unread"&&s(u.id)>0||u.thread_type===i)&&e.matchesQuery(r(u),e.getMessageSearchQuery()))}function r(i){let u=e.getMessagesByThreadId()[i.id]||[],m=e.getMessageThreadMembers().filter(c=>c.thread_id===i.id).map(c=>e.teamMemberName(c.user_id));return[i.title,e.messageThreadScopeLabel(i),...m,...u.map(c=>c.body||"")]}function s(i){let u=e.getMessageReadsByThreadId()[i]?.last_read_at,m=u?new Date(u).getTime():0;return(e.getMessagesByThreadId()[i]||[]).filter(c=>c.sender_id===e.getCurrentUser()?.id?!1:new Date(c.created_at).getTime()>m).length}function f(){return e.getMessageThreads().reduce((i,u)=>i+s(u.id),0)}function a(){return e.getMessageThreads().filter(i=>i.thread_type==="direct").reduce((i,u)=>i+s(u.id),0)}return{recentMessageLinkWorkOrders:n,filteredMessageThreads:t,messageThreadSearchValues:r,unreadMessageCount:s,totalUnreadMessages:f,directUnreadMessages:a}}window.MaintainOpsMessageThreadFilterDisplay={createMessageThreadFilterDisplayHelpers:l}})();(function(){function l(e){function n(t){let r=e.getActiveStatusFilter();return r==="overdue"?e.getDueState(t)?.className==="overdue":r==="completed_month"?e.isCompletedThisMonth(t):r==="completed_week"?e.isCompletedThisWeek(t):r==="active"||r==="all"?t.status!=="completed":t.status===r}return{workOrderMatchesStatusFilter:n}}window.MaintainOpsWorkOrderStatusFilterDisplay={createWorkOrderStatusFilterDisplayHelpers:l}})();(function(){function l(e){function n(t){let r=e.getPartsUsedByWorkOrder()[t.id]||[],s=e.getCommentsByWorkOrder()[t.id]||[],f=e.getEventsByWorkOrder()[t.id]||[],a=e.getPhotosByWorkOrder()[t.id]||[],i=e.getProcedureTemplates().find(c=>c.id===t.procedure_template_id),u=Object.values(e.getStepResultsByWorkOrder()[t.id]||{}),m=e.getProfilesByUserId();return[t.title,t.description,t.status,e.statusLabel(t.status),t.priority,t.type,t.assets?.name,e.assignmentLabel(t),t.production_action,m[t.production_action_assigned_to]?.full_name,t.failure_cause,t.resolution_summary,t.completion_notes,t.current_update,i?.name,i?.description,...(i?.procedure_steps||[]).flatMap(c=>[c.prompt,c.step_type]),...r.flatMap(c=>[c.parts?.name,c.parts?.sku,c.parts?.supplier_name,c.quantity_used,c.unit_cost]),...s.flatMap(c=>[c.body,m[c.author_id]?.full_name]),...f.flatMap(c=>[c.event_type,c.summary,m[c.actor_id]?.full_name]),...a.flatMap(c=>[c.file_name,c.original_file_name,c.content_type]),...u.flatMap(c=>[c.value,c.notes])]}return{workOrderSearchValues:n}}window.MaintainOpsWorkOrderSearchDisplay={createWorkOrderSearchDisplayHelpers:l}})();(function(){function l(e){function n(){let t=e.getCurrentUser()?.id;return e.getWorkOrders().filter(r=>e.matchesActiveLocation(r)?(e.getMyWorkFilter()==="created"?r.created_by===t:e.isWorkOrderAssignedToUser(r,t))&&e.matchesSearch(e.workOrderSearchValues(r)):!1)}return{myWorkQueueOrders:n}}window.MaintainOpsMyWorkQueueDisplay={createMyWorkQueueDisplayHelpers:l}})();(function(){function l(e){function n(t){return e.isMissingColumnError(t,"work_order_id")?{message:"Run supabase/step-next-message-work-order-links.sql before linking message threads to work orders.",messagesReady:null}:e.isColumnSchemaError(t,["message_threads","message_thread_members","messages"])||String(t?.message||"").includes("message_threads")?{message:"Run supabase/step-next-message-center.sql before using Messages.",messagesReady:!1}:{message:t?.message||String(t),messagesReady:null}}return{messageCenterErrorState:n}}window.MaintainOpsMessageCenterErrorDisplay={createMessageCenterErrorDisplayHelpers:l}})();(function(){function l(e){function n(t){return e.isColumnSchemaError(t,["app_issue_reports"])||String(t?.message||"").includes("app_issue_reports")?{message:"Run supabase/step-next-app-issue-reports.sql before saving app issue reports.",appIssueReportsReady:!1}:{message:t?.message||String(t),appIssueReportsReady:null}}return{appIssueReportErrorState:n}}window.MaintainOpsAppIssueErrorDisplay={createAppIssueErrorDisplayHelpers:l}})();var Yi=B(Cn()),Ki=B($n()),Ji=B(Pn()),Zi=B(An()),Xi=B(Rn()),es=B(En()),ts=B(On()),ns=B(Wn());(function(){function l(t){if(!t)return"";let r=new Date(t),s=new Date,f=new Date(s.getFullYear(),s.getMonth(),s.getDate()).getTime(),a=new Date(r.getFullYear(),r.getMonth(),r.getDate()).getTime(),i=r.toLocaleTimeString([],{hour:"numeric",minute:"2-digit"});return a===f?`Today ${i}`:a===f-864e5?`Yesterday ${i}`:r.toLocaleDateString([],{month:"short",day:"numeric"})}function e(t){if(!t)return"";let r=new Date(t),s=new Date,f=new Date(s.getFullYear(),s.getMonth(),s.getDate()).getTime(),a=new Date(r.getFullYear(),r.getMonth(),r.getDate()).getTime();return a===f?"Today":a===f-864e5?"Yesterday":r.toLocaleDateString([],{weekday:"short",month:"short",day:"numeric"})}function n(t){let r=String(t||"").trim().split(/\s+/).filter(Boolean);return r.length?r.slice(0,2).map(s=>s[0]).join("").toUpperCase():"MO"}window.MaintainOpsMessageFormatting=Object.freeze({formatMessageTime:l,formatMessageDay:e,initials:n})})();(function(){function l(e){function n(r){let s=r.sender_id===e.getCurrentUserId(),f=e.teamMemberName(r.sender_id);return`
  ...
  `}function t(r){let s=r.filter(a=>!a.deleted_at);if(!s.length)return'<p class="muted">No messages yet.</p>';let f="";return s.map(a=>{let i=e.formatMessageDay(a.created_at),u=i!==f?`<div class="message-day-divider"><span>${e.escapeHtml(i)}</span></div>`:"";return f=i,`${u}${n(a)}`}).join("")}return Object.freeze({renderMessageBubble:n,renderMessageList:t})}window.MaintainOpsMessageDisplay=Object.freeze({createMessageDisplayHelpers:l})})();})();
//# sourceMappingURL=runtime.66a8d60b16.js.map
