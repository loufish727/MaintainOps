...
`supabase/migrations/20260924025115_traveling_primary_equipment.sql` (UTC filename; local release date September 23) was applied to isolated QA `fsxqrngpaseqdxijggcm`. SHA256: `8de88d4ea9147dc6321dbbe40ee1cb926477345dc9164f93b2466f7186a651aa`. Production is not yet migrated at this checkpoint.

Adds the `traveling_machine` asset type, invoker-only hierarchy/location guards, atomic asset history and an authenticated operational-editor move RPC. No existing business rows are rewritten. Isolated PostgreSQL relationship/permission/rollback tests and signed-in Chromium desktop/mobile lifecycle checks passed. See `TRAVELING_EQUIPMENT.md` for the contract and non-destructive rollback.
