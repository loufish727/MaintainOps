...
- `supabase/migrations/202608041200_production_actions.sql` adds the Production role, work-order Production Actions, My Work routing, completion guards, and automatic history; testing-platform application and signed-in lifecycle proof are complete, while production application remains pending the protected GitHub proof.
...
|---|---|---|
| `supabase/migrations/202608041200_production_actions.sql` | Testing applied; production pending | Applied to isolated testing project `fsxqrngpaseqdxijggcm` on 2026-08-04 after migration-static, isolated PostgreSQL/RLS, recursive SQL-security, DOM-assignment, bundle, focused workflow, and local browser verification. Preflight and postflight retained 5 existing members and 8 work orders. Catalog checks found 7 Production Action columns, 5 constraints, 2 work-order triggers, the Production role constraint, and a security-definer count RPC restricted to `authenticated`. A disposable fifth QA role completed the signed-in lifecycle: assign, Team drill-down, My Work routing, work-order completion guard, Production Action completion, queue removal, and cleanup. The target work order returned to its original open/unassigned state with no action; automatic history remained. Production is intentionally not yet applied. |
