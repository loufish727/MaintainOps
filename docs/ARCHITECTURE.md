...
- `src/`
  ...
  - Manager, Financial, Team presentation, Admin Setup and Messages are separate hashed feature bundles. They load only when their screen is opened and initialize once per browser page. Messages also has a lazy hashed stylesheet; metadata/unread and realtime coordination remain lightweight startup services.
