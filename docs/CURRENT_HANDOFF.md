...
Completed the Equipment delete-request warning opener boundary extraction.

- Latest app behavior commit:
  - `3c0ce6d` (`Extract workspace asset delete request events`)
...
- Latest live cache tag:
  - `app.js?v=lfes-authority-asset-delete-request-events-1`
...
- Verification:
  - static JS checks passed for `app.js`, `src/utils/workspaceAssetDeleteCancelEvents.js`, and `tests/smoke/workspace-asset-delete-cancel-events-smoke.js`.
  - targeted mock-DOM Equipment delete warning/cancel smoke passed for delete-request callback, cancel pending-state clear, render, propagation stop, and missing-state no-op.
  - local resource smoke passed against `http://127.0.0.1:4193/`.
  - local browser boot smoke loaded the app shell with the new Equipment delete-request script/cache tag present.
  ...
  - live Equipment delete-request smoke created disposable asset `LFES disposable asset delete request 1779827205046`, verified Delete Equipment opened Cancel/Permanently Delete warning, clicked Cancel, verified Delete Equipment returned, then cleaned the disposable through the manager UI and verified data-layer `remaining: 0`.
  - hosted resource smoke passed for `3c0ce6d`; GitHub Actions verification remains deferred because the unauthenticated GitHub API verifier is rate-limited.
...
- Recommended next step:
  ...
  - quick work-order status, assignment, downtime-copy, detail status dropdown, completion, delete, Team work-view, Parts detail UI, Message Center local UI, Parts search, workspace section navigation, Message Center thread open/read-state, issue/admin local UI, Part delete-cancel, Work Message Start, Report Issue command, Submit Request command, New Work Order command, Quick Fix command, asset Quick Fix opener, request Quick Fix opener, Export CSV command, public request link copy, public QR print, asset-location warning, Equipment delete warning/cancel, Request delete-cancel, PM schedule delete-cancel, Procedure delete-cancel, textarea auto-grow, and Team invite cancel-warning boundaries are implemented and live verified.
