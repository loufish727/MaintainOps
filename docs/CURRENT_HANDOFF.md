...
Completed the Submit Request command-opener boundary extraction from `bindWorkspaceEvents()`.

- Latest app behavior commit:
  - `b5328a7` (`Extract workspace submit request command events`)
...
- Latest live cache tag:
  - `app.js?v=lfes-authority-submit-request-command-events-1`
...
- Verification:
  - static JS checks passed for `app.js`, `src/utils/workspaceSubmitRequestCommandEvents.js`, `tests/smoke/resource-load.spec.js`, and `tests/smoke/workspace-submit-request-command-events-smoke.js`.
  - targeted mock-DOM Submit Request command smoke passed for clearing active detail/form modes, switching to Requests, resetting request paging, persisting active section, and reloading the request queue.
  - local resource smoke passed against `http://127.0.0.1:4193/`.
  - local browser boot smoke loaded the app shell with the new Submit Request command event script/cache tag present and no browser warning/error logs.
  ...
  - live Submit Request command smoke opened the `More` disclosure, clicked Submit Request, confirmed Requests rendered with the request form, with new script/cache tags and no browser warning/error logs.
  - live `index.html` referenced `src/utils/workspaceSubmitRequestCommandEvents.js?v=lfes-authority-submit-request-command-events-1` and `app.js?v=lfes-authority-submit-request-command-events-1`.
  - `npm run test:smoke:github-actions` passed for `b5328a7`: Resource Load Smoke `26462192835` completed successfully. Pages build/deployment `26462191804` also completed successfully.
...
- Recommended next step:
  ...
  - quick work-order status, assignment, downtime-copy, detail status dropdown, completion, delete, Team work-view, Parts detail UI, Message Center local UI, Parts search, workspace section navigation, Message Center thread open/read-state, issue/admin local UI, Part delete-cancel, Work Message Start, Report Issue command, and Submit Request command boundaries are implemented and live verified.
