...
- 2026-05-27: Five-part readiness pass completed and deployed in `1e91b02`. Added `docs/OPERATIONS_BACKUP_RESTORE_INCIDENT_PLAN.md`, `docs/PUBLIC_REQUEST_INTAKE_HARDENING.md`, and `docs/PUBLIC_EXPOSURE_REVIEW_2026-05-27.md`. Extracted public request-link admin button binding into `src/utils/workspacePublicRequestLinkAdminEvents.js`; `app.js` still owns public request-link RPC updates, token generation, link data, auth/company/location state, public intake submit, SQL/RLS, and render behavior. Added `tests/smoke/workspace-public-request-link-admin-events-smoke.js`. `app.js` moved from 7,442 to 7,437 lines. Verification passed: static checks for `app.js`, the new module, and the new smoke; public request-link admin event smoke; public request-link copy smoke; public QR print smoke; Submit Request command smoke; Quick Fix workflow smoke; hosted resource smoke; GitHub Actions Resource Load Smoke for `1e91b02`.
