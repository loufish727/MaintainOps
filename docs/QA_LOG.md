...
- 2026-05-27: Hard-boundary Create Work Order renderer extraction completed locally. Added `src/render/createWorkOrderDisplay.js` and `tests/smoke/create-work-order-display-smoke.js`, wired `app.js` through explicit `createCreateWorkOrderDisplayHelpers(...)` dependency injection, and added the module to `index.html` / resource smoke. `app.js` moved from 7,706 to 7,627 lines. Static checks passed; Create Work Order display smoke passed; New Work Order command opener and Asset location warning event regression smokes passed. Scope stayed form-renderer-only: no submit handler, payload construction, insert, photo upload, part usage, comment creation, auth/session/company/location, SQL, or RLS changes.
