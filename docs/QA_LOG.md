...
- Draft-recovery checkpoint `f6cb6ce`: Full Strict 13/13 and authenticated proof 7/7 passed from a clean worktree, followed by final Chromium 2/2 and WebKit 9/9. Startup gzip payload decreased by 467 bytes after moving recovery into the existing lazy Messages bundle. This checkpoint is local, not a production deployment.
