...
- 2026-05-27: LFES active-work-order state wiring pass staged for deployment. `activeWorkOrderId` now initializes from `workspaceUiState`, active-work-order writes go through `setActiveWorkOrderIdState(...)`, and the pending app cache tag is `app.js?v=lfes-state-active-work-order-1`. Scope is state synchronization only: no work-order selectors, mutation sequencing, comments, photos, parts-used, detail forms, render, auth/company/location state, or Supabase access changes.
