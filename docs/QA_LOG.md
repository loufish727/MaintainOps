...
- 2026-05-27: HIGH-risk Quick Fix submit workflow extraction completed locally after dedicated disposable lifecycle smoke. Added `src/workflows/quickFixWorkflow.js`, `tests/smoke/quick-fix-workflow-smoke.js`, and `tests/smoke/quick-fix-live-lifecycle-smoke.js`; `app.js` now injects all Quick Fix submit dependencies through `createQuickFixWorkflow(...)`. `app.js` moved from 7,569 to 7,442 lines. Verification passed so far: static checks for `app.js`, workflow module, and smoke files; Quick Fix workflow mock smoke; Quick Fix display smoke; Quick Fix command opener, Asset Quick Fix opener, Request Quick Fix opener, and Asset location warning event regression smokes; local boot smoke. Pre-extraction live disposable lifecycle smoke passed with hosted app: created a Quick Fix, verified detail status `open`, deleted through app UI, and verified the heading was gone. Scope moved the submit workflow only; auth/session/company/location startup, SQL/RLS, storage policy, broad render, and unrelated workflows stayed in `app.js`.
