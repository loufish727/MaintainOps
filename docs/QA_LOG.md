...
- 2026-05-27: LFES active-part state wiring pass staged for deployment. `activePartId` now initializes from `workspaceUiState`, active-part writes go through `setActivePartIdState(...)`, and the pending app cache tag is `app.js?v=lfes-state-active-part-1`. Static checks, workspace UI state smoke, part-detail event smoke, message UI smoke, report-issue command smoke, section navigation smoke, workspace search state smoke, and hosted resource smoke baseline passed before deployment.
