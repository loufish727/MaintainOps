...
- 2026-05-27: Public request token helper extraction completed locally. Added `src/utils/publicRequestTokens.js` and `tests/smoke/public-request-token-smoke.js`; `app.js` now uses the helper for public request QR regeneration token generation while public request-link RPC updates, link data, auth/company/location state, public intake submit, SQL/RLS, and render behavior remain app-owned. `app.js` moved from 7,437 to 7,430 lines. Verification passed: static checks for `app.js`, the new helper, and the new smoke; public request token smoke; public request-link admin event smoke; public request-link copy smoke; public QR print smoke; Submit Request command smoke.
- 2026-05-27: Five-part readiness pass completed and deployed in `1e91b02`. Identified backup/restore, incident response, public request intake hardening, and public exposure review as ongoing production-readiness workstreams without promoting detailed internal runbooks in the public repo. Extracted public request-link admin button binding into `src/utils/workspacePublicRequestLinkAdminEvents.js`; `app.js` still owns public request-link RPC updates, token generation, link data, auth/company/location state, public intake submit, SQL/RLS, and render behavior. Added `tests/smoke/workspace-public-request-link-admin-events-smoke.js`. `app.js` moved from 7,442 to 7,437 lines. Verification passed: static checks for `app.js`, the new module, and the new smoke; public request-link admin event smoke; public request-link copy smoke; public QR print smoke; Submit Request command smoke; Quick Fix workflow smoke; hosted resource smoke; GitHub Actions Resource Load Smoke for `1e91b02`.
- 2026-05-27: HIGH-risk Quick Fix submit workflow extraction completed and deployed in `954466c` after dedicated disposable lifecycle smoke. Added `src/workflows/quickFixWorkflow.js`, `tests/smoke/quick-fix-workflow-smoke.js`, and `tests/smoke/quick-fix-live-lifecycle-smoke.js`; `app.js` now injects all Quick Fix submit dependencies through `createQuickFixWorkflow(...)`. `app.js` moved from 7,569 to 7,442 lines. Verification passed: static checks for `app.js`, workflow module, and smoke files; Quick Fix workflow mock smoke; Quick Fix display smoke; Quick Fix command opener, Asset Quick Fix opener, Request Quick Fix opener, and Asset location warning event regression smokes; local boot smoke; hosted resource smoke; GitHub Actions Resource Load Smoke #271; hosted post-extraction disposable lifecycle smoke. Hosted proof created `LFES disposable quick fix 1779910619626`, verified detail status `open`, deleted through app UI, confirmed `remainingHeading: 0`, served `app.js?v=lfes-quick-fix-workflow-1`, and captured no browser logs. Scope moved the submit workflow only; auth/session/company/location startup, SQL/RLS, storage policy, broad render, and unrelated workflows stayed in `app.js`.
- 2026-05-27: Live Quick Fix lifecycle smoke caught a post-renderer startup regression before mutation extraction. Hosted boot first failed with `Cannot access 'renderMessageCenter' before initialization`; after moving `init()` to the end, headless boot exposed `Cannot access 'renderCreateWorkOrder' before initialization`. Fixed by preserving startup after renderer factory initialization and passing `renderCreateWorkOrder` to Asset Detail as a lazy callback. A separate cache catch also appeared: the first startup fix did not bump the `app.js` cache tag, so live kept loading the old app bundle until `index.html` was updated.
- 2026-05-27: Hard-boundary Quick Fix renderer extraction completed and deployed in `b5b10aa`. Added `src/render/quickFixDisplay.js` and `tests/smoke/quick-fix-display-smoke.js`, wired `app.js` through explicit `createQuickFixDisplayHelpers(...)` dependency injection, and added the module to `index.html` / resource smoke. `app.js` moved from 7,627 to 7,569 lines. Static checks passed; Quick Fix display smoke passed; Quick Fix command opener, Asset Quick Fix opener, Request Quick Fix opener, and Asset location warning event regression smokes passed; hosted resource smoke passed; GitHub Actions Resource Load Smoke #265 passed by browser proof after the local unauthenticated verifier hit API rate limit. Scope stayed form-renderer-only: no `createQuickFix` submit handler, payload construction, work-order insert, new-equipment creation, photo upload, part usage, request conversion, asset status mutation, activity logging, auth/session/company/location, SQL, or RLS changes. LFES decision: the full `createQuickFix` mutation boundary was rejected for this run because it needs a dedicated disposable mutation/cleanup smoke before extraction.
- 2026-05-27: Hard-boundary Create Work Order renderer extraction completed and deployed in `c60cf05`. Added `src/render/createWorkOrderDisplay.js` and `tests/smoke/create-work-order-display-smoke.js`, wired `app.js` through explicit `createCreateWorkOrderDisplayHelpers(...)` dependency injection, and added the module to `index.html` / resource smoke. `app.js` moved from 7,706 to 7,627 lines. Static checks passed; Create Work Order display smoke passed; New Work Order command opener and Asset location warning event regression smokes passed; hosted resource smoke passed. GitHub Actions Resource Load Smoke run `26532736494` passed per authenticated browser proof after the local unauthenticated verifier hit the GitHub API rate limit. Scope stayed form-renderer-only: no submit handler, payload construction, insert, photo upload, part usage, comment creation, auth/session/company/location, SQL, or RLS changes.
- 2026-05-27: Hard-boundary Message Center renderer extraction completed and deployed in `e4380ec`. Added `src/render/messageCenterDisplay.js` and `tests/smoke/message-center-display-smoke.js`, wired `app.js` through explicit `createMessageCenterDisplayHelpers(...)` dependency injection, and added the module to `index.html` / resource smoke. `app.js` moved from 7,783 to 7,706 lines. Static checks passed; Message Center display smoke passed; Message UI, Message Thread, and Work Message Start event regression smokes passed; hosted resource smoke passed; GitHub Actions Resource Load Smoke run `26532554631` passed. LFES catch: the first module version read detail-only dependency getters before the messages-not-ready fallback; the module now preserves the old schema/setup fallback before reading message state dependencies.
- 2026-05-27: Hard-boundary Asset Detail renderer extraction completed and deployed in `c58aa67`. Added `src/render/assetDetailDisplay.js` and `tests/smoke/asset-detail-display-smoke.js`, wired `app.js` through explicit `createAssetDetailDisplayHelpers(...)` dependency injection, and added the module to `index.html` / resource smoke. `app.js` moved from 7,858 to 7,783 lines. Static checks passed; Asset Detail display smoke passed; Asset Quick Fix, Asset delete/cancel, and Asset location warning event regression smokes passed; hosted resource smoke passed; GitHub Actions Resource Load Smoke run `26532350053` passed. LFES catch: the first module version read detail-only dependency getters before the missing-asset early return; the smoke caught it and the module now preserves the old create-work-order fallback behavior.
- 2026-05-27: Hard-boundary Work Order Detail renderer extraction completed and deployed in `c3d3cf3`. Added `src/render/workOrderDetailDisplay.js` and `tests/smoke/work-order-detail-display-smoke.js`, wired `app.js` through explicit `createWorkOrderDetailDisplayHelpers(...)` dependency injection, and added the module to `index.html` / resource smoke. `app.js` moved from 8,059 to 7,858 lines. Static checks passed; Work Order Detail display smoke passed; Work Order edit, Quick Update, comment, and Work Message Start event regression smokes passed; hosted resource smoke passed; GitHub Actions Resource Load Smoke run `26531698094` passed. Signed-in live Work Order Detail renderer smoke passed with new cache tags and visible status, Quick Update, edit, completion, parts, photo, comment, history, and delete contracts. LFES catch: the first module version read detail-only dependency getters before the missing-work-order early return; the smoke caught it and the module now preserves the old missing-work-order behavior.
- 2026-05-27: Six LFES state-adapter cleanup phases completed and deployed in `0512029`. `app.js` moved from 8,126 to 8,059 lines. Changed boundaries: filter/pagination state adapter, inventory filter state adapter, part-search state adapter, team work-view redundant storage writes, message load redundant storage writes, and message composer redundant storage writes. Verification passed: static checks for `app.js` and changed modules; workspace filter/pagination state smoke; inventory filter state smoke; part-search smoke using production `workspaceUiState` persistence; workspace UI/search/filter/inventory regression smokes; section navigation smoke; message UI/thread smokes; team invite cancel smoke; hosted resource smoke; GitHub Actions Resource Load Smoke run `26531041777`. Signed-in live shell/cache smoke passed; deeper live click proof was not counted because stale workspace search state could not be cleared reliably through the in-app browser automation. Scope stayed client-only UI state persistence cleanup only.
- 2026-05-27: App-state and LFES-core review found a second active-doc drift after the auth callback phase: `app.js` is 8,126 lines, while several active docs still carried state-era counts or older workspace-search LFES status. Updated `docs/CURRENT_HANDOFF.md`, `docs/NEXT_STEPS.md`, `docs/LFES/audits/APP_JS_MODULARIZATION_PLAN.md`, and `docs/LFES/context/CODEX_LFES_EXECUTION_HANDOFF.md` so current app, RLS, auth, line-count, and LFES next-step guidance flow together.
- 2026-05-27: Documentation drift cleanup completed after the RLS and auth verification checkpoints. Reconciled `docs/NEXT_STEPS.md`, `docs/CURRENT_HANDOFF.md`, `docs/LFES/audits/RLS_LIVE_CHECKPOINT_2026-05-27.md`, and `docs/SUPABASE_SETUP.md` so current auth/RLS status no longer conflicts with old `e7c1a70` Actions-verifier wording or pre-fixture QA-location text. Added `docs/LFES/context/DOCUMENTATION_DRIFT_REVIEW_2026-05-27.md` as the review artifact for this cleanup.
- 2026-05-27: Fresh real-world signup verification path passed for the QA account path. A new MaintainOps signup for the Taylor Metal test address with display name `QA create user path passed` generated a verification email, the newest verification link returned through the MaintainOps app flow, and account creation/verification completed successfully. Do not store test passwords in docs.
- 2026-05-27: HIGH-risk auth verification callback phase implemented and deployed in commit `41a0fe9`. Added `src/utils/authRedirects.js`, `auth/callback/index.html`, and `auth/callback/callback.js`; signup now sends `emailRedirectTo` to `/auth/callback/`; root app startup now handles auth callback `code` or signup hash tokens without misrouting signup verification into password recovery; password recovery remains on the main app path. Supabase Auth URL Configuration was updated: Site URL `https://loufish727.github.io/MaintainOps/`, Redirect URL `https://loufish727.github.io/MaintainOps/auth/callback/`. Static checks, `tests/smoke/auth-callback-smoke.js`, local callback UI smoke, local root signup-token fallback smoke, local resource smoke, hosted resource smoke, hosted callback invalid-link smoke, signed-in hosted normal workspace boot, and GitHub Actions Resource Load Smoke #253 passed.
