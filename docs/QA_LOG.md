...
- 2026-05-27: LFES active-section state wiring pass completed locally. `activeSection` now initializes from `workspaceUiState`, active-section writes go through `setActiveSectionState(...)`, and the app cache tag is `app.js?v=lfes-state-active-section-1`. Static checks, workspace state/navigation/search/filter smokes, command/navigation regression smokes, hosted resource smoke, local boot smoke with the new cache tag, and GitHub Actions verifier for `505e9da` passed.
