...
- 2026-05-27: LFES message UI state wiring pass staged for deployment. `activeMessageThreadId`, `messageThreadFilter`, `messageSearchQuery`, `messageComposerWorkOrderId`, and `messageComposerOpen` now initialize from `workspaceUiState`, app-owned message setters synchronize through the factory, and the pending cache tag is `lfes-state-message-ui-1` for both `workspaceUiState.js` and `app.js`. Scope is state synchronization only: no read-state mutation, create-thread, send-reply, message data, render, auth/company/location state, or Supabase access changes.
