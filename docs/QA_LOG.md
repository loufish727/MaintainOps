...
- 2026-05-27: Hard-boundary Message Center renderer extraction completed locally. Added `src/render/messageCenterDisplay.js` and `tests/smoke/message-center-display-smoke.js`, wired `app.js` through explicit `createMessageCenterDisplayHelpers(...)` dependency injection, and added the module to `index.html` / resource smoke. `app.js` moved from 7,783 to 7,706 lines. Static checks passed; Message Center display smoke passed; Message UI, Message Thread, and Work Message Start event regression smokes passed. LFES catch: the first module version read detail-only dependency getters before the messages-not-ready fallback; the module now preserves the old schema/setup fallback before reading message state dependencies.
