...
- 2026-05-27: LFES active-asset state wiring pass staged for deployment. `activeAssetId` now initializes from `workspaceUiState`, active-asset writes go through `setActiveAssetIdState(...)`, and the pending app cache tag is `app.js?v=lfes-state-active-asset-1`. Scope is state synchronization only: no Equipment selectors, delete implementation, Quick Fix submit, forms, render, auth/company/location state, or Supabase access changes.
