...
For form changes, verify both browser validation and the real submit path. A field can look correct but still block submit before Supabase sees the request, especially when a JavaScript-rendered `pattern` uses backslashes; inspect the actual submitted result and the refreshed list/detail state, not only the filled form.
...
```text
file:///C:/Users/louie/Documents/Codex/2026-04-28/MaintainOps/index.html?qa_bust=debug-YYYYMMDD-HHMM
```
...
- For work/request/PM forms that include equipment, verify that choosing equipment from another location shows the cross-location routing warning before save. If the user continues, the record should save under the equipment's location; if the user cancels, the record should not save.
