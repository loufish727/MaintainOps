...
const locationStateSource = readFileSync("src/appShell/companyLocationState.js", "utf8");
...
assert.match(locationStateSource, /function companyOptionLabel\(company\)/, "company options should have a disambiguating label helper");
assert.match(locationStateSource, /duplicateCount > 1 \? `\$\{name\} \(\$\{String\(company\.id \|\| ""\)\.slice\(0, 8\)\}\)` : name/, "duplicate company names should show an id suffix");
