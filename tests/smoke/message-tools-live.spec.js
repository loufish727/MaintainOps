...
test('message tools: search, organization, discussions, private files and voice drafts',async({browser,request})=>{
  ...
  try {
    ...
    await page.getByRole('button',{name:'Record voice message',exact:true}).click();await expect.poll(()=>page.evaluate(()=>window.__micCalls)).toBe(1);
    ...
    await page.getByRole('button',{name:'Record voice message',exact:true}).click();await page.getByRole('button',{name:'Stop',exact:true}).click();await expect(page.locator('.message-pending-file')).toContainText('Voice message.wav');
    ...
    await page.getByRole('button',{name:'Send reply',exact:true}).click();await expect(page.locator('#message-reply-error')).toContainText('Injected commit failure');
    ...
    await page.getByRole('button',{name:'Send reply',exact:true}).click();await expect(page.locator('.message-file')).toHaveCount(3,{timeout:30000});await expect(page.locator('.message-pending-file')).toHaveCount(0);
