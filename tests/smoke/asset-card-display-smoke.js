...
const { renderAssetCard } = createAssetCardDisplayHelpers({
  ...
  assetStatusLabel: (status) => ({ running: "Running", watch: "Watch", degraded: "Degraded", offline: "Offline / Down" }[status] || status),
  getWorkOrders: () => [
...
assert.match(html, /<small>Status<\/small><strong>Running<\/strong>/);
assert.match(html, /<small>Sub<\/small><strong>1<\/strong>/);
assert.match(html, /<small>Parts<\/small><strong>2<\/strong>/);
assert.match(html, /<small>Work<\/small><strong>2<\/strong>/);
assert.match(html, /<small>Files<\/small><strong>2<\/strong>/);
