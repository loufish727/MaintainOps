...
assert.match(html, /Existing equipment/);
...
assert.equal((html.match(/data-open-date-picker/g) || []).length, 2);
...
const quickUpdateFormHtml = html.match(/<form[^>]+id="quick-update-work-order-form"[\s\S]*?<\/form>/)?.[0] || "";
const fullDetailsFormHtml = html.match(/<form[^>]+id="edit-work-order-form"[\s\S]*?<\/form>/)?.[0] || "";
assert.match(quickUpdateFormHtml, /name="asset_id"/);
assert.doesNotMatch(fullDetailsFormHtml, /name="asset_id"/);
