...
assert.match(html, /CIDAN \/ Forstner Cut-To-Length Line Flow/);
assert.match(html, /78 charts \/ 12 per page/);
...
assert.match(html, /CIDAN \/ Forstner Cut-To-Length Line Flow[\s\S]*Measuring wheel \/ encoder[\s\S]*compare command to cut length/);
assert.match(html, /CIDAN \/ Forstner Cut-To-Length Line Flow[\s\S]*Slitting knives[\s\S]*measure width \+ burr side/);
assert.match(html, /CIDAN\/Forstner machine manuals, coil processing setup notes, material spec, first-piece inspection records/);
