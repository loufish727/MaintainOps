...
assert.ok(BOLT_REFERENCE.filter((row) => row.teaching).length >= 10);
assert.ok(WRENCH_REFERENCE.filter((row) => row.teaching).length >= 9);
assert.equal(BOLT_REFERENCE.find((row) => row.threads === "1/4-20 UNC")?.signal, "Very common");
assert.equal(WRENCH_REFERENCE.find((row) => row.thread === "1/4" && row.wrenchIn === "7/16")?.signal, "Very common");
...
assert.match(html, /Explain 1\/4-20 UNC/);
assert.match(html, /1\/4-20 is one of the most common inch coarse threads/);
assert.match(html, /Very common/);
assert.match(html, /bolt-reference-detail/);
assert.match(html, /bolt-reference-row-detail/);
assert.match(html, /bolt-reference-line-detail/);
...
assert.match(html, /Explain 1\/4 \/ 7\/16/);
assert.match(html, /Thread diameter and wrench size are different measurements/);
...
assert.match(html, /Drill \/ Tap Quick Reference[\s\S]*#10-24 and #10-32 share the same screw number/);
assert.match(html, /Wire Gauge Reference[\s\S]*For 30 amp and above, stop using memory/);
