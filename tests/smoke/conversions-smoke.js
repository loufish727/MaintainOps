...
assert.match(html, /<td data-label="Verify by">read code \+ OEM tree<\/td>/);
assert.match(html, /<td data-label="Verify by">dry run \+ active modal check<\/td>/);
...
assert.match(html, /SPN: same SPN with different FMI, inactive history, manufacturer-specific code text/);
assert.match(html, /SPN: same SPN with different FMI, inactive history, manufacturer-specific code text\. Verify before replacing, tightening, wiring, or ordering\./);
...
assert.doesNotMatch(sparkSection, /Details for row above: Normal/);
assert.match(sparkSection, /Details for row above: Dry carbon fouling/);
assert.match(sparkSection, /Details for row above: Ash deposits/);
assert.match(sparkSection, /Details for row above: Overheated \/ blistered/);
...
assert.doesNotMatch(hydraulicFluidSection, /Details for row above: Clear \/ normal color/);
assert.match(hydraulicFluidSection, /Details for row above: Milky \/ cloudy oil/);
...
assert.doesNotMatch(extensionCordSection, /Details for row above: 14 AWG/);
assert.match(extensionCordSection, /Details for row above: 16 AWG/);
