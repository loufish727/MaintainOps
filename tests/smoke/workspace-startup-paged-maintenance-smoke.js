...
(async () => {
  ...
  test("PM visibility follows relocated and traveling equipment without rewriting schedule data", async () => {
    const schedules = [
    ...
    assert.equal(result.scheduleResponse.data[1].location_id, "new");
    assert.equal(schedules[0].location_id, "old");
    assert.equal(schedules[1].location_id, "old");
  });
