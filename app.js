...
import {
  ...
  isAuthCallbackParams as isAuthCallbackParamsFromRouting,
  isPasswordRecoveryParams as isPasswordRecoveryParamsFromRouting,
  passwordRecoveryParamsFromUrl as passwordRecoveryParamsFromRouting,
  publicRequestQrTokenFromUrl as publicRequestQrTokenFromRouting,
  publicRequestTokenFromUrl as publicRequestTokenFromRouting,
} from "./src/appShell/startupRouting.js";
...
const { bindWorkspacePlanningDueDateEvents } = window.MaintainOpsWorkspacePlanningDueDateEvents;
...
const lazyResourceHelpers = createLazyResourceHelpers({
  windowRef: window,
  documentRef: document,
  escapeHtml,
...
let planningWorkOrdersLoadPromise = null;
let planningWorkOrdersLoadToken = 0;
...
async function init() {
  ...
  try {
    const startup = await initializeStartupRoute({
      ...
      setSupabaseClient: (value) => { supabaseClient = value; },
      setSession: (value) => { session = value; },
    });
...
function passwordRecoveryParamsFromUrl() {
  return passwordRecoveryParamsFromRouting({
    windowRef: window,
    authRedirects: window.MaintainOpsAuthRedirects,
  });
}
...
function passwordRecoveryParamsFromHref(href) {
  return authParamsFromHref(window.MaintainOpsAuthRedirects, href);
}
...
function isPasswordRecoveryParams(params) {
  return isPasswordRecoveryParamsFromRouting(window.MaintainOpsAuthRedirects, params);
}

function isAuthCallbackParams(params) {
  return isAuthCallbackParamsFromRouting(window.MaintainOpsAuthRedirects, params);
}
...
function publicRequestTokenFromUrl() {
  return publicRequestTokenFromRouting(window);
}

function publicRequestQrTokenFromUrl() {
  return publicRequestQrTokenFromRouting(window);
}
...
async function loadPlanningWorkOrders(options = {}) {
  ...
  if (
    ...
    ((dueResponse.error && isColumnSchemaError(dueResponse.error, ["location_id", "locations"])) ||
      (noDueResponse.error && isColumnSchemaError(noDueResponse.error, ["location_id", "locations"])) ||
...
async function ensurePlanningWorkOrdersLoaded(options = {}) {
  ...
  planningWorkOrdersReady = false;
  planningWorkOrdersError = "";
  const loadPromise = loadWorkspaceResponse("Planning work orders", loadPlanningWorkOrders(), 16000)
    .then((response) => {
...
function renderWorkspace() {
  ...
  if (recoveredDraft?.composerOpen) {
    ...
    setMessageComposerWorkOrderIdState(workOrders.some(order => order.id === recoveredDraft.workOrderId) ? recoveredDraft.workOrderId : "");
  }
  for (const [threadId, quoteId] of recoveredDraft?.quotes || []) {
    const quote = messageHistory[threadId]?.rows.find(row => row.id === quoteId && !row.deleted_at);
    if (quote) messageQuotes[threadId] = quote;
  }
  ...
  const assetInventoryControls = activeSection === "assets" && !showGlobalSearch
    ? window.MaintainOpsAssetInventoryDisplay.renderAssetInventoryControls({ assets, matchesActiveLocation, workspaceUiState, ASSET_TYPE_OPTIONS, escapeHtml }) : "";
  ...
  const teamSectionIsOpen = (sectionId) => existingTeamSections.length
    ? openTeamSectionIds.has(sectionId)
    : sectionId === "members";
          ...
          <section class="panel full-width">
            <div class="panel-header">
              ...
              <span>${planningWorkOrdersReady
                ? (planningWorkOrdersError ? "Unavailable" : `${planningItems().length + planningItems("no_due").length + followUpItems().length + planningPmItems().length} queue items`)
                : "Loading queue"}</span>
            </div>
...
function filteredWorkOrders() {
  return workOrders.filter((workOrder) => {
    ...
    const typeMatch = activeSection !== "work"
      || workOrderTypeFilter === "all"
      || normalizeWorkOrderType(workOrder.type) === workOrderTypeFilter;
    const priorityMatch = activeSection !== "work"
      || workOrderPriorityFilter === "all"
      || workOrder.priority === workOrderPriorityFilter;
    return statusMatch && queueMatch && typeMatch && priorityMatch && matchesSearch(workOrderSearchValues(workOrder));
...
function initializeMessagesFeature() {
  ...
  messageWorkflow = window.MaintainOpsMessageWorkflow.createMessageWorkflow({
    ...
    clearDraft: (key, submitted, scope) => messageDrafts.clear(document, key, submitted, scope),
